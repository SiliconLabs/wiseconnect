...
#define NCP_HOST_BLE_NOTIFICATION_EVENT SL_SI91X_NOTIFICATION_FLAG(SI91X_BT_CMD)
...
static sli_si91x_socket_t *get_socket_from_packet(sl_si91x_packet_t *socket_packet)
{
  ...
  if (socket_packet->command == RSI_WLAN_RSP_SOCKET_CREATE) {
    const sl_si91x_socket_create_response_t *socket_create_response =
                                               ...
                                               sl_wifi_buffer_t *packet)
{
  ...
  buffer->id              = (uint8_t)cmd_queues[SI91X_WLAN_CMD].packet_id;
  packet->id              = (uint8_t)cmd_queues[SI91X_WLAN_CMD].packet_id;
  ...
  if ((*event & SL_SI91X_NCP_HOST_BUS_RX_EVENT)
    ...
    switch (queue_id) {
        ...
        switch (frame_type) {
          ...
          case RSI_WLAN_RSP_SET_MULTICAST_FILTER: {
            ...
            if (((cmd_queues[SI91X_WLAN_CMD].flags & SI91X_PACKET_RESPONSE_STATUS) == SI91X_PACKET_RESPONSE_STATUS)
              ...
              if (sl_si91x_host_elapsed_time(cmd_queues[SI91X_WLAN_CMD].command_tickcount)
                ...
                // no user thread is waiting for the response so flush the packet
                sl_si91x_host_free_buffer(packet);
                if ((cmd_queues[SI91X_WLAN_CMD].flags & SI91X_PACKET_RESPONSE_PACKET) == SI91X_PACKET_RESPONSE_PACKET) {
                  sl_si91x_host_free_buffer(buffer);
                }
              }
              cmd_queues[SI91X_WLAN_CMD].command_tickcount = 0;
          ...
          case RSI_WLAN_RSP_MDNSD: {
            ...
            if ((frame_type == cmd_queues[SI91X_NETWORK_CMD].frame_type)
              ...
              packet->id        = (uint8_t)cmd_queues[SI91X_NETWORK_CMD].packet_id;
              buffer->id        = (uint8_t)cmd_queues[SI91X_NETWORK_CMD].packet_id;
            } else {
            ...
            if (((frame_type == RSI_WLAN_RSP_MQTT_REMOTE_TERMINATE)
              ...
              error_packet->id        = (uint8_t)cmd_queues[SI91X_NETWORK_CMD].packet_id;
      ...
      case RSI_BT_Q: {
        ...
        sli_si91x_add_to_queue(&cmd_queues[SI91X_BT_CMD].event_queue, buffer);
...
void sli_si91x_process_wifi_events()
{
  ...
  while (sl_si91x_host_queue_status(&cmd_queues[SI91X_WLAN_CMD].event_queue) != 0) {
    ...
    if (sli_si91x_remove_from_queue(&cmd_queues[SI91X_WLAN_CMD].event_queue, &buffer) == SL_STATUS_OK) {
      ...
      uint16_t frame_status     = get_si91x_frame_status(packet);
...
void sli_si91x_process_socket_events()
{
  ...
  while (sl_si91x_host_queue_status(&cmd_queues[SI91X_SOCKET_CMD].event_queue) != 0) {
...
void sli_si91x_process_ble_events()
{
  ...
  while (sl_si91x_host_queue_status(&cmd_queues[SI91X_BT_CMD].event_queue) != 0) {
    if (sli_si91x_remove_from_queue(&cmd_queues[SI91X_BT_CMD].event_queue, &buffer) == SL_STATUS_OK) {
      sl_si91x_packet_t *packet = (sl_si91x_packet_t *)sl_si91x_host_get_buffer_data(buffer, 0, NULL);
