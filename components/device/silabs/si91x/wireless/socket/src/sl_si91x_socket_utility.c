...
{
  const sl_si91x_packet_t *packet = (sl_si91x_packet_t *)buffer->data;
...
int sli_si91x_accept(int socket, struct sockaddr *addr, socklen_t *addr_len, sl_si91x_socket_accept_callback_t callback)
{
  ...
  const sl_si91x_rsp_ltcp_est_t *ltcp = (sl_si91x_rsp_ltcp_est_t *)packet->data;
                                       ...
                                       sl_si91x_packet_t *rx_packet)
{
  ...
  // Handle remote socket termination response
    ...
    const sl_si91x_socket_select_rsp_t *socket_select_rsp = (sl_si91x_socket_select_rsp_t *)rx_packet->data;
...

{
  ...
  if (socket->state == DISCONNECTED && command == RSI_WLAN_REQ_SOCKET_READ_DATA) {
  ...
  if (wait_period != 0) {
    ...
    if (command == RSI_WLAN_REQ_SOCKET_READ_DATA) {
      packet          = (sl_si91x_packet_t *)sl_si91x_host_get_buffer_data(*response_buffer, 0, NULL);
...
uint8_t sli_prepare_select_request(int nfds,
                                   const fd_set *readfds,
                                   const fd_set *writefds,
                                   sl_si91x_socket_select_req_t *request)
{
...
void sli_handle_timeout(const struct timeval *timeout, sl_si91x_socket_select_req_t *request, uint32_t *wait_time)
{
...
sl_status_t sli_wait_for_select_response(const sl_si91x_socket_select_req_t *request, uint32_t wait_time)
{
                                 ...
                                 fd_set *exceptfds)
{
  ...
  if (select_request_table[select_request->select_id].frame_status == SL_STATUS_OK) {
    total_fd_set_count = handle_select_response(select_request_table[select_request->select_id].response_data,
                                                readfds,
                                                writefds,
                                                exceptfds);
  }
