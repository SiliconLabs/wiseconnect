 ...
 ******************************************************/
static si91x_socket_t *sli_si91x_sockets[NUMBER_OF_SOCKETS]                       = { 0 };
...
void reset_socket_state(int socket)
{
  if (sli_si91x_sockets[socket] != NULL) {
    if (NULL != sli_si91x_sockets[socket]->socket_events) {
...
static si91x_socket_t *get_si91x_server_socket(uint16_t src_port)
{
  int i = 0;

  for (i = 0; i < NUMBER_OF_SOCKETS; i++) {
    if (NULL != sli_si91x_sockets[i]) {
      if ((SI91X_SOCKET_TCP_SERVER == sli_si91x_sockets[i]->role)
...
static uint16_t get_socket_id_from_socket_command(sl_si91x_packet_t *packet)
{
  switch (packet->command) {
    case RSI_WLAN_REQ_SOCKET_READ_DATA: {
      const sl_si91x_req_socket_read_t *read_request = (sl_si91x_req_socket_read_t *)(packet->data);
      return read_request->socket_id;
    }
    case RSI_WLAN_REQ_SOCKET_CLOSE: {
      const sl_si91x_socket_close_request_t *socket_close_request = (sl_si91x_socket_close_request_t *)(packet->data);
      return socket_close_request->socket_id;
    }
    case RSI_WLAN_REQ_SOCKET_ACCEPT: {
      const sl_si91x_socket_accept_request_t *accept_request = (sl_si91x_socket_accept_request_t *)(packet->data);
...
static uint8_t si91x_socket_identification_function(sl_wifi_buffer_t *buffer, void *user_data)
{
  ...
  const sl_si91x_socket_close_response_t *remote_socket_closure = (sl_si91x_socket_close_response_t *)user_data;
...
sl_status_t sli_si91x_sync_accept_command(si91x_socket_t *server_socket, void *cmd, uint32_t cmd_length)
{
  ...
  server_socket->user_accept_callback = NULL;
  // Set the callback and client socket ID
  status = sl_si91x_driver_send_async_command(RSI_WLAN_REQ_SOCKET_ACCEPT, SI91X_SOCKET_CMD_QUEUE, cmd, cmd_length);
  SOCKET_VERIFY_STATUS_AND_RETURN(status, SL_STATUS_OK, SI91X_UNDEFINED_ERROR);

  events = osEventFlagsWait(server_socket->socket_events,
                            (SLI_SI91X_SOCKET_ACCEPT_SUCCESS_EVENT | SLI_SI91X_SOCKET_ACCEPT_FAILURE_EVENT),
                            osFlagsWaitAny,
                            osWaitForever);
  if (events & SLI_SI91X_SOCKET_ACCEPT_SUCCESS_EVENT) {
    // handle data;
  } else if (events & SLI_SI91X_SOCKET_ACCEPT_FAILURE_EVENT) {
    // handle error;
  } else {
...
int sli_si91x_shutdown(int socket, int how)
{
  ...
  for (uint8_t index = 0; index < NUMBER_OF_SOCKETS; index++) {
    si91x_socket_t *socket_id = get_si91x_socket(index);
    //Verifying socket existence
    if (socket_id == NULL)
      continue;
    if (close_request_type == SHUTDOWN_BY_ID && socket_id->id == socket_close_response->socket_id) {
                                       ...
                                       sl_si91x_packet_t *rx_packet)
{
  ...
  if (rx_packet->command == RSI_WLAN_RSP_CONN_ESTABLISH) {
    ...
    si91x_socket_t *server_socket            = get_si91x_server_socket(accept_response->src_port_num);
    ...
    if (NULL != server_socket->user_accept_callback) {
      ...
      osEventFlagsSet(server_socket->socket_events, SLI_SI91X_SOCKET_ACCEPT_SUCCESS_EVENT);
    }
  }
  // Handle remote socket termination response
    ...
    for (uint8_t index = 0; index < NUMBER_OF_SOCKETS; index++) {
      ...
      sl_si91x_host_flush_nodes_from_queue(SI91X_SOCKET_CMD_QUEUE,
                                           ...
                                           (sl_si91x_compare_function_t)si91x_socket_identification_function,
    ...
    const si91x_socket_t *client_socket = get_si91x_socket(host_socket);
    //Verifying socket existence
