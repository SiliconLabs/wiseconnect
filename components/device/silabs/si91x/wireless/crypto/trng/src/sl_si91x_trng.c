...
#else
#include "cmsis_os2.h"
static osMutexId_t sli_trng_os_mutex = NULL;
#endif
#include <string.h>
...
sl_status_t sl_si91x_trng_get_random_num(uint32_t *random_number, uint16_t length)
{
  ...
  sli_si91x_trng_request_t request;
  ...
  mutex_result = sl_si91x_crypto_mutex_acquire(crypto_trng_mutex);
#else
  if (sli_trng_os_mutex == NULL) {
    sli_trng_os_mutex = osMutexNew(NULL);
  }
  if (sli_trng_os_mutex != NULL) {
    (void)osMutexAcquire(sli_trng_os_mutex, osWaitForever);
  }
#endif
#ifdef SL_SI91X_SIDE_BAND_CRYPTO
...
#else
    if (sli_trng_os_mutex != NULL) {
      (void)osMutexRelease(sli_trng_os_mutex);
    }
#endif
  }
  VERIFY_STATUS_AND_RETURN(status);
