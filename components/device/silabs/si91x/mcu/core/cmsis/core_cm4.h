...
{
  uint32_t reg_value;
  uint32_t PriorityGroupTmp = (PriorityGroup & (uint32_t)0x07U);               /* only values 0..7 are used          */
...
/*  NVIC->ISER[((uint32_t)(IRQn) >> 5UL)] = (1UL << ((uint32_t)(IRQn) & 0x1FUL));  enable interrupt */
  NVIC->ISER[(uint32_t)((int32_t)IRQn) >> 5UL] = (uint32_t)(1UL << ((uint32_t)((int32_t)IRQn) & (uint32_t)0x1FUL)); /* enable interrupt */
}
...
{
  NVIC->ICER[((uint32_t)(IRQn) >> 5UL)] = (1UL << ((uint32_t)(IRQn) & 0x1FUL)); /* disable interrupt */
}
...
{
  return((uint32_t) ((NVIC->ISPR[(uint32_t)(IRQn) >> 5UL] & (1UL << ((uint32_t)(IRQn) & 0x1FUL)))?1UL:0UL)); /* Return 1 if pending else 0 */
}
...
{
  NVIC->ISPR[((uint32_t)(IRQn) >> 5UL)] = (1UL << ((uint32_t)(IRQn) & 0x1FUL)); /* set interrupt pending */
}
...
{
  NVIC->ICPR[((uint32_t)(IRQn) >> 5UL)] = (1UL << ((uint32_t)(IRQn) & 0x1FUL)); /* Clear pending interrupt */
}
...
{
  return((uint32_t)((NVIC->IABR[(uint32_t)(IRQn) >> 5UL] & (1UL << ((uint32_t)(IRQn) & 0x1FUL)))?1UL:0UL)); /* Return 1 if active else 0 */
}
...
{
  uint32_t PriorityGroupTmp = (PriorityGroup & 0x07UL);          /* only values 0..7 are used          */
  uint32_t PreemptPriorityBits;
  uint32_t SubPriorityBits;

  PreemptPriorityBits = ((7UL - PriorityGroupTmp) > __NVIC_PRIO_BITS) ? __NVIC_PRIO_BITS : 7UL - PriorityGroupTmp;
  SubPriorityBits     = ((PriorityGroupTmp + __NVIC_PRIO_BITS) < 7UL) ? 0UL : PriorityGroupTmp - 7UL + __NVIC_PRIO_BITS;
...
{
  if (ticks > SysTick_LOAD_RELOAD_Msk)  return (1UL);            /* Reload value impossible */

  SysTick->LOAD  = (ticks & SysTick_LOAD_RELOAD_Msk) - 1UL;      /* set reload register */
