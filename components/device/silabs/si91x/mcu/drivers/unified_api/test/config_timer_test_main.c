...
void test_config_timer_set_ocu_configuration(void)
{
  ...
  ocu_config_handle1.is_counter1_ocu_8bit_mode_enabled      = true;
...
void test_config_timer_select_action_event(void)
{
  ...
  status = sl_si91x_config_timer_select_action_event(ACTION_LAST, SL_CT_EVENT_LAST, SL_EVENT0_FALLING_EDGE);
  TEST_ASSERT_EQUAL_HEX(SL_STATUS_INVALID_PARAMETER, status);
  ...
  status = sl_si91x_config_timer_select_action_event(START, SL_EVENT0_RISING_EDGE, SL_CT_EVENT_LAST);
  TEST_ASSERT_EQUAL_HEX(SL_STATUS_INVALID_PARAMETER, status);
