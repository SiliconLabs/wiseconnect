...
#define MAX_WORD_LENGTH             65536     //Maximum output value of 16 bit data
#define MAX_ADC_CHANNEL_SAMPLE_RATE 2500000   // Maximum ADC channel sample rate.
#define M4SS_MEM
#ifdef ULPSS_MEM
...
#endif
#define SL_ADC_POWER_MODE_LAST 2 // Invalid value for power state.
...
void test_adc_set_channel_configuration(void);
void test_adc_channel_sampling_rate(void);
void test_adc_register_event_callback(void);
...
int main()
{
  ...
  RUN_TEST(test_adc_set_channel_configuration, __LINE__);
  ...
  RUN_TEST(test_adc_register_event_callback, __LINE__);
...
void test_adc_init(void)
{
  ...
  status = sl_si91x_adc_init(adc_channel_config, adc_config, vref_value);
  ...
  vref_value                          = 3.3;
  adc_channel_config.sampling_rate[0] = MAX_ADC_CHANNEL_SAMPLE_RATE + 1;
  status                              = sl_si91x_adc_init(adc_channel_config, adc_config, vref_value);
...
void test_adc_power_control(void)
{
  ...
  status = sl_si91x_adc_set_power_mode(ADC_POWER_OFF);
  TEST_ASSERT_EQUAL_HEX(SL_STATUS_OK, status);
  ...
  status = sl_si91x_adc_set_power_mode(SL_ADC_POWER_MODE_LAST);
  TEST_ASSERT_EQUAL_HEX(SL_STATUS_INVALID_PARAMETER, status);
...
{
  UnityPrintf("\n");
  UnityPrintf("Testing ADC channel configuration \n");
  sl_status_t status;
  ...
  default_adc_channel_config_struct(&adc_channel_config);
  adc_channel_config.sampling_rate[0] = MAX_ADC_CHANNEL_SAMPLE_RATE + 1;
  status                              = sl_si91x_adc_set_channel_configuration(adc_channel_config, adc_config);
  TEST_ASSERT_EQUAL_HEX(SL_STATUS_INVALID_RANGE, status);
  UnityPrintf("Status of API is correct, ADC maximum sampling rate Test Passed successfully \n");
  ...
  UnityPrintf("ADC channel configuration test completed \n ");
}

/*******************************************************************************
 * Function to test the ADC register event callback function.
 ******************************************************************************/
void test_adc_register_event_callback(void)
{
  UnityPrintf("\n");
  UnityPrintf("Testing ADC register event callback function \n");
  sl_status_t status;

  UnityPrintf("Testing with NULL pointer \n");
  status = sl_si91x_adc_register_event_callback(NULL);
  TEST_ASSERT_EQUAL_HEX(SL_STATUS_NULL_POINTER, status);
  UnityPrintf("Status of API is correct, Null Pointer Test Passed successfully \n");
  ...
  TEST_ASSERT_EQUAL_HEX(SL_STATUS_OK, status);
  UnityPrintf("Status of API is correct, ADC registered event callback successfully \n");

  UnityPrintf("ADC register event callback test completed \n ");
}
...
void test_adc_configure_static_mode(void)
{
  ...
  default_adc_channel_config_struct(&adc_channel_config);
  adc_channel_config.sampling_rate[0] = MAX_ADC_CHANNEL_SAMPLE_RATE + 1;
  status                              = sl_si91x_adc_configure_static_mode(adc_channel_config, channel_num);
  TEST_ASSERT_EQUAL_HEX(SL_STATUS_INVALID_RANGE, status);
  UnityPrintf("Status of API is correct, ADC maximum sampling rate Test Passed successfully \n");
...
void test_adc_channel_sampling_rate(void)
{
  ...
  default_adc_internal_struct(&adc_internal_config);
  adc_internal_config.ping_addr[0] = ((uint32_t)0x2F400 + 1);
  status                           = sl_si91x_adc_configure_channel_sampling_rate(adc_internal_config, chnl_num);
  TEST_ASSERT_EQUAL_HEX(SL_STATUS_INVALID_PARAMETER, status);
  ...
  default_adc_internal_struct(&adc_internal_config);
  adc_internal_config.pong_addr[0] = ((uint32_t)0x2F7FF + 1);
  status                           = sl_si91x_adc_configure_channel_sampling_rate(adc_internal_config, chnl_num);
  TEST_ASSERT_EQUAL_HEX(SL_STATUS_INVALID_PARAMETER, status);
...
void test_adc_threshold_configuration(void)
{
  ...
  status                   = sl_si91x_adc_threshold_configuration(adc_threshold);
  TEST_ASSERT_EQUAL_HEX(SL_STATUS_OK, status);
  UnityPrintf("ADC threshold1 condition = %ld \n", adc_threshold.threshold1_cond);
  UnityPrintf("ADC threshold2 condition = %ld \n", adc_threshold.threshold2_cond);
...
void test_adc_read_data_static(void)
{
  ...
  sl_status_t status;
  uint16_t read_static_data;

  UnityPrintf("Testing with above maximum sampling rate instance \n");
  default_adc_config_struct(&adc_config);
  default_adc_channel_config_struct(&adc_channel_config);
  adc_channel_config.sampling_rate[0] = MAX_ADC_CHANNEL_SAMPLE_RATE + 1;
  status = sl_si91x_adc_read_data_static(adc_channel_config, adc_config, &read_static_data);
  TEST_ASSERT_EQUAL_HEX(SL_STATUS_INVALID_RANGE, status);
  UnityPrintf("Status of API is correct, ADC maximum sampling rate Test Passed successfully \n");
...
void test_adc_stop(void)
{
  ...
  UnityPrintf("Testing without ADC start \n");
  default_adc_config_struct(&adc_config);
  status = sl_si91x_adc_stop(adc_config);
  TEST_ASSERT_EQUAL_HEX(SL_STATUS_OK, status);
  UnityPrintf("Status of API is correct, ADC stop failed without ADC start Test Passed successfully \n");
...
static void default_adc_channel_config_struct(sl_adc_channel_config_t *adc_channel_config)
{
  ...
  adc_channel_config->opamp_gain[0]        = OPAMP_GAIN;
