...
extern uint32 hpldo_tran[];
extern uint32 buck_fast_transient_duty_1p8[];
...
void update_ipmu_calib_data(efuse_ipmu_t *ipmu_calib_data)
{
...
#endif
  //ROW 4,5,6
  //data = (ipmu_calib_data->trim_0p5na1 << 18) | (((ipmu_calib_data->bg_r_vdd_ulp >> 3) & 0x1) << 12) | (ipmu_calib_data->resbank_trim << 10);
  //https://confluence.silabs.com/display/RS9116RP/Release+V1.22+Porting+Guide .
#ifdef RSI_M4_INTERFACE
  ...
  //ROW 10
  update_ipmu_data(ULPCLKS_32KRO_CLK_REG_OFFSET, ULP_SPI, (ipmu_calib_data->freq_trim << 16), MASK_BITS(5, 16));
  ...
  *(volatile uint32 *)(ULP_TASS_MISC_CONFIG_REG + 0x34) = data;
  ...
  // Setting DPWM_FREQ_1P3 to 2MHz based on the data obtained from Calibration Data
#ifndef RSI_M4_INTERFACE
  if (glbl_common_info_ext_p->mfg_version >= 0x17) {
    data = (((ipmu_calib_data->dpwm_freq_trim)) << 13);
    mask = MASK_BITS(4, 13);
...
uint32 init_ipmu_calib_data(uint32 m4_present)
{
  ...
  cmemcpy((void *)ipmu_calib_data_p, (void *)(MANF_DATA_BASE_ADDR + IPMU_VALUES_OFFSET), sizeof(efuse_ipmu_t));
#ifndef RSI_M4_INTERFACE
