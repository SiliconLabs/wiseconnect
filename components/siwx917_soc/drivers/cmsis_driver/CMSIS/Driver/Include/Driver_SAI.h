...
#define ARM_SAI_MASK_SLOTS_TX \
  (0x05U) ///< Mask Transmitter slots; arg1 = mask (bit: 0=active, 1=inactive); all configured slots are active by default
#define ARM_SAI_MASK_SLOTS_RX \
  (0x06U) ///< Mask Receiver    slots; arg1 = mask (bit: 0=active, 1=inactive); all configured slots are active by default
...
#define ARM_SAI_FRAME_SYNC_WIDTH(n) \
  ((((n)-1) & 0xFFU)                \
   << ARM_SAI_FRAME_SYNC_WIDTH_Pos) ///< Frame Sync width in bits (1..256); default=1; User Protocol only (ignored otherwise)
...
#define ARM_SAI_FRAME_SYNC_POLARITY_HIGH \
  (0U                                    \
   << ARM_SAI_FRAME_SYNC_POLARITY_Pos) ///< Frame Sync is active high (default); User Protocol only (ignored otherwise)
...
#define ARM_SAI_SLOT_COUNT(n) \
  ((((n)-1) & 0x1FU)          \
   << ARM_SAI_SLOT_COUNT_Pos) ///< Number of slots in frame (1..32); default=1; User Protocol only (ignored otherwise)
...
#define ARM_SAI_SLOT_OFFSET(n) \
  (((n)&0x1FU)                 \
   << ARM_SAI_SLOT_OFFSET_Pos) ///< Offset of first data bit in slot (0..31); default=0; User Protocol only (ignored otherwise)
