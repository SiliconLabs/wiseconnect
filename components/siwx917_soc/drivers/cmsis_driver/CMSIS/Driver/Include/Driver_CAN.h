...
#define ARM_CAN_SET_FD_MODE \
  (1UL << ARM_CAN_CONTROL_Pos) ///< Set FD operation mode;                   arg: 0 = disable, 1 = enable
#define ARM_CAN_ABORT_MESSAGE_SEND \
  (2UL << ARM_CAN_CONTROL_Pos) ///< Abort sending of CAN message;            arg = object
#define ARM_CAN_CONTROL_RETRANSMISSION \
  (3UL                                 \
   << ARM_CAN_CONTROL_Pos) ///< Enable/disable automatic retransmission; arg: 0 = disable, 1 = enable (default state)
#define ARM_CAN_SET_TRANSCEIVER_DELAY \
  (4UL << ARM_CAN_CONTROL_Pos) ///< Set transceiver delay;                   arg = delay in time quanta
...
#define ARM_CAN_EXTENDED_ID(id) \
  ((id & 0x1FFFFFFFUL) | ARM_CAN_ID_IDE_Msk) ///< CAN identifier in extended format (29-bits)
...
typedef void (*ARM_CAN_SignalObjectEvent_t)(
  uint32_t obj_idx,
...
typedef struct _ARM_CAN_CAPABILITIES {
  ...
  uint32_t
    reentrant_operation : 1; ///< Support for reentrant calls to \ref ARM_CAN_MessageSend, \ref ARM_CAN_MessageRead, \ref ARM_CAN_ObjectConfigure and abort message sending used by \ref ARM_CAN_Control
