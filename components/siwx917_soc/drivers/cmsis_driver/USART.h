...
#define USART_OVERRUN_ERR \
  (0x01 << 1) /*!< This is used to indicate the occurrence of an overrun error. This occurs
                                                         if a new data character was received before the previous data was read.        */
#define USART_PARITY_ERR \
  (0x01 << 2) /*!< This is used to indicate the occurrence of a parity error in the receiver if
                                                         the Parity Enable                                                              */
#define USART_FRAMING_ERR \
  (0x01 << 3) /*!< This is used to indicate the occurrence of a framing error in the receiver.
                                                         A framing error occurs when the receiver does not detect a valid STOP bit 
                                                         in the received data                                                           */
#define USART_BREAK_ERR \
  (0x01 << 4) /*!< Break Interrupt bit. This is used to indicate the detection of a break sequence on the serial
                                                         input data.                                                                    */
#define USART_RECV_FIFO_ERR \
  (0x01 << 7) /*!< This is used to indicate if there is at least one parity error,
