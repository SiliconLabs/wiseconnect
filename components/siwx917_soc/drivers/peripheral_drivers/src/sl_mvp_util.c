...
extern __INLINE int sli_mvp_util_offset_nhwc(int height, int width, int depth, int n, int h, int w, int c);
                                    ...
                                    unsigned short cols)
{
  ...
  for (int batch = 0; batch < batches; ++batch) {
    ...
    sli_mvp_pb_config_array_nhwc(p->p,
                                 SLI_MVP_ARRAY(0),
                                 dst,
                                 parallel == true ? SLI_MVP_DATATYPE_COMPLEX_BINARY16 : SLI_MVP_DATATYPE_BINARY16,
                                 height,
                                 width,
                                 depth,
                                 &status);
