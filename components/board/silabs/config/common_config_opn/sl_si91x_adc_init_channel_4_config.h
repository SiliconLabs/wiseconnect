...
#if (!defined(SL_ADC_CH4_N_PORT) && (SL_ADC_CHANNEL_4_INPUT_TYPE == ADC_DIFFERENTIAL_INPUT))
#warning "Channel_4's ADC_N pin is not set up for a differential input type"
#endif
#else

#warning \
  "ADC Channel 4 pins are not configured. To configure, either install [ENABLE USER CONFIGURATION] component or define USER_CONFIGURATION_ENABLE macro to 1, then configure the pins as per the Custom board."

#endif // USER_CONFIGURATION_ENABLE
// Positive Input Channel Selection
#ifdef SL_ADC_CH4_P_PIN
#define SL_ADC_CHANNEL_4_POS_INPUT_CHNL_SEL \
  ((SL_ADC_CH4_P_PIN == 0)    ? 0           \
   : (SL_ADC_CH4_P_PIN == 2)  ? 1           \
   : (SL_ADC_CH4_P_PIN == 4)  ? 2           \
   : (SL_ADC_CH4_P_PIN == 6)  ? 3           \
   : (SL_ADC_CH4_P_PIN == 8)  ? 4           \
   : (SL_ADC_CH4_P_PIN == 10) ? 5           \
   : (SL_ADC_CH4_P_PIN == 25) ? 6           \
   : (SL_ADC_CH4_P_PIN == 27) ? 7           \
   : (SL_ADC_CH4_P_PIN == 29) ? 8           \
   : (SL_ADC_CH4_P_PIN == 1)  ? 10          \
   : (SL_ADC_CH4_P_PIN == 3)  ? 11          \
   : (SL_ADC_CH4_P_PIN == 5)  ? 12          \
   : (SL_ADC_CH4_P_PIN == 11) ? 13          \
   : (SL_ADC_CH4_P_PIN == 9)  ? 14          \
...
#define SL_ADC_CHANNEL_4_NEG_INPUT_CHNL_SEL \
  ((SL_ADC_CH4_N_PIN == 1)    ? 0           \
   : (SL_ADC_CH4_N_PIN == 3)  ? 1           \
   : (SL_ADC_CH4_N_PIN == 5)  ? 2           \
   : (SL_ADC_CH4_N_PIN == 11) ? 3           \
   : (SL_ADC_CH4_N_PIN == 9)  ? 4           \
