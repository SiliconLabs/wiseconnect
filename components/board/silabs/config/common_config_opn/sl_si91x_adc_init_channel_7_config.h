...
#if (!defined(SL_ADC_CH7_N_PORT) && (SL_ADC_CHANNEL_7_INPUT_TYPE == ADC_DIFFERENTIAL_INPUT))
#warning "Channel_7's ADC_N pin is not set up for a differential input type"
#endif
#else

#warning \
  "ADC Channel 7 pins are not configured. To configure, either install [ENABLE USER CONFIGURATION] component or define USER_CONFIGURATION_ENABLE macro to 1, then configure the pins as per the Custom board."
