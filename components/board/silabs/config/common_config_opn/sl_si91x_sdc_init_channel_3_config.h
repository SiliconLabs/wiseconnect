...
#if (!defined(SL_SDC_CH3_N_PORT) && (SL_SDC_CHANNEL_3_INPUT_TYPE == SL_SDC_DIFFERENTIAL_MODE))
#warning "Channel_3's SDC_N pin is not set up for a differential input type"
#endif

#else

#warning \
  "SDC Channel 3 pins are not configured. To configure, either install [ENABLE USER CONFIGURATION] component or define USER_CONFIGURATION_ENABLE macro to 1, then configure the pins as per the Custom board."
...
#define SL_SDC_CHANNEL_3_POS_INPUT_CHNL_SEL \
  ((SL_SDC_CH3_P_PIN == 0)    ? 0           \
   ...
   : (SL_SDC_CH3_P_PIN == 25) ? 6           \
   : (SL_SDC_CH3_P_PIN == 27) ? 7           \
   : (SL_SDC_CH3_P_PIN == 29) ? 8           \
   ...
   : (SL_SDC_CH3_P_PIN == 26) ? 16          \
   : (SL_SDC_CH3_P_PIN == 28) ? 17          \
   : (SL_SDC_CH3_P_PIN == 30) ? 18          \
                              : -1)
#else
...
#define SL_SDC_CHANNEL_3_NEG_INPUT_CHNL_SEL \
  ((SL_SDC_CH3_N_PIN == 1)    ? 0           \
   ...
   : (SL_SDC_CH3_N_PIN == 26) ? 6           \
   : (SL_SDC_CH3_N_PIN == 28) ? 7           \
   : (SL_SDC_CH3_N_PIN == 30) ? 8           \
                              : -1)
#else
