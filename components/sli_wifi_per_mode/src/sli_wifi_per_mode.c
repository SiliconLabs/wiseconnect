                                 ...
                                 sli_wifi_get_txircal_config_t *tx_ir_calib)
{
  ...
  memcpy(&tx_ir_calibration_request.TxIRCal_config, &tx_ir_conf, sizeof(sli_wifi_txircal_config_t));
  ...
  if ((tx_ir_calib != NULL) && (packet != NULL)) {
    memcpy(tx_ir_calib, packet->data, packet->length);
    sli_wifi_memory_manager_free_buffer(buffer);
  }
...
sl_status_t sli_wifi_rx_ir_calib(sl_wifi_interface_t interface, sli_wifi_RxIRCal_config_t rx_ir_calib)
{
  ...
  memcpy(&rx_ir_calibration_request.RxIRCal_config, &rx_ir_calib, sizeof(sli_wifi_RxIRCal_config_t));
...
sl_status_t sli_wifi_get_rx_ircal_data(sl_wifi_interface_t interface, sli_wifi_get_RxIRCal_config_t *get_rx_ircal)
{
  ...
  if ((get_rx_ircal != NULL) && (packet != NULL)) {
    memcpy(get_rx_ircal, packet->data, packet->length);
    sli_wifi_memory_manager_free_buffer(buffer);
  }
