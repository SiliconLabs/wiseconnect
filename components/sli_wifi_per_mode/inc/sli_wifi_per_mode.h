...
typedef struct {
  ...
  uint32_t TXDCOC_offset[4];
  uint32_t TXIRCal_offset[4];
} sli_wifi_txircal_config_t;

typedef struct {
  uint32_t calibration_flags;
  // Bit 0 → TxDCOC offset present
  // Bit 1 → IR cal compensations present
  uint32_t TXDCOC[4];  //offset 1;
  uint32_t TXIRCal[4]; //offset 1;
} sli_wifi_get_txircal_config_t;
typedef struct {
  uint32_t calibration_flags;
  // Bit 0 → RxDCOC offset present
  // Bit 1 → Rx IR cal compensations present
  uint32_t RXDCOC[4];  //offset 1;
  uint32_t RXIRCal[4]; //offset 1;
} sli_wifi_get_RxIRCal_config_t;

typedef struct {
  sli_wifi_frame_body_type_t frame_body_type;
  sli_wifi_txircal_config_t TxIRCal_config;
...
typedef struct {
  ...
  uint32_t RXDCOC_offset[4];
  uint32_t RXIRCal_offset[4];
} sli_wifi_RxIRCal_config_t;

typedef struct {
  sli_wifi_frame_body_type_t frame_body_type;
  sli_wifi_RxIRCal_config_t RxIRCal_config;
...
sl_status_t sli_wifi_tx_ir_calib(sl_wifi_interface_t interface,
                                 sli_wifi_txircal_config_t tx_ir_conf,
                                 sli_wifi_get_txircal_config_t *tx_ir_calib);
sl_status_t sli_wifi_get_tx_ir_dcoc_calibration(sl_wifi_interface_t interface,
                                                sli_wifi_get_txircal_config_t *tx_ir_dcoc_calib);
sl_status_t sli_wifi_rx_ir_calib(sl_wifi_interface_t interface, sli_wifi_RxIRCal_config_t rx_ir_calib);
sl_status_t sli_wifi_get_rx_ircal_data(sl_wifi_interface_t interface, sli_wifi_get_RxIRCal_config_t *get_rx_ircal);
