...
typedef struct {
  ...
  sl_wifi_callback_function_v2_t function_v2; /// User provided callback function pointer
...
{
  sl_wifi_callback_entry_t *entry = get_callback_entry(group);
  if (entry != NULL && entry->function_v2 == NULL) {
    entry->function_v2 = function;
    entry->arg         = optional_arg;
    return SL_STATUS_OK;
  }
  return SL_STATUS_FAIL;
...
sl_status_t sl_wifi_default_event_handler(sl_wifi_event_t event, sl_wifi_buffer_t *buffer)
{
  ...
  if (event == SL_WIFI_COMMAND_ENGINE_STATUS_EVENT) {
    // If the event is a command engine status event, we need to handle it differently
    sl_status_t status = *(sl_status_t *)buffer;
    if (entry->function_v2) {
      return entry->function_v2(event, status, NULL, 0, entry->arg);
    }
    ...
    *status_pointer = status;
    status          = entry->function(event, status_pointer, 0, entry->arg);
    free(status_pointer);
    return status;
  }
  ...
  if (event == SL_WIFI_TRANSCEIVER_TX_DATA_STATUS_CB) {
    ...
    if (entry->function_v2) {
      return entry->function_v2(event, SL_STATUS_OK, tx_cfm_cb_data, 0, entry->arg);
    }
  ...
  if (event == SL_WIFI_TRANSCEIVER_RX_DATA_RECEIVE_CB) {
    ...
    if (entry->function_v2) {
      return entry->function_v2(event, SL_STATUS_OK, rx_cb_data, 0, entry->arg);
    }
