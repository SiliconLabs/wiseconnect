                   ...
                   const ip_addr_t* remote_ip, u16_t local_port, u16_t remote_port)
{
  ...
  if (state == ESTABLISHED) {
    ...
    /* CLOSED state - not on any list */
    ip_addr_copy(pcb->local_ip, *local_ip);
    pcb->local_port = local_port;
    ip_addr_copy(pcb->remote_ip, *remote_ip);
    pcb->remote_port = remote_port;
  } else {
    /* All other active states (SYN_SENT, SYN_RCVD, FIN_WAIT_1, FIN_WAIT_2, 
     * CLOSE_WAIT, CLOSING, LAST_ACK) go on active list */
    TCP_REG(&tcp_active_pcbs, pcb);
    ip_addr_copy(pcb->local_ip, *local_ip);
    pcb->local_port = local_port;
    ip_addr_copy(pcb->remote_ip, *remote_ip);
...
/* ========================================================================
 * TCP Dynamic Timer Test Helper Functions
 * These functions provide access to internal TCP dynamic timer state for
 * comprehensive unit testing and verification.
 * ======================================================================== */

/* Wrapper functions - these call the accessor functions from tcp.c */

u32_t test_tcp_time_diff(u32_t current, u32_t previous)
{
  return tcp_get_time_diff(current, previous);
}

u32_t test_tcp_tw_remaining_time_ms(struct tcp_pcb *pcb)
{
  return tcp_get_tw_remaining_time_ms(pcb);
}

u8_t test_tcp_pcb_needs_active_mode(struct tcp_pcb *pcb)
{
  return tcp_get_pcb_needs_active_mode(pcb);
}

/* Getter functions for internal state verification */

u8_t test_tcp_get_tcp_timer(void)
{
  return tcp_get_timer();
}

u8_t test_tcp_get_tcp_timer_ctr(void)
{
  return tcp_get_timer_ctr();
}
...
{
  return tcp_dynamic_state.accumulated_fractional_ms;
}

u32_t test_tcp_get_last_timer_call(void)
{
  return tcp_dynamic_state.last_timer_call;
}

u8_t test_tcp_get_first_call(void)
{
  return tcp_dynamic_state.first_call;
}

u8_t test_tcp_get_force_active_mode(void)
{
  return tcp_dynamic_state.force_active_mode;
}

u8_t test_tcp_get_current_mode(void)
{
  return tcp_dynamic_state.current_mode;
}

/* Setter functions for internal state testing */
