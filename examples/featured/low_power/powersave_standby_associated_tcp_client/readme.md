...
This application demonstrates SiWx91x to enable TCP data transfer in low power mode, with NWP is in associated power save mode with retention and keeping M4 in sleep with retention after data transfer. NWP wakes up periodically based on the DTIM or listen interval configured by the user.

The application also demonstrates SiWx91x to connect to a remote server to send TCP data. Also enables the analysis of various performance profiles using a power analyzer during power save with data transfer via TCP.
...
> - The measured current is for `DTIM = 10` and with `TCP Keep Alive at 240 secs`. Please refer Datasheet for current consumption value for Standby associated for TCP protocol.  
