...
static void ncp_callback(uint32_t event)
{
  switch (event) {
    ...
    case SL_USART_EVENT_RECEIVE_COMPLETE:
      rx_complete = 1;
      if (rx - rx_buf < SL_SIMPLE_COM_RX_BUF_SIZE) {
        *rx++ = rx_byte;
      }
...
void sl_simple_com_step(void)
{
  ...
  if (rx - rx_buf == SL_SIMPLE_COM_RX_BUF_SIZE) { // buffer full
    reset_rx();
  }
...
SL_WEAK void sl_simple_com_receive_cb(sl_status_t status, uint32_t len, uint8_t *data)
{
  (void)status;
  (void)data;
  (void)len;
