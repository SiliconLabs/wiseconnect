...
#define MSG_GET_LEN(x) ((uint16_t)(SL_BGAPI_MSG_LEN((x)->header) + SL_BGAPI_MSG_HEADER_LEN))
...
void sl_ncp_step(void)
{
  ...
  if (cmd_is_available() && !busy) {
    ...
    if ((result & SL_NCP_SEC_EVT_PROCESS) == SL_NCP_SEC_EVT_PROCESS) {
      ...
      cmd_dequeue();
    } else if ((result & SL_NCP_SEC_RSP_PROCESS) == SL_NCP_SEC_RSP_PROCESS) {
      response = sl_ncp_sec_process_response(sl_wifi_get_command_response(), cmd_is_encrypted);
#else
    {
      response = sl_wifi_get_command_response();
#endif // SL_CATALOG_NCP_SEC_PRESENT
      // Clear command buffer
      cmd_dequeue();
      busy = true;
      ...
      sl_simple_com_transmit((uint32_t)(MSG_GET_LEN(response)), (uint8_t *)response);
    }
  }
  ...
  if (evt_is_available() && !busy) {
    ...
    data_ptr = (uint8_t *)sl_ncp_sec_process_event((app_wifi_msg_t *)evt.buf);
 ...
 *****************************************************************************/
SL_WEAK bool sl_ncp_local_evt_process(app_wifi_msg_t *evt)
{
  (void)evt;
  return true;
}
 ...
 * @return True if message fits into buffer, otherwise false.
 *****************************************************************************/
bool sl_wifi_can_process_event(uint32_t len)
{
...
static void cmd_enqueue(uint16_t len, uint8_t *data)
{
  ...
  if (state == CMD_RECEIVED) {
    ...
    // To Do: Error check
    app_timer_start(&cmd_timer, SL_NCP_CMD_TIMEOUT_MS, cmd_timer_cb, NULL, false);
  }
}
