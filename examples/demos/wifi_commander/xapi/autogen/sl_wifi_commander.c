...
static struct bgapi_device bgapi_wifi_device = {
  .dev_type = sl_bgapi_dev_type_wifi,
};
...
static const struct sli_bgapi_class *const app_wifi_class_table[] = {
  APP_WIFI_BGAPI_CLASS(system),      APP_WIFI_BGAPI_CLASS(net_intf),
  APP_WIFI_BGAPI_CLASS(net_profile), APP_WIFI_BGAPI_CLASS(ap),
  APP_WIFI_BGAPI_CLASS(net_cred),    APP_WIFI_BGAPI_CLASS(common),
  APP_WIFI_BGAPI_CLASS(scan),        APP_WIFI_BGAPI_CLASS(ping),
  APP_WIFI_BGAPI_CLASS(client),      NULL
};

/********************************************************************
 *  APIs to handle BGAPI event to host                              *
 *********************************************************************/
sl_status_t app_wifi_init_classes(const struct sli_bgapi_class *const *classes)
{
  bgapi_wifi_device.lib_table = (const struct bgapi_lib *const *)classes;
  return bgapi_init_device(&bgapi_wifi_device);
}
...
{
  const uint32_t msg_size = SL_BGAPI_MSG_HEADER_LEN + SL_BGAPI_MSG_LEN(event->header);
...
{
  uint8_t dev_type                   = SL_BGAPI_MSG_DEVICE_TYPE(event->header);
  uint8_t class_id                   = SLI_BGAPI_MSG_CLASS_ID(event->header);
  uint8_t command_id                 = SLI_BGAPI_MSG_COMMAND_ID(event->header);
  uint32_t msg_data_fix_size         = SL_BGAPI_MSG_LEN(event->header);
  uint32_t msg_fix_size              = SL_BGAPI_MSG_HEADER_LEN + msg_data_fix_size;
  ...
  event->header = SLI_BGAPI_MSG_HEADER(class_id,
                                       command_id,
                                       (uint8_t)sl_bgapi_msg_type_evt | (uint8_t)dev_type,
...
/********************************************************************
 *  APIs to handle BGAPI command (Used by NCP Component)                           *
 *********************************************************************/
void app_wifi_handle_command(uint32_t header, void *data)
{
  sl_bgapi_handle_command(header, data);
}