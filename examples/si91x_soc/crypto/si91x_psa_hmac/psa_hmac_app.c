...
void test_psa_hmac(psa_algorithm_t hash_alg)
{
  ...
  uint8_t mac_buf[PSA_MAC_MAX_SIZE] = { 0 };
  size_t mac_len;
  ...
  ret = psa_crypto_init();
  if (ret != PSA_SUCCESS) {
    printf("PSA Crypto Init Failed with error : %d\n", ret);
  } else {
  ...
  if (ret != PSA_SUCCESS) {
    printf("PSA Import key failed with error : %d\n", ret);
  } else {
  ...
  ret = psa_mac_compute(key_id, PSA_ALG_HMAC(hash_alg), hmac_msg, sizeof(hmac_msg), mac_buf, sizeof(mac_buf), &mac_len);
  if (ret != PSA_SUCCESS || (memcmp(mac_buf, expected_mac, sizeof(expected_mac)))) {
    printf("MAC Compute failed with error : %d\n", ret);
  } else {
  ...
  if (ret != PSA_SUCCESS) {
    printf("MAC Verify Failed with error : %d\n", ret);
  } else {
