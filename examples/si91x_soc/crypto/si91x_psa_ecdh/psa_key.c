                               ...
                               psa_algorithm_t algo)
{
  ...
  } else if (algo == PSA_ALG_ECB_NO_PADDING || algo == PSA_ALG_CBC_NO_PADDING || algo == PSA_ALG_CFB
             || algo == PSA_ALG_CTR || algo == PSA_ALG_CCM || algo == PSA_ALG_GCM) {
    // AES cipher or AEAD
  ...
  if (storage_type < IMPORT_KEY_OFFSET) {
    ...
    print_error_cycle(psa_import_key(&key_attr, key_buf, key_len, &key_id));
  }
}
 ...
 * Copy a key.
 ******************************************************************************/
psa_status_t copy_key(psa_key_id_t id, psa_key_usage_t usage, psa_algorithm_t algo)
{
  ...
  if (PSA_ALG_IS_RANDOMIZED_ECDSA(algo) || algo == PSA_ALG_PURE_EDDSA) {
    ...
    usage |= PSA_KEY_USAGE_DERIVE;
             ...
             || algo == PSA_ALG_CTR || algo == PSA_ALG_CCM || algo == PSA_ALG_GCM || algo == PSA_ALG_STREAM_CIPHER
  ...
  print_error_cycle(psa_copy_key(key_id, &key_attr, &key_id));
}
                             ...
                             psa_algorithm_t algo)
{
  ...
  if (algo == PSA_ALG_ECB_NO_PADDING || algo == PSA_ALG_CBC_NO_PADDING || algo == PSA_ALG_CFB || algo == PSA_ALG_CTR
      || algo == PSA_ALG_CCM || algo == PSA_ALG_GCM) {
    // AES cipher or AEAD
    type = PSA_KEY_TYPE_AES;
    usage |= PSA_KEY_USAGE_ENCRYPT | PSA_KEY_USAGE_DECRYPT;
  } else if (algo == PSA_ALG_STREAM_CIPHER || algo == PSA_ALG_CHACHA20_POLY1305) {
...
{
  // Derive a key
  print_error_cycle(psa_key_derivation_output_key(&key_attr, operation, &key_id));
}
...
{
  print_error_cycle(psa_export_key(key_id, key_buf, sizeof(key_buf), &key_len));
}
...
{
  print_error_cycle(psa_export_public_key(key_id, key_buf, sizeof(key_buf), &key_len));
}
