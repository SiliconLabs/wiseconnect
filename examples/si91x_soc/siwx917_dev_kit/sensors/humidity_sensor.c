...
void humidity_sensor_init(void)
{
  ...
  if (sl_si91x_gpio_driver_get_uulp_npss_pin(SENSOR_ENABLE_GPIO_PIN) != 1) {
    ...
    if (status != SL_STATUS_OK) {
      printf("sl_si91x_gpio_driver_set_uulp_npss_direction, Error code: %u\r\n", (unsigned int)status);
      return;
    }
    // Set UULP GPIO pin
    status = sl_si91x_gpio_driver_set_uulp_npss_pin_value(SENSOR_ENABLE_GPIO_PIN, SET);
    if (status != SL_STATUS_OK) {
      printf("sl_si91x_gpio_driver_set_uulp_npss_pin_value, Error code: %u\r\n", (unsigned int)status);
      return;
    }
  }
#else
  ...
  if (status != SL_STATUS_OK) {
    printf("sl_gpio_driver_get_pin, Error code: %u\r\n", (unsigned int)status);
    return;
  }
  if (pin_value != 1) {
    // Enable GPIO CLK
    ...
    if (status != SL_STATUS_OK) {
      printf("sl_gpio_driver_set_pin_mode, Error code: %u\r\n", (unsigned int)status);
      return;
    }
    ...
    if (status != SL_STATUS_OK) {
      printf("sl_si91x_gpio_driver_set_pin_direction, Error code: %u\r\n", (unsigned int)status);
      return;
    }
    // Set GPIO pin
    status = sl_gpio_driver_set_pin(&sensor_enable_port_pin); // Set ULP GPIO pin
    if (status != SL_STATUS_OK) {
      printf("sl_gpio_driver_set_pin, Error code: %u\r\n", (unsigned int)status);
      return;
    }
  }
#endif
  ...
  //      printf("sl_i2c_driver_init : Invalid Parameters, Error Code: 0x%X \r\n", (unsigned int)status);
  //      return;
  //    } else {
  ...
  //    if (status != SL_I2C_SUCCESS) {
  //      printf("sl_i2c_driver_configure_fifo_threshold : Invalid Parameters, Error Code: 0x%X \r\n", (unsigned int)status);
  //      return;
  //    } else {
  ...
  if (status != SL_STATUS_OK) {
    printf("Sensor reset un-successful, Error Code: 0x%X \r\n", (unsigned int)status);
    return;
  }
  osDelay(10);
  ...
  if (status != SL_STATUS_OK) {
    printf("Sensor initialization un-successful, Error Code: 0x%X \r\n", (unsigned int)status);
    return;
  }
  osDelay(10);
  ...
  if (status != SL_STATUS_OK) {
    printf("Sensor firmware version un-successful, Error Code: 0x%X \r\n", (unsigned int)status);
    return;
  }
  osDelay(10);
  ...
  if (status != SL_STATUS_OK) {
    printf("Sensor user register 1 write data failed, Error Code: 0x%X \r\n", (unsigned int)status);
    return;
  }
  // Reads register data from sensor
  status = sl_si91x_si70xx_read_control_register(I2C, SI70XX_SLAVE_ADDR, SL_RH_T_USER_REG, &value);
  if (status != SL_STATUS_OK) {
    printf("Sensor user register 1 read failed, Error Code: 0x%X \r\n", (unsigned int)status);
    return;
  }
  osDelay(10);
  ...
  if (status != SL_STATUS_OK) {
    printf("Sensor temperature read failed, Error Code: 0x%X \r\n", (unsigned int)status);
    return;
  }
  // measure humidity data from sensor
  status = sl_si91x_si70xx_measure_humidity(I2C, SI70XX_SLAVE_ADDR, &humidity);
  if (status != SL_STATUS_OK) {
    printf("Sensor humidity read failed, Error Code: 0x%X \r\n", (unsigned int)status);
    return;
  }
  osDelay(10);
