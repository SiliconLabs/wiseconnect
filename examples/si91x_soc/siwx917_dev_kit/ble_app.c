...
// Local device name
#define RSI_BLE_APP_DEVICE_NAME "WIFI_SENSOR"
...
static sl_wifi_scan_result_t *scanresult = NULL;
 ...
 * @brief      this function is used to add characteristic value attribute with security support.
...
adv:
      ...
      case RSI_BLE_WLAN_JOIN_STATUS: //Send the connected status to mobile ble application WYZBEE CONFIGURATOR
      {
        ...
        ip.type            = client_profile.ip.type;
        ip.ip.v4.value     = client_profile.ip.ip.v4.ip_address.value;
      ...
      case RSI_BLE_GATT_CLIENT_WRITE_EVENT: {
        //! GATT write event - process command and send response from task context
