 ...
 * intentionally emitted via DEBUGOUT so that they remain visible
 ...
 * (or SL_PRINT_STRING_DEBUG for verbose trace). */
  ...
  if (i2c_status != SL_I2C_SUCCESS) {
    DEBUGOUT("sl_i2c_driver_configure_fifo_threshold : Invalid "
    ...
    switch (current_mode) {
      case RECEIVE_DATA:
        if (receive_data_flag) {
          ...
          if (i2c_status != SL_I2C_SUCCESS) {
            DEBUGOUT("sl_i2c_driver_receive_data_blocking : Invalid "
                     "Parameters, Error Code : %u \n",
                     i2c_status);
          ...
          DEBUGOUT("I2C_Task: Data read from Leader successfully and "
      ...
      case SEND_DATA:
        if (send_data_flag) {
          ...
          DEBUGOUT("I2C_Task: Data read from USART_Message_Queue "
          ...
          if (i2c_status != SL_I2C_SUCCESS) {
            DEBUGOUT("sl_i2c_driver_send_data_blocking : Invalid "
                     "Parameters, Error Code : %u \n",
                     i2c_status);
            if (i2c_status != SL_I2C_TIMEOUT) {
...
static void compare_data(void)
{
  ...
  if (data_index == BUFFER_SIZE) {
    DEBUGOUT("Leader-follower read-write data comparison is "
             "successful, Test Case Passed \n");
  } else {
    DEBUGOUT("Leader-follower read-write data comparison is not "
