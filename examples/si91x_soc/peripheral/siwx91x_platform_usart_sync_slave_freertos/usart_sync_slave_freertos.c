/***************************************************************************/ /**
 * @file usart_sync_slave_freertos.c
 * @brief USART Synchronous Slave FreeRTOS example
 *******************************************************************************
 * # License
...
#include "usart_sync_slave_freertos.h"
#include "cmsis_os2.h"
#include "rsi_debug.h"
...
/* FreeRTOS task attributes for the USART sync slave task */
static const osThreadAttr_t usart_sync_thread_attributes = {
  .name       = "usart_sync_slave_task",
  .stack_size = 2048,
 ...
 ******************************************************************************/
/*******************************************************************************
 * @brief  Entry point: creates the USART sync slave FreeRTOS task.
 * @return None
 ******************************************************************************/
 ...
 * (or SL_PRINT_STRING_DEBUG for verbose trace). */
    ...
    SL_PRINT_STRING_ERROR("Failed to create USART sync slave task\r\n");
  }
}

/*******************************************************************************
 * @brief  Initializes the USART peripheral in synchronous slave mode.
...
static sl_status_t usart_sync_init_function(void)
{
  ...
  SL_PRINT_STRING_ERROR("USART SLAVE initialization is successful\r\n");
...
static void usart_sync_task(void *argument)
{
  ...
  /* Initialize USART peripheral in sync slave mode */
  status = usart_sync_init_function();
  if (status != SL_STATUS_OK) {
    SL_PRINT_STRING_ERROR("USART sync slave init failed\r\n");
    osThreadExit();
  }
  ...
  /* Fill TX buffer with slave's test pattern (different from master's pattern) */
  for (uint16_t i = 0; i < USART_BUFFER_SIZE; i++) {
    usart_data_out[i] = (uint8_t)(USART_BUFFER_SIZE - i);
  }
...
/*******************************************************************************
 * @brief  Validates RX buffer against the master's expected TX pattern.
 *         Master sends (i + 1), which differs from slave's
 *         (USART_BUFFER_SIZE - i) pattern — this prevents false positives.
 * @return sl_status_t SL_STATUS_OK if all bytes match, SL_STATUS_FAIL otherwise.
 ******************************************************************************/
static sl_status_t usart_app_compare_data(void)
{
  for (uint16_t i = 0; i < USART_BUFFER_SIZE; i++) {
    if (usart_data_in[i] != (uint8_t)(i + 1)) {
