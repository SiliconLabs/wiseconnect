/***************************************************************************/
/**
 * @file ulp_i2c_leader_example_non_blocking.c
 * @brief ULP_I2C Leader Non Blocking example functions
 *******************************************************************************
 * # License
 ...
 *
 ******************************************************************************/
#include "ulp_i2c_leader_example_non_blocking.h"
#include "rsi_debug.h"
...
#define I2C_ULP_BANK_OFFSET 0x800                                             // ULP Memory bank offset value.
#define I2C_TX_BUF_MEMORY   (ULP_SRAM_START_ADDR + (1 * I2C_ULP_BANK_OFFSET)) // TX Buffer memory address
#define I2C_RX_BUF_MEMORY   (ULP_SRAM_START_ADDR + (2 * I2C_ULP_BANK_OFFSET)) // RX Buffer memory address
/*******************************************************************************
 ******************************  Data Types  ***********************************
 ******************************************************************************/
  ...
  switch (current_mode) {
    case SL_ULP_I2C_SEND_DATA:
      if (i2c_send_data_flag) {
        ...
        if (current_power_state == SL_SI91X_POWER_MANAGER_PS2) {
          // In ULP Mode, we need to utilize ULP memory for transfers.
          i2c_status = sl_i2c_driver_send_data_non_blocking(i2c_instance,
                                                            FOLLOWER_I2C_ADDR,
                                                            (uint32_t *)I2C_TX_BUF_MEMORY,
                                                            I2C_BUFFER_SIZE,
                                                            &p_dma_config);
        } else {
          i2c_status = sl_i2c_driver_send_data_non_blocking(i2c_instance,
      ...
      if (i2c_driver_dma_error) {
        ...
        DEBUGOUT("Data is not transferred to Follower\n");
        i2c_receive_data_flag = true;
        current_mode          = SL_ULP_I2C_RECEIVE_DATA;
      }
      break;
    case SL_ULP_I2C_RECEIVE_DATA:
      if (i2c_receive_data_flag) {
        ...
        delay(1);
        if (current_power_state == SL_SI91X_POWER_MANAGER_PS2) {
          // The non-blocking receive API isn't functioning as expected in PS2, or ULP Mode, so we are using the blocking API instead.
          i2c_status = sl_i2c_driver_receive_data_blocking(i2c_instance,
                                                           FOLLOWER_I2C_ADDR,
                                                           (uint8_t *)I2C_RX_BUF_MEMORY,
                                                           I2C_BUFFER_SIZE);
        } else {
    ...
    garbage data or no data in console output. */
        ...
        memcpy((uint32_t *)I2C_TX_BUF_MEMORY, i2c_dma_write_buffer, sizeof(i2c_dma_write_buffer));
      }
...
static void compare_data(void)
{
  ...
  if (current_power_state == SL_SI91X_POWER_MANAGER_PS2) {
    // The received data is stored in ULP memory, so it is copied into the read buffer.
    memcpy(i2c_read_buffer, (uint8_t *)I2C_RX_BUF_MEMORY, sizeof(i2c_read_buffer));
  }
