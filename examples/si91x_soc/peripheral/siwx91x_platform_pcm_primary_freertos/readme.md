...
- [SiWx91x Platform PCM Primary FreeRTOS](#siwx91x-platform-pcm-primary-freertos)
...
This application demonstrates the Pulse Code Modulation (PCM) primary device data transfer using the I2S interface in a FreeRTOS environment.
...
- **`app_init()`** calls **`pcm_primary_example_init()`**, which only creates the PCM thread. **`app_process_action()`** is empty.
- Initialize the PCM peripheral (via I2S) and store the driver handle in `pcm_handle` using [sl_si91x_pcm_init()](https://docs.silabs.com/wiseconnect/latest/wiseconnect-api-reference-guide-si91x-peripherals/pcm#sl-si91x-pcm-init).
- Create **`osEventFlags`** for TX/RX complete; the PCM callback calls **`osEventFlagsSet`**; the task **`osEventFlagsWait`s** (no polling in `app_process_action`).
...
- After GPIO synchronization with the secondary board, the FreeRTOS task performs **one** full cycle: receive (wait for completion), compare with the expected pattern, transmit (wait for completion), then **`osThreadExit()`** (single-shot run). To test again, reset both boards and repeat the flow below.
...
- The application waits until data transmission is complete, then the PCM task ends.
...
2. Open **siwx91x_platform_pcm_primary_freertos.slcp** project file, select **Software component** tab and search for **PCM** in search bar.
...
Configure the following macros in `pcm_primary_freertos.c` and update or modify them if required.
...
- Change the `PCM_INSTANCE` macro value to `ULP_PCM` in `pcm_primary_freertos.c`:
