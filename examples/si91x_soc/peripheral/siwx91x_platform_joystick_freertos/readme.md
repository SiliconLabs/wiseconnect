...
- [SiWx91x Platform Joystick FreeRTOS](#siwx91x-platform-joystick-freertos)
...
This application demonstrates the **joystick** peripheral on SiWx91x under **FreeRTOS**. A dedicated CMSIS-RTOS2 task initializes the **sl_si91x_joystick** driver, wakes on a periodic timer-driven semaphore, samples position, and prints discrete directions on the serial console.

## Overview

- The joystick stack is UC-driven: install channel instances and keep **ADC Peripheral Common Configuration** channel counts consistent with those instances.
- A CMSIS-RTOS2 **periodic software timer** (**`JOYSTICK_SAMPLE_PERIOD_MS`**) releases **`JOYSTICK_SEM_MAX`** tokens on a **counting semaphore** so brief task stalls do not permanently lose ticks.
- Enable serial console / **iostream** if you rely on position prints on VCOM.

## FreeRTOS Architecture

- On startup, `main.c` calls `sl_main_second_stage_init()` to initialize SDK components, then calls `app_init()`, which invokes [`joystick_example_init()`]. That creates the `joystick` CMSIS-RTOS2 thread with `osThreadNew()` (**`2048`**-byte stack, **`osPriorityLow1`**).
- [`app_process_action()`] is a no-op; sampling and console prints run in the joystick task.
- Inside **`joystick_task()`**, [`joystick_init_function()`] creates a counting semaphore (**`JOYSTICK_SEM_MAX`**, default **`8`**) and a periodic **`osTimer`** (**`joy_tick`**). The timer callback **`joystick_tick_timer_callback()`** calls **`osSemaphoreRelease()`** on each tick so brief task stalls do not permanently lose samples. Initialization continues with **`sl_si91x_joystick_init()`**, **`sl_si91x_joystick_start(SL_JOYSTICK_ENABLED)`**, then **`osTimerStart()`** using **`JOYSTICK_SAMPLE_PERIOD_MS`** (converted via **`joystick_ms_to_kernel_ticks()`**). On failure the task logs and **`osThreadExit()`**.
- The task loop **`osSemaphoreAcquire(..., osWaitForever)`**, then **`sl_si91x_joystick_get_position(SL_JOYSTICK_ENABLED, &pos)`** and prints the discrete direction.
...
- Configure the following macro in [**joystick_freertos.c**] file and update/modify following macro, if required.

  - `JOYSTICK_SAMPLE_PERIOD_MS`: Interval in milliseconds between timer callbacks that **`osSemaphoreRelease`** the sample semaphore; default **400** ms.

    ```c
      #define JOYSTICK_SAMPLE_PERIOD_MS 400U
    ```

- Semaphore backlog cap **`JOYSTICK_SEM_MAX`** (**default `8`**) avoids losing **`osSemaphoreRelease`** calls if the joystick task briefly stalls.

- Open **siwx91x_platform_joystick_freertos.slcp**, select the **Software Components** tab, and search for **joystick**.
