  ...
  switch (current_mode) {
    case I2C_SEND_DATA:
      if (i2c_send_data_flag) {
        ...
        if (i2c_status != SL_I2C_SUCCESS) {
          SL_PRINT_STRING_ERROR("sl_i2c_driver_send_data_blocking : Invalid Parameters, Error Code : %u \n",
                                i2c_status);
    ...
    case I2C_RECEIVE_DATA:
      if (i2c_receive_data_flag) {
        ...
        if (i2c_status != SL_I2C_SUCCESS) {
          SL_PRINT_STRING_ERROR("sl_i2c_driver_receive_data_blocking : Invalid Parameters, Error Code : %u \n",
                                i2c_status);
          if (i2c_status != SL_I2C_TIMEOUT) {
...
static void compare_data(void)
{
  ...
  if (data_index == I2C_BUFFER_SIZE) {
    SL_PRINT_STRING_ERROR("Leader-Follower read-write Data comparison is successful, Test Case Passed \n");
  } else {
    SL_PRINT_STRING_ERROR("Leader-Follower read-write Data comparison is not successful, Test Case Failed \n");
  }
}
