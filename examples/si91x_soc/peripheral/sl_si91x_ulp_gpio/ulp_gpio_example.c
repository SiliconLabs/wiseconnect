...
void gpio_example_process_action(void)
{
  ...
  switch (ulp_gpio_current_state) {
    case SL_ULP_GPIO_PROCESS_ACTION:
      ...
      do {
        ...
        if (ULP_GPIO_PIN == SET) {
          ...
          if (status != SL_STATUS_OK) {
            SL_PRINT_STRING_ERROR("sl_gpio_driver_toggle_pin, Error code: %lu", status);
            break;
          }
        }
    ...
    garbage data or no data in console output. */
        ...
        if (status != SL_STATUS_OK) {
          SL_PRINT_STRING_ERROR("sl_gpio_driver_deinit, Error code: %lu", status);
          break;
        }
...
static void configuring_ps2_power_state(void)
{
  ...
  peri.ulpss_peripheral = SL_SI91X_POWER_MANAGER_ULPSS_PG_SSI | SL_SI91X_POWER_MANAGER_ULPSS_PG_I2S
                          ...
                          | SL_SI91X_POWER_MANAGER_ULPSS_PG_UDMA | SL_SI91X_POWER_MANAGER_ULPSS_PG_FIM
