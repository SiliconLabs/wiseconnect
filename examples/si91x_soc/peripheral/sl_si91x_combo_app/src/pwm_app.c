...
void pwm_init(void)
{
  ...
  do {
    ...
    if (pwm_thread_sem == NULL) {
      DEBUGOUT("!! failed to create pwm_semaphore\n"); // Semaphore object not created, handle failure
    } else {
      DEBUGOUT("pwm_semaphore created successfully\n");
    }
