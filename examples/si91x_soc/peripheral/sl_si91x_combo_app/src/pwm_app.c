 ...
 * pulse waveform, which is useful in motor control and power control
 applications.
 * @section Description :
 * It generates four different duty cycles on different channels using
 programmable
...
osSemaphoreId_t pwm_thread_sem;
#endif
/*******************************************************************************
 ...
 * (or SL_PRINT_STRING_DEBUG for verbose trace). */
  ...
  do {
    ...
    if (status != SL_STATUS_OK) {
      SL_PRINT_STRING_ERROR("sl_si91x_pwm_control_dead_time, Error code: %lu", status);
      break;
    }
    SL_PRINT_STRING_ERROR("PWM Control dead time insertion is successful \n");
    ...
    pwm_thread_sem = osSemaphoreNew(1, 0, NULL);
    if (pwm_thread_sem == NULL) {
      SL_PRINT_STRING_ERROR("!! failed to create pwm_semaphore\n"); // Semaphore object not
    } else {
      SL_PRINT_STRING_ERROR("pwm_semaphore created successfully\n");
    }
...
void pwm_app(void)
{
  ...
  SL_PRINT_STRING_ERROR("PWM is running, press BTN0 to stop\n");

  while (1) {
#if defined(SL_CATALOG_KERNEL_PRESENT)
    val = osSemaphoreAcquire(pwm_thread_sem, 0); // returns instantly
    if (val == osOK) {
      SL_PRINT_STRING_ERROR("PWM task: acquired semaphore\n");
      sl_si91x_pwm_stop(sl_pwm_channel_0_config.channel);
      SL_PRINT_STRING_ERROR("PWM stopped, press BTN0 to restart\n");
      val = osSemaphoreAcquire(pwm_thread_sem, osWaitForever);
      if (val == osOK) {
        SL_PRINT_STRING_ERROR("PWM task: acquired semaphore\n");
        sl_si91x_pwm_start(sl_pwm_channel_0_config.channel);
        SL_PRINT_STRING_ERROR("PWM restarted, press BTN0 to stop\n");
      }
    }
    osDelay(10);
#endif
...
{
  switch (event) {
    case RISE_TIME_PERIOD_MATCH_CH0: // Event for 0th channel without considering
                                     // post scaler
      event_flag = 1;
      flag[0]    = 1;
      break;
    case TIME_PERIOD_MATCH_CH0: // Event for 0th channel which considering post
                                // scaler
      event_flag = 1;
      flag[1]    = 1;
      break;
    ...
    case FAULT_B: // Fault B pin Event
      ...
      flag[3]    = 1;
      break;
    case RISE_TIME_PERIOD_MATCH_CH1: // Event for 1st channel without considering
                                     // post scaler
      event_flag = 1;
      flag[4]    = 1;
      break;
    case TIME_PERIOD_MATCH_CH1: // Event for 1st channel which considering post
                                // scaler
      event_flag = 1;
      flag[5]    = 1;
      break;
    case RISE_TIME_PERIOD_MATCH_CH2: // Event for 2nd channel without considering
                                     // post scaler
      event_flag = 1;
      flag[6]    = 1;
      break;
    case TIME_PERIOD_MATCH_CH2: // Event for 2nd channel which considering post
                                // scaler
      event_flag = 1;
      flag[7]    = 1;
      break;
    case RISE_TIME_PERIOD_MATCH_CH3: // Event for 3rd channel without considering
                                     // post scaler
      event_flag = 1;
      flag[8]    = 1;
      break;
    case TIME_PERIOD_MATCH_CH3: // Event for 3rd channel which considering post
                                // scaler
      event_flag = 1;
      flag[9]    = 1;
      break;
