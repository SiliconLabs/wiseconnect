...
- [SiWx91x Platform ADC Static Mode FreeRTOS](#platform-siwx91x-adc-static-mode-freertos)
...
- Once sampling is done, the callback releases a semaphore to unblock the task, which then reads the sampled data using [sl_si91x_adc_read_data_static](https://docs.silabs.com/wiseconnect/latest/wiseconnect-api-reference-guide-si91x-peripherals/adc#sl-si91x-adc-read-data-static) API for static mode of ADC. This process will run continuously.
...
- Open **siwx91x_platform_adc_static_mode_freertos.slcp** project file, select **Software Component** tab, and search for **ADC** in search bar.
...
- Configure the following macros in [`adc_static_mode_freertos.c`](adc_static_mode_freertos.c) if required:

- `ADC_MAX_OP_VALUE`: Maximum raw 12-bit value that can be read from the ADC data register. By default, it is set to 4095.
...
>
> > **Note:** If the positive input to ADC is given as 2.4V and negative input is given as 1.5V, the ADC output will be a digital value which is equivalent to 0.9V.
>
> **Single ended Mode:**
>
> vout = (((float)ADC output/(float)4095) * Vref Voltage);
>
> > **Note:** If the positive input to ADC is given as 2.4V, ADC output will be a digital value which is equivalent to 2.4V.
>
> **Note:**
>
