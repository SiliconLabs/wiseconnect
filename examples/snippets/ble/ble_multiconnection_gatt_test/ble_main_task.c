...
#if RSI_DEBUG_EN
        SL_DEBUG_LOG_V2(INFO, "Device %s already connected!!!\r\n", (uintptr_t)(adv_dev_addr));
#endif
        break;
      }
    }
...
static void rsi_ble_on_connect_event(rsi_ble_event_conn_status_t *resp_conn)
{
  ...
  if (resp_conn->status != 0) {
    //SL_DEBUG_LOG_V2(INFO, "On connect event status report : %d", resp_conn->status);
...
void rsi_ble_on_enhance_conn_status_event(rsi_ble_event_enhance_conn_status_t *resp_enh_conn)
{
  ...
  if (resp_enh_conn->status != 0 && resp_enh_conn->status != 63) {
    SL_DEBUG_LOG_V2(INFO, "On enhanced connect event\r\n");
    if (remote_device_role == PERIPHERAL_ROLE) {
      peripheral_connection_in_prgs = 0;
    }
    rsi_ble_app_set_event(RSI_BLE_SCAN_RESTART_EVENT);
...
#endif
  if (resp_status != 0) {
    SL_DEBUG_LOG_V2(ERROR, "RSI_BLE_CONN_UPDATE_EVENT FAILED\r\n");
    return;
  } else {
    //! convert to ascii
...
static void rsi_ble_on_event_write_resp(uint16_t event_status, rsi_ble_set_att_resp_t *rsi_ble_event_set_att_rs)
{
  ...
  if (event_status != RSI_SUCCESS) {
    SL_DEBUG_LOG_V2(ERROR, "Write event response failed\r\n");
  } else {
    //! set conn specific event
 ...
 * @return     none.
 * @section description
 * This callback function is invoked when an SMP request event is received(we are in Cetral mode)
    ...
    switch (event_id) {
      case RSI_APP_EVENT_ADV_REPORT: {
        ...
        if (peripheral_task_instances < RSI_BLE_MAX_NBR_PERIPHERALS) {
          ...
          if ((peripheral_conn_id < TOTAL_CONNECTIONS) && (peripheral_con_req_pending == 1)) {
            ...
            ble_app_task_handle[peripheral_conn_id] =
              osThreadNew(rsi_ble_task_on_conn,
                          ...
                          &ble_peripheral_attr);
            if (ble_app_task_handle[peripheral_conn_id] == NULL) {
              SL_DEBUG_LOG_V2(ERROR, "task%d", peripheral_conn_id);
              SL_DEBUG_LOG_V2(ERROR, "failed to create, reason = %ld", status);
          ...
          SL_DEBUG_LOG_V2(INFO, "Maximum peripheral connections reached\r\n");
        }
      } break;
          ...
          if (central_task_instances < RSI_BLE_MAX_NBR_CENTRALS) {
            ...
            if (central_conn_id < TOTAL_CONNECTIONS) {
              ...
              if (ble_app_task_handle[central_conn_id] == NULL) {
                SL_DEBUG_LOG_V2(ERROR, "task%d failed to create\r\n", central_conn_id);
            ...
            SL_DEBUG_LOG_V2(INFO, "Max central connections reached\r\n");
          }
        } else {
          SL_DEBUG_LOG_V2(INFO, "Check why this state occurred?\r\n");
          while (1)
            ;
        }
      } break;
        ...
        if (remote_device_role == CENTRAL_ROLE) {
          if (central_task_instances < RSI_BLE_MAX_NBR_CENTRALS) {
            ...
            if (central_conn_id < TOTAL_CONNECTIONS) {
              ...
              rsi_parsed_conf.rsi_ble_config.rsi_ble_conn_config[RSI_BLE_MAX_NBR_PERIPHERALS + central_task_instances]
                .conn_id = central_conn_id;
              //SL_DEBUG_LOG_V2(INFO, "free bytes remaining before connection1 - %ld \r\n", xPortGetFreeHeapSize());
              //! create a thread for processing new central connection
              const osThreadAttr_t ble_central_attr = {
                .name       = "ble_central_task",
                .priority   = osPriorityNormal,
...
#if RSI_DEBUG_EN
        //! event invokes when an MTU event is received
      ...
      case RSI_BLE_SCAN_RESTART_EVENT: {
        ...
        if (status != RSI_SUCCESS) {
          SL_DEBUG_LOG_V2(INFO, "Scanning stop cmd status = %lx\r\n", status);
          //return status;	//! TODO
        } else {
          rsi_scan_in_progress = 0;
        }

        SL_DEBUG_LOG_V2(INFO, "Scanning restarted\r\n");

        status = rsi_ble_start_scanning();
        if (status != RSI_SUCCESS) {
          SL_DEBUG_LOG_V2(INFO, "Scanning start cmd status = %lx\r\n", status);
          //return status;	//! TODO
        } else {
