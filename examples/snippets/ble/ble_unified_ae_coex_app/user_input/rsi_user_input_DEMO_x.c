 ...
 * Variables
 ******************************************************************************/
osSemaphoreId_t ui_task_response_semaphore;
#ifdef M4_UART
...
uint8_t stats;
extern osEventFlagsId_t ui_task_handle;
#endif
/*******************************************************************************
...
void ARM_USART_SignalEvent(uint32_t event)
{
  ...
  switch (event) {
    ...
    case ARM_USART_EVENT_RECEIVE_COMPLETE:
      ...
      osEventFlagsSet(ui_task_handle, event);
      break;
...
void configure_m4_uart(void)
{
  ...
  if (stats != ARM_DRIVER_OK) {
    SL_DEBUG_LOG_V2(ERROR, "UART Initialization Failed, Error Code : %d\r\n", stats);
    return;
  } else {
    SL_DEBUG_LOG_V2(INFO, "UART Initialization Success\r\n");
  }

  // Power up the UART peripheral
  stats = USARTdrv->PowerControl(ARM_POWER_FULL);
  if (stats != ARM_DRIVER_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Failed to Set Power to UART, Error Code : %d\r\n", stats);
    return;
  } else {
    SL_DEBUG_LOG_V2(INFO, "Configured Power to UART \r\n");
  }
  // Enable Receiver and Transmitter lines
  stats = USARTdrv->Control(ARM_USART_CONTROL_TX, 1);
  if (stats != ARM_DRIVER_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Failed to Set  Transmitter lines to UART, Error Code : %d\r\n", stats);
    return;
  } else {
    SL_DEBUG_LOG_V2(INFO, "Set  Transmitter lines to UART is success \r\n");
  }

  stats = USARTdrv->Control(ARM_USART_CONTROL_RX, 1);
  if (stats != ARM_DRIVER_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Failed to Set  Receiver lines to UART, Error Code : %d \r\n", stats);
    return;
  } else {
    SL_DEBUG_LOG_V2(INFO, "Set  Receiver lines to UART\r\n");
  }
  ...
  stats = USARTdrv->Control(ARM_USART_MODE_ASYNCHRONOUS | ARM_USART_DATA_BITS_8 | ARM_USART_PARITY_NONE
                            ...
                            BAUD_VALUE);
  if (stats != ARM_DRIVER_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Failed to set  UART control Configuration , Error Code : %d \r\n", stats);
    return;
  } else {
    SL_DEBUG_LOG_V2(INFO, "Successfully Configured UART control  \r\n");
  }
}
#endif
/*==============================================*/
...
void rsi_ui_app_task(void)
{
  ...
  while (1) {
    ...
    if (stats != ARM_DRIVER_OK) {
      SL_DEBUG_LOG_V2(ERROR, "Failed to Receive data , Error Code : %d \r\n", stats);
      return;
    } else {
      //SL_DEBUG_LOG_V2(DEBUG, "Receives data success  ");
    }
#endif
    //! Wait for uart interrupt reception
#ifdef M4_UART
    osEventFlagsWait(ui_task_handle, ulInterruptStatus, (osFlagsWaitAny | osFlagsNoClear), osWaitForever);
#if 1
