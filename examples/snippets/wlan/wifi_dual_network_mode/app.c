...
static const sl_wifi_device_configuration_t client_configuration = {
  ...
  .boot_config = { .oper_mode = SL_SI91X_CLIENT_MODE,
                   ...
                   .ext_tcp_ip_feature_bit_map =
                     (SL_SI91X_EXT_TCP_IP_DUAL_MODE_ENABLE | SL_SI91X_CONFIG_FEAT_EXTENSION_VALID),
...
static void send_ping_raw_data_frame(void)
{
  ...
  while (1) {
    SL_DEBUG_LOG_V2(INFO, "Send raw data frame (ping packet) \r\n");
...
sl_status_t sl_si91x_host_process_data_frame(sl_wifi_interface_t interface, sl_wifi_buffer_t *buffer)
{
  ...
  SL_DEBUG_LOG_V2(INFO, "Received raw data frame, frame length : %d\r\n", rx_pkt->length);
...
static void application_start(void *argument)
{
  ...
  SL_DEBUG_LOG_V2(INFO, "Bringing up Wi-Fi client connection...\r\n");
  ...
  remote_ip_address.type = SL_IPV6;
  SL_DEBUG_LOG_V2(INFO, "Enabled IPv6 configuraion\r\n");
#else
  sl_ip_address_t remote_ip_address = { 0 };
  ...
  while (1) {
    ...
    SL_DEBUG_LOG_V2(INFO, "Send ping request using offload stack\r\n");
