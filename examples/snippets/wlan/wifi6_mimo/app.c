...
void receive_data_from_tcp_client()
{
  ...
  if ((sl_si91x_bind(server_socket, (struct sockaddr *)&server_address, sizeof(server_address))) < 0) {
    printf("\r\nSocket bind failed with bsd error: %d\r\n", errno);
    return;
  }

  socket_return_value = sl_si91x_listen(server_socket, BACKLOG_COUNT);
  if (socket_return_value < 0) {
...
void receive_data_from_udp_socket()
{
...
#if SOCKET_ASYNC_FEATURE
  module_socket = sl_si91x_socket_async(AF_INET, SOCK_DGRAM, IPPROTO_UDP, &data_callback);
  if (module_socket < 0) {
    printf("\r\nSocket creation failed with bsd error: %d\r\n", errno);
    return;
  }
  ...
  if ((sl_si91x_bind(module_socket, (struct sockaddr *)&socket_address, sizeof(socket_address))) < 0) {
    printf("\r\nSocket bind failed with bsd error: %d\r\n", errno);
    return;
  }
  printf("\r\nListening on Local Port %d\r\n", SERVER_PORT);
