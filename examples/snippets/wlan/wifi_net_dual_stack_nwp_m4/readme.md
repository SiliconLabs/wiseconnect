...
This application demonstrates how to configure the SiWx91x module in dual network stack mode using the **SL_SI91X_EXT_TCP_IP_DUAL_MODE_ENABLE** feature. It  showcases simultaneous TCP/UDP data transfer from both the NWP offload stack and the M4 host-side LwIP stack.
...
- The application can be configured to suit your requirements and development environment. Review the following sections and make changes as needed.

- In the Project Explorer pane, expand the **config/** folder and open the ``sl_net_default_values.h`` file. Configure the following parameters to enable your Silicon Labs device to connect to your Wi-Fi network.
...
### Dual Stack Configuration in app.c

Configure the following parameters in `app.c` to test dual stack data transfer application.

 The application can be configured to run TCP/UDP Tx or RX traffic from both NWP and LwIP stacks simultanuously. By default, traffic runs only from offload stack. Set `HOST_DATA_TRANSFER` to 1 to enable host data transfer.
...
To run UDP Rx traffic, configure the SiWx91x module as a UDP server and start UDP clients on the remote PC using two command prompts.  
...
To run TCP Tx traffic, configure the SiWx91x module as a TCP client and start TCP servers on the remote PC using two command prompts. Open the [iPerf Application](https://sourceforge.net/projects/iperf2/files/iperf-2.0.8-win.zip/download) and run the following commands from the installed folder's path in two separate command prompts to start TCP servers on different ports:
...
To run TCP Rx traffic, configure the SiWx91x module as a TCP server and start TCP clients on the remote PC using two command prompts. Open two command prompts in the iPerf installed folder and run the following commands to start TCP clients on different ports.
...
This sends TCP traffic to the SiWx91x module on both ports for simultaneous receive testing.
...
- Triggering the DHCP process from external host is currently not supported.
