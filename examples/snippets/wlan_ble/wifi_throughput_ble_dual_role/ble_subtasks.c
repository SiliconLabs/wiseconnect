 ...
 *
 *  @section Description  This application connects as a Peripheral/Central with l2cap connection.
 */
...
void rsi_ble_task_on_conn(void *parameters)
{
  ...
  while (1) {
    ...
    if (disable_factor_count == DISABLE_ITER_COUNT) {
      SL_DEBUG_LOG_V2(INFO, "Reach the disable factor in ble sub task\r\n");
      ...
      if (l_conn_id != 0xff) {
        ...
        //while(l_conn_id_local >=0 )
        {
          if (rsi_ble_conn_info[l_conn_id_local].conn_status == 1) {
            ...
            if (status == 0) {
              SL_DEBUG_LOG_V2(INFO, "Disabled ble connection (%d) activity \r\n", l_conn_id_local);
    ...
    switch (event_id) {
      case RSI_APP_EVENT_ADV_REPORT: {
        ...
        if (status != RSI_SUCCESS) {
          SL_DEBUG_LOG_V2(ERROR, "Scan stopping failed with status : %ld - conn%d \r\n", status, l_conn_id);
        }
#if WLAN_TRANSIENT_CASE
        ble_scanning_is_there = 0;
#endif
        ...
        SL_DEBUG_LOG_V2(INFO,
                        ...
                        l_conn_id);
        if (status != RSI_SUCCESS) {
          SL_DEBUG_LOG_V2(ERROR, "Connecting failed with status : 0x%lx - conn%d", status, l_conn_id);
          //return status;
        ...
        if ((event_id == -1) || (!(event_id & (RSI_BLE_CONN_EVENT | RSI_BLE_ENHC_CONN_EVENT)))) {
          ...
          if (status != RSI_SUCCESS) {
            SL_DEBUG_LOG_V2(ERROR, " BLE connect cancel cmd status = %lx \r\n", status);
          } else {
      ...
      case RSI_BLE_CONN_EVENT: {
...
#endif
        osThreadSuspend(ble_app_task_handle[l_conn_id]);
#endif
        ...
        if (ble_conn_conf->add_to_acceptlist) {
          ...
          if (status != RSI_SUCCESS) {
            SL_DEBUG_LOG_V2(ERROR, "Failed to clear the accept list, error: 0x%lx - conn%d\r\n", status, l_conn_id);
            break;
          }
          ...
          SL_DEBUG_LOG_V2(INFO, "Device %s added to acceptlist - conn%d ", (uintptr_t)str_remote_address, l_conn_id);
        }
        ...
        if (rsi_ble_conn_info[l_conn_id].remote_device_role == CENTRAL_ROLE) {
          ...
          SL_DEBUG_LOG_V2(INFO, "Number of central devices connected:%d - conn%d", num_of_conn_centrals, l_conn_id);

          if (num_of_conn_centrals < RSI_BLE_MAX_NBR_CENTRALS) {
            //! advertise device with default interval
            status = rsi_ble_start_advertising();
            if (status != RSI_SUCCESS) {
              SL_DEBUG_LOG_V2(ERROR, "Advertising failed with status = 0x%lx - conn%d \r\n", status, l_conn_id);
              continue;
            }
#if WLAN_TRANSIENT_CASE
            ble_adv_is_there = 1;
          ...
          SL_DEBUG_LOG_V2(INFO, "Advertising device - conn%d\r\n", l_conn_id);
        } else {
          //! assuming that connection is from remote peripheral device
          num_of_conn_peripherals++;
          SL_DEBUG_LOG_V2(INFO, "Number of peripheral devices connected: %d \r\n", num_of_conn_peripherals);

          if (rsi_ble_conn_info[l_conn_id].remote_device_role == PERIPHERAL_ROLE) {
            SL_DEBUG_LOG_V2(INFO, "Start scanning - conn%d\r\n", l_conn_id);
            status = rsi_ble_start_scanning();
            if (status != RSI_SUCCESS) {
              SL_DEBUG_LOG_V2(ERROR, "Scan channel failed to open 0x%lx - conn%d\r\n", status, l_conn_id);
              continue;
            }
#if WLAN_TRANSIENT_CASE
      ...
      case RSI_BLE_ENHC_CONN_EVENT: {
        ...
        if (rsi_ble_conn_info[l_conn_id].remote_device_role == CENTRAL_ROLE) {
          ...
          if (num_of_conn_centrals < RSI_BLE_MAX_NBR_CENTRALS) {
            ...
            status = rsi_ble_stop_advertising();
            if (status != RSI_SUCCESS) {
              SL_DEBUG_LOG_V2(ERROR, "Advertising failed to stop, with status = 0x%lx - conn%d ", status, l_conn_id);
              //continue;
            }
#if WLAN_TRANSIENT_CASE
            ble_adv_is_there = 0;
            ...
            status = rsi_ble_stop_scanning();
            if (status != RSI_SUCCESS) {
              SL_DEBUG_LOG_V2(ERROR, "Scanning stop failed, cmd status = %lx - conn%d\r\n", status, l_conn_id);
            } else {
        ...
        if (mtu_exchange_done) {
          ...
          if (status != RSI_SUCCESS) {
            ...
            //! check for buffer full error, which is not expected for this procedure
              ...
              SL_DEBUG_LOG_V2(ERROR, " rsi_ble_get_profiles_async failed with buffer full error - conn%d ", l_conn_id);
              break;
            } else {
              SL_DEBUG_LOG_V2(ERROR,
        ...
        if (!prof_resp_recvd) {
          ...
          if (!done_profiles_query) {
...
#if RSI_DEBUG_EN
            SL_DEBUG_LOG_V2(INFO, "profiles end handle = 0x%x - conn%d \r\n", profiles_endhandle, l_conn_id);
#endif
            //! allocate memory
            if (rsi_ble_profile_list_by_conn.profile_desc == NULL) {
              ...
              if (temp == NULL) {
                SL_DEBUG_LOG_V2(ERROR,
                                "Failed to allocate memory for rsi_ble_profile_list_by_conn.profile_desc , conn_id: %d",
                                l_conn_id);
                return;
              }
...
#if RSI_DEBUG_EN
                SL_DEBUG_LOG_V2(INFO, "earch for profile :0x%x -conn%d \r\n", search_serv.val.val16, l_conn_id);
#endif
...
#if RSI_DEBUG_EN
                SL_DEBUG_LOG_V2(INFO, "Search for profile :0x%x - conn%d \r\n", search_serv.val.val32, l_conn_id);
#endif
...
#if RSI_DEBUG_EN
          SL_DEBUG_LOG_V2(INFO,
                          "GATT profile:Start handle: 0x%x  End handle:0x%x - conn%d ",
...
#if RSI_DEBUG_EN
          SL_DEBUG_LOG_V2(INFO, "Completed GATT profile - conn%d ", l_conn_id);
#endif
          if (!char_resp_recvd) {
...
#if RSI_DEBUG_EN
        SL_DEBUG_LOG_V2(INFO, "In GATT test:RSI_BLE_GATT_DESC_SERVICES : temp1 %d - conn%d \r\n", temp1, l_conn_id);
#endif
        if (temp1 < total_remote_profiles) {
          temp2++;
      ...
      case RSI_BLE_SELECT_DATATRANSFER: {
        ...
        if (ble_conn_conf->tx_write) {
          ...
          if (!write_handle_found) {
            SL_DEBUG_LOG_V2(WARN, "Client GATT write service not found - conn%d \r\n", l_conn_id);
          }
        }
        ...
        //! if receive 'write no response' is configured, then check for client characteristic handle having Gatt 'write No response' property
          ...
          if (!write_wwr_handle_found) {
            SL_DEBUG_LOG_V2(WARN, "Client GATT write no response service not found - conn%d \r\n", l_conn_id);
          }
        }
        ...
        if (ble_conn_conf->rx_indications) {
          ...
          if (!indication_handle_found) {
            SL_DEBUG_LOG_V2(WARN, " Client GATT Indication service not found - conn%d \r\n", l_conn_id);
          }
        }
        ...
        //! if receive 'Notifications' is configured, then check for client characteristic handle having Gatt 'Notification' property
          ...
          if (!notify_handle_found) {
            SL_DEBUG_LOG_V2(WARN, "Client GATT Notification service not found \r\n");
          }
        }
        ...
        if (!conn_param_req_given) {
          ...
          if (status != RSI_SUCCESS) {
            ...
            if (status == RSI_ERROR_BLE_ATT_CMD_IN_PROGRESS) {
              ...
              SL_DEBUG_LOG_V2(ERROR, "Failed to update connection parameters error:0x%lx - conn%d ", status, l_conn_id);
            }
            break;
          } else {
      ...
      case RSI_BLE_CONN_UPDATE_COMPLETE_EVENT: {
        ...
        SL_DEBUG_LOG_V2(INFO,
                        " conn_interval : %d supervision timeout : %d - conn%d",
      ...
      case RSI_BLE_DISCONN_EVENT: {
        ...
        if (rsi_ble_conn_info[l_conn_id].remote_device_role == CENTRAL_ROLE) {
          ...
          if (num_of_conn_centrals < RSI_BLE_MAX_NBR_CENTRALS) {
            status = rsi_ble_stop_advertising();
            if (status != RSI_SUCCESS) {
              SL_DEBUG_LOG_V2(ERROR, "Advertising failed to stop = 0x%lx - conn%d ", status, l_conn_id);
            }
            ...
            status = rsi_ble_start_advertising();
            if (status != RSI_SUCCESS) {
              SL_DEBUG_LOG_V2(ERROR, "Advertising failed to start = 0x%lx - conn%d ", status, l_conn_id);
              continue;
            }
          ...
          status = rsi_ble_start_scanning();
          if (status != RSI_SUCCESS) {
            SL_DEBUG_LOG_V2(ERROR, "Scanning start failed, cmd status = %lx - conn%d\r\n", status, l_conn_id);
          } else {
#if WLAN_TRANSIENT_CASE
            ble_scanning_is_there = 1;
#endif
...
#endif
        //! delete the task
        if (ble_app_task_handle[l_conn_id] != NULL) {
      ...
      case RSI_BLE_GATT_WRITE_EVENT: {
        ...
        if (*(uint16_t *)rsi_ble_conn_info[l_conn_id].app_ble_write_event.handle == (indication_handle)) {
          if (ble_conn_conf->rx_indications) {
            ...
            if (status != RSI_SUCCESS) {
              SL_DEBUG_LOG_V2(ERROR, "Indication confirm failed \t reason = %lx - conn%d", status, l_conn_id);
            } else {
      ...
      case RSI_DATA_TRANSMIT_EVENT: {
#if BLE_DATA_TRANSFER_START && WLAN_THROUGHPUT_TEST && WLAN_SYNC_REQ
        if (wlan_tx_throughput_test) {
          osSemaphoreAcquire(ble_wlan_throughput_sync_sem, osWaitForever);
          SL_DEBUG_LOG_V2(INFO, " Start BLE data transmit\r\n");
        }
#endif
        ...
        if (write_handle_found && ble_conn_conf->tx_write) {
          ...
          if (status != RSI_SUCCESS) {
            ...
            if (status == RSI_ERROR_BLE_ATT_CMD_IN_PROGRESS) {
              ...
              SL_DEBUG_LOG_V2(ERROR, "Write with response failed with buffer error - conn%d ", l_conn_id);
              rsi_current_state[l_conn_id] |= BIT64(RSI_DATA_TRANSMIT_EVENT);
              break;
            } else {
              SL_DEBUG_LOG_V2(ERROR, "Write with response failed with status = %lx - conn%d ", status, l_conn_id);
            }
          } else {
            write_cnt++;
          }
        ...
        //! Gatt write no response to remote device continuously
          ...
          if (status != RSI_SUCCESS) {
            ...
            } else if (status == RSI_ERROR_IN_BUFFER_ALLOCATION) //! TO-DO, add proper error code
            {
              ...
              SL_DEBUG_LOG_V2(ERROR, "Write without response failed with status = 0x%lx - conn%d ", status, l_conn_id);
            }
          } else {
            wwr_count++;
          }
        ...
        //! Notify to remote device continuously
          ...
          if (status != RSI_SUCCESS) {
            if (status == RSI_ERROR_BLE_DEV_BUF_FULL) {
#if RSI_DEBUG_EN
              SL_DEBUG_LOG_V2(ERROR, "Notify %d failed with buffer full error - conn%d \r\n", notfy_cnt, l_conn_id);
#endif
              rsi_ble_clear_event_based_on_conn(l_conn_id, RSI_DATA_TRANSMIT_EVENT);
              rsi_current_state[l_conn_id] |= BIT64(RSI_DATA_TRANSMIT_EVENT);
              break;
            } else if (status == RSI_ERROR_IN_BUFFER_ALLOCATION) //! TO-DO, add proper error code
            {
              ...
              SL_DEBUG_LOG_V2(ERROR, "Notify %d failed with error code %lx - conn%d", notfy_cnt, status, l_conn_id);
            }
          } else {
#if RSI_DEBUG_EN
            SL_DEBUG_LOG_V2(INFO, "Notify %d success - conn%d ", notfy_cnt, l_conn_id);
#endif
            notfy_cnt++;
          }
        }
        ...
        if (notify_handle_found && ble_conn_conf->rx_notifications) {
          ...
          if (status != RSI_SUCCESS) {
            if (status == RSI_ERROR_BLE_DEV_BUF_FULL) {
              ...
              SL_DEBUG_LOG_V2(ERROR, "Notify value failed with status = %lx - conn%d \r\n", status, l_conn_id);
            }
          }
        }
        //! Transmit Gatt indications to remote device
          ...
          if (status != RSI_SUCCESS) {
            if (status == RSI_ERROR_BLE_DEV_BUF_FULL) {
              ...
              SL_DEBUG_LOG_V2(ERROR, "Indication failed with status = %lx - conn%d \r\n", status, l_conn_id);
            }
          }
        }
      } break;
      ...
      case RSI_BLE_READ_REQ_EVENT: {
        ...
        if (status != RSI_SUCCESS) {
          SL_DEBUG_LOG_V2(ERROR, "Read response failed, error:0x%lx - conn%d \r\n", status, l_conn_id);
        } else {
          SL_DEBUG_LOG_V2(INFO,
      ...
      case RSI_APP_EVENT_DATA_LENGTH_CHANGE: {
        ...
        if (rsi_ble_conn_info[l_conn_id].remote_dev_feature.remote_features[1] & 0x01) {
          status = rsi_ble_setphy((int8_t *)rsi_ble_conn_info[l_conn_id].remote_dev_feature.dev_addr,
                                  ...
                                  CODDED_PHY_RATE);
          if (status != RSI_SUCCESS) {
            SL_DEBUG_LOG_V2(ERROR, "Failed to cancel the connection request: 0x%lx  - conn%d", status, l_conn_id);
          }
        }
      } break;
      ...
      case RSI_BLE_SMP_REQ_EVENT: {
        ...
        SL_DEBUG_LOG_V2(INFO, "In SMP request  - conn%d ", l_conn_id);
        if (ble_conn_conf->smp_enable) {
          ...
          if (first_connect == 0) {
            if (!smp_pairing_initated) {
              ...
              if (status != RSI_SUCCESS) {
                SL_DEBUG_LOG_V2(ERROR,
                                "RSI_BLE_SMP_REQ_EVENT: failed to initiate the SMP pairing process: 0x%lx  - conn%d",
                                status,
                                l_conn_id);
              } else {
                smp_pairing_initated = true;
      ...
      case RSI_BLE_LTK_REQ_EVENT: {
							...
							l_rsi_encryption_enabled.localrand, 8))))*/
        {
          ...
          if (status != RSI_SUCCESS) {
            SL_DEBUG_LOG_V2(ERROR, "Failed to restart SMP pairing with status: 0x%lx - conn%d\r\n", status, l_conn_id);
          }
        } else {
