...
extern osThreadId_t common_app_thread_id;
#endif
/*========================================================================*/
//!  CALLBACK FUNCTIONS
/*=======================================================================*/
...
void rsi_ble_on_enhance_conn_status_event(rsi_ble_event_enhance_conn_status_t *resp_enh_conn)
{
  ...
  if (resp_enh_conn->status != 0 && resp_enh_conn->status != 63) {
    SL_DEBUG_LOG_V2(INFO, "On enhanced connect event status report : %d \r\n", resp_enh_conn->status);
    if (remote_device_role == PERIPHERAL_ROLE) {
      peripheral_connection_in_prgs = 0;
    }
    rsi_ble_app_set_event(RSI_BLE_SCAN_RESTART_EVENT);
...
#endif
  osThreadSuspend(common_app_thread_id);
#endif
  ...
  while (1) {
    ...
    if (disable_factor_count == DISABLE_ITER_COUNT) {
      ...
      if (!(peripheral_task_instances || central_task_instances)) {
        if (rsi_wlan_running) {
          osSemaphoreRelease(wlan_sync_coex_ble_sem);
        }
      }
    }
#endif
