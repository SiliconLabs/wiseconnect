...
void light_sensor_init(void)
{
  ...
  if (status != SL_I2C_SUCCESS) {
    SL_DEBUG_LOG_V2(ERROR, "sl_i2c_driver_init : Invalid Parameters, Error Code: 0x%ld \r\n", status);
    return;
  }
...
sl_status_t white_light_read(float *white_light_lux)
{
  ...
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "white ch lux measurement failed, error Code: 0x%ld \r\n", status);
  }
  return status;
}
...
sl_status_t ambient_light_read(float *ambient_light_lux)
{
  ...
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "ambient ch lux measurement failed, error Code: 0x%ld \r\n", status);
  }
  return status;
}
...
void light_sensor_deinit(void)
{
  ...
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "sl_i2c_driver_deinit failed, error Code: 0x%ld \r\n", status);
    return;
  }
