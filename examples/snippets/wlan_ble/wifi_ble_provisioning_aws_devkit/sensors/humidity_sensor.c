...
void humidity_sensor_init(void)
{
  ...
  if (sl_si91x_gpio_driver_get_uulp_npss_pin(SENSOR_ENABLE_GPIO_PIN) != 1) {
    ...
    if (status != SL_STATUS_OK) {
      SL_DEBUG_LOG_V2(ERROR, "sl_si91x_gpio_driver_enable_clock, Error code: %lu\r\n", status);
      return;
    }
    // Set NPSS GPIO pin MUX
    status = sl_si91x_gpio_driver_set_uulp_npss_pin_mux(SENSOR_ENABLE_GPIO_PIN, NPSS_GPIO_PIN_MUX_MODE0);
    if (status != SL_STATUS_OK) {
      SL_DEBUG_LOG_V2(ERROR, "sl_si91x_gpio_driver_set_uulp_npss_pin_mux, Error code: %lu\r\n", status);
      return;
    }
    ...
    if (status != SL_STATUS_OK) {
      SL_DEBUG_LOG_V2(ERROR, "sl_si91x_gpio_driver_set_uulp_npss_direction, Error code: %lu\r\n", status);
      return;
    }
    // Set UULP GPIO pin
    status = sl_si91x_gpio_driver_set_uulp_npss_pin_value(SENSOR_ENABLE_GPIO_PIN, SET);
    if (status != SL_STATUS_OK) {
      SL_DEBUG_LOG_V2(ERROR, "sl_si91x_gpio_driver_set_uulp_npss_pin_value, Error code: %lu\r\n", status);
      return;
    }
  }
#else
  ...
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "sl_gpio_driver_get_pin, Error code: %lu\r\n", status);
    return;
  }
  if (pin_value != 1) {
    // Enable GPIO CLK
    ...
    if (status != SL_STATUS_OK) {
      SL_DEBUG_LOG_V2(ERROR, "sl_gpio_driver_set_pin_mode, Error code: %lu\r\n", status);
      return;
    }
    ...
    if (status != SL_STATUS_OK) {
      SL_DEBUG_LOG_V2(ERROR, "sl_si91x_gpio_driver_set_pin_direction, Error code: %lu\r\n", status);
      return;
    }
    // Set GPIO pin
    status = sl_gpio_driver_set_pin(&sensor_enable_port_pin); // Set ULP GPIO pin
    if (status != SL_STATUS_OK) {
      SL_DEBUG_LOG_V2(ERROR, "sl_gpio_driver_set_pin, Error code: %lu\r\n", status);
      return;
    }
  }
#endif
  ...
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Sensor reset un-successful, Error Code: 0x%ld \r\n", status);
    return;
  }
  osDelay(10);
  ...
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Sensor initialization un-successful, Error Code: 0x%ld \r\n", status);
    return;
  }
  osDelay(10);
  ...
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Sensor firmware version un-successful, Error Code: 0x%ld \r\n", status);
    return;
  }
  osDelay(10);
  ...
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Sensor user register 1 write data failed, Error Code: 0x%ld \r\n", status);
    return;
  }
  // Reads register data from sensor
  status = sl_si91x_si70xx_read_control_register(I2C, SI70XX_SLAVE_ADDR, SL_RH_T_USER_REG, &value);
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Sensor user register 1 read failed, Error Code: 0x%ld \r\n", status);
    return;
  }
  osDelay(10);
  ...
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Sensor temperature read failed, Error Code: 0x%ld \r\n", status);
    return;
  }
  // measure humidity data from sensor
  status = sl_si91x_si70xx_measure_humidity(I2C, SI70XX_SLAVE_ADDR, &humidity);
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Sensor humidity read failed, Error Code: 0x%ld \r\n", status);
    return;
  }
  osDelay(10);
  ...
  osDelay(10);
  SL_DEBUG_LOG_V2(INFO, "Humidity sensor initialized\r\n");
}
...
{
  sl_status_t status = sl_si91x_si70xx_measure_temperature(I2C, SI70XX_SLAVE_ADDR, temperature);
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Temperature read failed, Error Code: 0x%ld \r\n", status);
  }
  return status;
}