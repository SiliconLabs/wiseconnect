...
uint8_t rsi_get_remote_device_role(uint8_t *remote_dev_addr)
{
  ...
  //! Loop all structures and if the device addr is matched for peripheral structure,
  //! then return peripheral role or else central role
  for (i = 0; i < TOTAL_CONNECTIONS; i++) {
...
uint8_t rsi_add_ble_conn_id(uint8_t *remote_dev_addr, uint8_t *remote_name, uint8_t size)
{
  ...
  if (conn_id == 0xff) {
    SL_DEBUG_LOG_V2(ERROR, " failed to add dev to connection buffer");
  }
