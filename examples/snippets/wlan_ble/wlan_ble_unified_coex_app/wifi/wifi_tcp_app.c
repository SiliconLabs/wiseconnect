...
void send_data_to_tcp_server(void)
{
  ...
  if (status != 0) {
    SL_DEBUG_LOG_V2(ERROR, "socket option for max_retry_timeout failed");
    return;
  }

  SL_DEBUG_LOG_V2(WARN, "socket option for max_retry_timeout successful");

  status = sl_si91x_setsockopt(client_socket, SOL_TCP, SL_SI91X_SO_MAXRETRY, &max_tcp_retry, sizeof(max_tcp_retry));
  if (status != 0) {
    SL_DEBUG_LOG_V2(ERROR, "socket option for max_tcp_retry failed");
    return;
  }
  SL_DEBUG_LOG_V2(WARN, "socket option for max_tcp_retry successful");
