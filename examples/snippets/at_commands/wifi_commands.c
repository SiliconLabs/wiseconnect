...
/// Wi-Fi enterprise client configuration
...
sl_wifi_device_configuration_t si91x_init_configuration = { 0 };
...
sl_status_t set_oper_mode_command_handler(console_args_t *arguments)
{
  ...
  uint16_t oper_mode = GET_OPTIONAL_COMMAND_ARG(arguments, 0, SL_SI91X_CLIENT_MODE, sl_si91x_operation_mode_t);
  switch (oper_mode) {
...
sl_status_t set_coex_mode_command_handler(console_args_t *arguments)
{
  ...
  uint16_t coex_mode = GET_OPTIONAL_COMMAND_ARG(arguments, 0, SL_SI91X_WLAN_ONLY_MODE, sl_si91x_coex_mode_t);
...
sl_status_t set_region_code_command_handler(console_args_t *arguments)
{
  ...
  uint8_t region_code                  = GET_OPTIONAL_COMMAND_ARG(arguments, 0, US, sl_si91x_region_code_t);
...
sl_status_t set_mac_address_command_handler(console_args_t *arguments)
{
  ...
  const char *mac_addr = GET_OPTIONAL_COMMAND_ARG(arguments, 0, 0, const char *);
  PRINT_AT_CMD_SUCCESS;
  if (mac_addr != NULL) {
...
static sl_status_t show_scan_results(sl_wifi_scan_result_t *scan_result)
{
  ...
  printf("at+WIFI_SCAN_RESULTS=%lu,,", scan_result->scan_count);
...
sl_status_t wifi_disconnect_command_handler(console_args_t *arguments)
{
  ...
  sl_status_t status            = sl_wifi_disconnect(interface);
  PRINT_AT_CMD_SUCCESS;
  return status;
}
...
sl_status_t wifi_get_network_info_handler(const console_args_t *arguments)
{
  ...
  PRINT_AT_CMD_SUCCESS;
  printf("%s %u \"%s\" ", info.wlan_state ? "CONNECTED" : "DISCONNECTED", info.channel_number, info.ssid);

  print_mac_address((const sl_mac_address_t *)&info.mac_address);
  printf(" %u ", info.sec_type);
  for (uint8_t ii = 0; ii < 32; ii++) {
    printf("%x", info.psk_pmk[ii]);
  }
  printf(" ");
