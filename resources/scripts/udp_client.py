...
def main():
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
            s.settimeout(5)  # Timeout for recv operations

            # Send an initial packet to the server to "initiate" communication
            initial_message = "Hello, UDP Server!"
            s.sendto(initial_message.encode(), (server_ip, server_port))
            print(f"Sent initial message to {server_ip}:{server_port}")
