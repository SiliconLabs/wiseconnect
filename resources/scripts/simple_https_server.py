"""
Description: This script is a simple python HTTPS web server which accepts a PUT request and writes the supplied data to a file.
...
class SimpleHandler(BaseHTTPRequestHandler):
    ...
    def do_PUT(self):
        ...
        # Configurable response (same knobs as HTTP server)
        try:
    ...
    def do_POST(self):
        ...
        resp = json.dumps(
            {"status": "ok", "received_bytes": len(data)}
        ).encode("utf-8")
    ...
    def do_GET(self):
        ...
        if rel in ("", ".", os.path.sep):
            banner = b"WiseConnect HTTPS server OK\n"
        ...
        if os.path.isdir(full_path):
            ...
            print(
                "GET: directory requested; sending 404 bytes =",
                len(msg),
                "for",
                raw_path,
            )
...
def run_on(port):
    ...
    # TLS configuration modeled after modern ssl.SSLContext usage.
    ...
    # Prefer at least TLS 1.2 when supported
    ...
    print(
        "Listening on: https://0.0.0.0:%d (Python %s, %s)"
        % (port, sys.version.split()[0], platform.system())
    )
