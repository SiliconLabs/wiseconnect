...
def set_cert(filetype,cert_inx,cert):
    cert = cert.replace('\r\n','\n')
    ...
    if sys.argv[1] == '2':
     num =len (cert) + 1
     length_cert=str(num)
     out='at+rsi_cert_inx=' + filetype + ','+length_cert+','+cert_inx+',,'+cert+'\0'+'\r\n'
    else:
     out='at+rsi_cert_inx=' + filetype + ','+length_cert+','+cert_inx+',,'+cert+'\r\n'
    print out
    root(out)
...
def main(filetype,cert_inx,filepath):
    cert = read_file(filepath)
    set_cert(filetype,cert_inx,cert) 

if __name__ == "__main__":
    if len(sys.argv) == 4:
        main(sys.argv[1],sys.argv[2],sys.argv[3])
    else:
        print 'Usage: %s <certificate_type> <certificate_index> <certificate_path>' % sys.argv[0]
