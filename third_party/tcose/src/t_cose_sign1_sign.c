 ...
 * way.
 */
static inline struct q_useful_buf_c encode_protected_parameters(int32_t cose_algorithm_id,
 ...
 * cbor_encode_ctx.
 */
static inline enum t_cose_err_t add_unprotected_parameters(const struct t_cose_sign1_sign_ctx *me,
                                                           ...
                                                           QCBOREncodeContext *cbor_encode_ctx)
{
  ...
  if (me->content_type_uint != T_COSE_EMPTY_UINT_CONTENT_TYPE && me->content_type_tstr != NULL) {
  ...
  if (me->content_type_uint != T_COSE_EMPTY_UINT_CONTENT_TYPE) {
    QCBOREncode_AddUInt64ToMapN(cbor_encode_ctx, COSE_HEADER_PARAM_CONTENT_TYPE, me->content_type_uint);
  }

  if (me->content_type_tstr != NULL) {
    QCBOREncode_AddSZStringToMapN(cbor_encode_ctx, COSE_HEADER_PARAM_CONTENT_TYPE, me->content_type_tstr);
  }
#else
 ...
 */
enum t_cose_err_t t_cose_sign1_encode_parameters_internal(struct t_cose_sign1_sign_ctx *me,
 ...
 */
static inline enum t_cose_err_t sign1_sign_short_circuit(struct t_cose_sign1_sign_ctx *me,
                                                         ...
                                                         struct q_useful_buf_c *signature)
{
  ...
  /* Create the hash of the to-be-signed bytes. Inputs to the
     ...
     */
  return_value =
    create_tbs_hash(me->cose_algorithm_id, me->protected_parameters, aad, payload, buffer_for_tbs_hash, &tbs_hash);
  if (return_value) {
    goto Done;
  }
  ...
  if (buffer_for_signature.ptr == NULL) {
    ...
    return_value = short_circuit_sign(me->cose_algorithm_id, tbs_hash, buffer_for_signature, signature);
  }
 ...
 */
static inline enum t_cose_err_t sign1_sign_eddsa(struct t_cose_sign1_sign_ctx *me,
                                                 struct q_useful_buf_c aad,
                                                 struct q_useful_buf_c payload,
                                                 struct q_useful_buf buffer_for_signature,
                                                 struct q_useful_buf_c *signature)
{
  ...
  /* Serialize the TBS data into the auxiliary buffer.
     ...
     * the necessary size.
     */
  return_value = create_tbs(me->protected_parameters, aad, payload, me->auxiliary_buffer, &tbs);
  ...
  if (buffer_for_signature.ptr == NULL) {
    /* Output size calculation. Only need signature size. */
    signature->ptr = NULL;
    return_value   = t_cose_crypto_sig_size(me->cose_algorithm_id, me->signing_key, &signature->len);
    ...
    /* Perform the public key signing over the TBS bytes we just
         * serialized.
         */
    return_value = t_cose_crypto_sign_eddsa(me->signing_key, tbs, buffer_for_signature, signature);
  }
 ...
 * will compute the necessary size, and update \c signature accordingly.
 */
static inline enum t_cose_err_t sign1_sign_default(struct t_cose_sign1_sign_ctx *me,
                                                   struct q_useful_buf_c aad,
                                                   struct q_useful_buf_c payload,
                                                   struct q_useful_buf buffer_for_signature,
                                                   struct q_useful_buf_c *signature)
{
  ...
  if (buffer_for_signature.ptr == NULL) {
    ...
    return_value =
      t_cose_crypto_sign(me->cose_algorithm_id, me->signing_key, tbs_hash, buffer_for_signature, signature);
  }
 ...
 * Semi-private function. See t_cose_sign1_sign.h
 */
enum t_cose_err_t t_cose_sign1_encode_signature_aad_internal(struct t_cose_sign1_sign_ctx *me,
                                                             struct q_useful_buf_c aad,
...
#ifndef T_COSE_DISABLE_SHORT_CIRCUIT_SIGN
  if (me->option_flags & T_COSE_OPT_SHORT_CIRCUIT_SIG) {
    return_value = sign1_sign_short_circuit(me, aad, signed_payload, buffer_for_signature, &signature);
  } else
#endif
#ifndef T_COSE_DISABLE_EDDSA
    if (me->cose_algorithm_id == COSE_ALGORITHM_EDDSA) {
    return_value = sign1_sign_eddsa(me, aad, signed_payload, buffer_for_signature, &signature);
  } else
#endif
  {
    return_value = sign1_sign_default(me, aad, signed_payload, buffer_for_signature, &signature);
  }
 ...
 * Semi-private function. See t_cose_sign1_sign.h
 */
enum t_cose_err_t t_cose_sign1_sign_aad_internal(struct t_cose_sign1_sign_ctx *me,
                                                 bool payload_is_detached,
                                                 struct q_useful_buf_c payload,
                                                 struct q_useful_buf_c aad,
