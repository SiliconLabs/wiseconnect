 ...
 */
static inline enum t_cose_err_t add_label_to_list(const QCBORItem *item, struct t_cose_label_list *label_list)
{
 ...
 */
inline static bool is_label_list_clear(const struct t_cose_label_list *list)
{
 ...
 */
static inline enum t_cose_err_t decode_critical_parameter(QCBORDecodeContext *decode_context,
 ...
 */
enum t_cose_err_t check_critical_labels(const struct t_cose_label_list *critical_labels,
                                        const struct t_cose_label_list *unknown_labels)
{
  ...
  for (num_unknown = 0; unknown_labels->int_labels[num_unknown]; num_unknown++) {
    ...
    for (num_critical = 0; critical_labels->int_labels[num_critical]; num_critical++) {
  ...
  for (num_unknown = 0; !q_useful_buf_c_is_null(unknown_labels->tstr_labels[num_unknown]); num_unknown++) {
    ...
    for (num_critical = 0; !q_useful_buf_c_is_null(critical_labels->tstr_labels[num_critical]); num_critical++) {
      if (!q_useful_buf_compare(critical_labels->tstr_labels[num_critical], unknown_labels->tstr_labels[num_unknown])) {
        /* Found a critical label that is unknown to us */
        return_value = T_COSE_ERR_UNKNOWN_CRITICAL_PARAMETER;
        goto Done;
      }
...
static QCBORError header_parameter_callback(void *pCallbackCtx, const QCBORItem *pItem)
{
  ...
  if (pItem->uLabelType == QCBOR_TYPE_INT64 && pItem->label.int64 == COSE_HEADER_PARAM_CRIT) {
 ...
 */
enum t_cose_err_t parse_cose_header_parameters(QCBORDecodeContext *decode_context,
...
#define END_INDEX        5
  ...
  /* This call takes care of duplicate detection in the map itself.
     ...
     */
  QCBORDecode_GetItemsInMapWithCallback(decode_context, header_items, &callback_context, header_parameter_callback);
  ...
  if (header_items[CONTENT_TYPE].uDataType == QCBOR_TYPE_TEXT_STRING) {
    ...
    if (header_items[CONTENT_TYPE].val.int64 < 0 || header_items[CONTENT_TYPE].val.int64 > UINT16_MAX) {
