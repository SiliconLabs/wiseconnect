...
static inline struct q_useful_buf_c useful_buf_copy_offset(struct q_useful_buf dest,
                                                           size_t offset,
...
static inline struct q_useful_buf_c q_useful_buf_copy(struct q_useful_buf dest, struct q_useful_buf_c src)
{
  return UsefulBuf_Copy(dest, src);
}

static inline struct q_useful_buf_c q_useful_buf_set(struct q_useful_buf dest, uint8_t value)
{
  return UsefulBuf_Set(dest, value);
}

static inline struct q_useful_buf_c q_useful_buf_copy_ptr(struct q_useful_buf d, const void *ptr, size_t len)
{
  return UsefulBuf_CopyPtr(d, ptr, len);
}

static inline struct q_useful_buf_c q_useful_buf_head(struct q_useful_buf_c buf, size_t amount)
{
  return UsefulBuf_Head(buf, amount);
}

static inline struct q_useful_buf_c q_useful_buf_tail(struct q_useful_buf_c buf, size_t amount)
{
  return UsefulBuf_Tail(buf, amount);
}

static inline int q_useful_buf_compare(const struct q_useful_buf_c buf1, const struct q_useful_buf_c buf2)
{
  return UsefulBuf_Compare(buf1, buf2);
}

static inline size_t q_useful_buf_is_value(const struct q_useful_buf_c buf, uint8_t uValue)
{
  return UsefulBuf_IsValue(buf, uValue);
}

static inline size_t q_useful_buf_find_bytes(const struct q_useful_buf_c bytes_to_search,
