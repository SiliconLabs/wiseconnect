...
#ifndef MQTTSI91X_LWIP_H
#define MQTTSI91X_LWIP_H
...
typedef struct TLS_ctx {
  uint8_t *cacert;        // Pointer to CA certificate
  size_t cacert_len;      // Length of CA certificate
  uint8_t *client_cert;   // Pointer to client certificate
  size_t client_cert_len; // Length of client certificate
  uint8_t *client_key;    // Pointer to client private key
...
typedef struct {
  ...
  mbedtls_x509_crt client_cert;   // mbedTLS client certificate
  mbedtls_pk_context private_key; // mbedTLS private key
  mbedtls_entropy_context entropy;
  mbedtls_ctr_drbg_context ctr_drbg;
  int socket_fd;
  bool initialized;
  bool in_use;
  TLS_cert_ctx_t cert_ctx; // Certificate data storage
} mqtt_tls_context_t;
#endif
