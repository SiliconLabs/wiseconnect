...
static int lfs_alloc(lfs_t *lfs, lfs_block_t *block)
{
  while (true) {
    ...
    if (lfs->lookahead.ckpoint <= 0) {
      LFS_ERROR("No more free space 0x%" PRIx32, (lfs->lookahead.start + lfs->lookahead.next) % lfs->block_count);
...
nextname:
    ...
    *path = name;
    ...
    if (lfs_tag_id(tag) != 0x3ff) {
      lfs_stag_t res =
        lfs_dir_get(lfs, dir, LFS_MKTAG(0x700, 0x3ff, 0), LFS_MKTAG(LFS_TYPE_STRUCT, lfs_tag_id(tag), 8), dir->tail);
      if (res < 0) {
        return res;
      }
...
static int lfs_dir_orphaningcommit(lfs_t *lfs, lfs_mdir_t *dir, const struct lfs_mattr *attrs, int attrcount)
{
  ...
  if (state == LFS_OK_DROPPED) {
    ...
    lpair[1] = pdir.pair[1];
    lfs_pair_tole32(dir->tail);
    state = lfs_dir_relocatingcommit(lfs,
                                     &pdir,
                                     lpair,
...
static lfs_ssize_t lfs_file_flushedread(lfs_t *lfs, lfs_file_t *file, void *buffer, lfs_size_t size)
{
  ...
  while (nsize > 0) {
    ...
    if (!(file->flags & LFS_F_READING) || file->off == lfs->cfg->block_size) {
      if (!(file->flags & LFS_F_INLINE)) {
        int err =
          lfs_ctz_find(lfs, NULL, &file->cache, file->ctz.head, file->ctz.size, file->pos, &file->block, &file->off);
        if (err) {
          return err;
        }
      } else {
  ...
  if (lfs->inline_max == (lfs_size_t)-1) {
    ...
    lfs->inline_max =
      lfs_min(lfs->cfg->cache_size,
              lfs_min(lfs->attr_max, ((lfs->cfg->metadata_max) ? lfs->cfg->metadata_max : lfs->cfg->block_size) / 8));
  }
...
static int lfs_fs_stat_(lfs_t *lfs, struct lfs_fsinfo *fsinfo)
{
  ...
  if (!lfs_gstate_needssuperblock(&lfs->gstate)) {
    ...
    lfs_superblock_t superblock;
    lfs_stag_t tag = lfs_dir_get(lfs,
                                 &dir,
                                 LFS_MKTAG(0x7ff, 0x3ff, 0),
                                 LFS_MKTAG(LFS_TYPE_INLINESTRUCT, 0, sizeof(superblock)),
                                 &superblock);
...
static int lfs_fs_parent_match(void *data, lfs_tag_t tag, const void *buffer)
{
  ...
  lfs_block_t child[2];
  int err =
    lfs_bd_read(lfs, &lfs->pcache, &lfs->rcache, lfs->cfg->block_size, disk->block, disk->off, &child, sizeof(child));
  if (err) {
    return err;
  }
...
static int lfs_fs_gc_(lfs_t *lfs)
{
  ...
  if (lfs->cfg->compact_thresh < lfs->cfg->block_size - lfs->cfg->prog_size) {
    ...
    while (!lfs_pair_isnull(mdir.tail)) {
      ...
      if (!mdir.erased
          || ((lfs->cfg->compact_thresh == 0) ? mdir.off > lfs->cfg->block_size - lfs->cfg->block_size / 8
