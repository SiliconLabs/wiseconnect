...
/**
 * os_wait_for_thread_stop_flag - Waits for eloop thread stop flag for
 *                                given amount of time.
 * @sec: Time in Seconds
 * @usec: Time in microseconds
 * Returns: Thread stop event flag before clearing or error code if highest
 *           bit set.
 */
...
extern long glbl_free_val;
extern long totalloc;
extern long dbg_max_taken;
#define os_malloc(x)                           \
  _os_malloc(x);                               \
  dbg_printf_obe_final(" ");                   \
  dbg_printf_obe_final(__func__);              \
  dbg_printf_obe_final(" M ");                 \
  dbg_printf_val_obe_final_dec(x);             \
  dbg_printf_obe_final(" ");                   \
  dbg_printf_val_obe_final_dec(totalloc);      \
  dbg_printf_obe_final(" ");                   \
  dbg_printf_val_obe_final_dec(dbg_max_taken); \
  dbg_printf_obe_final("\r\n");

#define os_realloc(x, y)                       \
  _os_realloc(x, y);                           \
  dbg_printf_obe_final(" ");                   \
  dbg_printf_obe_final(__func__);              \
  dbg_printf_obe_final(" R ");                 \
  dbg_printf_val_obe_final_dec(y);             \
  dbg_printf_obe_final(" ");                   \
  dbg_printf_val_obe_final_dec(totalloc);      \
  dbg_printf_obe_final(" ");                   \
  dbg_printf_val_obe_final_dec(dbg_max_taken); \
...
#define os_free(x)                             \
  _os_free(x);                                 \
  dbg_printf_val_obe_final(x);                 \
  dbg_printf_obe_final(" ");                   \
  dbg_printf_obe_final(__func__);              \
  dbg_printf_obe_final(" F ");                 \
  dbg_printf_val_obe_final_dec(glbl_free_val); \
  dbg_printf_obe_final(" ");                   \
  dbg_printf_val_obe_final_dec(totalloc);      \
  dbg_printf_obe_final(" ");                   \
  dbg_printf_val_obe_final_dec(dbg_max_taken); \
...
#define os_zalloc(x)     _os_zalloc(x)
#define OS_DELAY(x)       osDelay(pdMS_TO_TICKS(x))
#endif
/**
 ...
 */
void *_os_realloc(void *ptr, size_t size);
...
#if defined(ROM_IMAGE_20) || defined(SUPPLICANT_NON_ROM)
...
extern void *os_realloc_array(void *ptr, size_t nmemb, size_t size);
