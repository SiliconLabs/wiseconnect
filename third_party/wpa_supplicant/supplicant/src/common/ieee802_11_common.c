				    ...
				    size_t nei_rep_len)
{
	...
	while (pos) {
		...
		*nei_pos++ = sl_atoi((uint8_t *)pos); /* Operating Class */
		pos = os_strchr(pos, ',');
		if (pos == NULL) {
		...
		*nei_pos++ = sl_atoi((uint8_t *)pos); /* Channel Number */
		pos = os_strchr(pos, ',');
		if (pos == NULL) {
		...
		*nei_pos++ = sl_atoi((uint8_t *)pos); /* PHY Type */
		end = os_strchr(pos, ' ');
		tmp = os_strchr(pos, ',');
		if (tmp && (!end || tmp < end)) {
...
Boolean ieee802_11_rsnx_capab_len(const u8 *rsnxe, size_t rsnxe_len,
			       unsigned int capab)
{
	const u8 *end;
	size_t flen, i;
...
Boolean ieee802_11_rsnx_capab(const u8 *rsnxe, unsigned int capab)
{
