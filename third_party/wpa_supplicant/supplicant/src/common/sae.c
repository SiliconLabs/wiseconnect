...
//	res = dragonfly_is_quadratic_residue_blind(sae->tmp->ec, qr, qnr,						   y_sqr);
	res = is_quadratic_residue_blind(sae, prime, bits, qr, qnr, y_sqr);
	crypto_bignum_deinit(y_sqr, 1);
	if (res < 0)
		return res;
	return res;//const_time_select_int(in_range, res, 0);
}
...
//	u8 x_bin[SAE_MAX_ECC_PRIME_LEN];
	...
	x_bin   =(u8 *)os_malloc(SAE_MAX_ECC_PRIME_LEN);
	x_cand_bin   =(u8 *)os_malloc(SAE_MAX_ECC_PRIME_LEN);
	qr_bin	=(u8 *)os_malloc(SAE_MAX_ECC_PRIME_LEN);
	qnr_bin	=(u8 *)os_malloc(SAE_MAX_ECC_PRIME_LEN);

	os_memset(x_bin, 0, 66 /*sizeof(x_bin)*/);
	prime	=(u8 *)os_malloc(SAE_MAX_ECC_PRIME_LEN);	// handling memory(stack)
	...
	if (crypto_bignum_to_bin(sae->tmp->prime, prime, 66/*sizeof(prime)*/,
				 prime_len) < 0)
	{
		goto fail;
	}
	...
	if (get_random_qr_qnr(prime, prime_len, sae->tmp->prime, bits, &qr, &qnr) < 0 ||
	    crypto_bignum_to_bin(qr, qr_bin, 66/*sizeof(qr_bin) */, prime_len) < 0 ||
	    crypto_bignum_to_bin(qnr, qnr_bin, 66/*sizeof(qnr_bin) */, prime_len) < 0)
	{
		goto fail;
	}
				     ...
				     const struct crypto_bignum *u)
{
	...
	if (crypto_bignum_sqrmod(x1, prime, t1) < 0 ||
       crypto_bignum_mulmod(t1, x1, prime, t1) <0 ||
#else
	...
	/* gx2 = x2^3 + a * x2 + b */
#ifdef SUPPLICANT_PORTING
	if (crypto_bignum_sqrmod(x2, prime, t1) < 0 ||
       crypto_bignum_mulmod(t1, x2, prime, t1) <0 ||
#else
			   ...
			   u8 *confirm)
{
	...
	addr[4] = element2;
	len[4] = element2_len;
	hkdf_extract(sae->tmp->kck_len, sae->tmp->kck, sae->tmp->kck_len,
			    5, addr, len, confirm);
}
