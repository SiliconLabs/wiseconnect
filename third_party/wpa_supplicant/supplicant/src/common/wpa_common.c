		      ...
		      const u8 *buf, size_t len, u8 *mic)
{
  ...
  /*If fips mode is enable should no use RC4 cipher */
...
/* We are now going to derive PTK from the BTK
 * To do that, apply PRF-XXX onto the BTK
 * The various fields include the following:
 * key is the input key to be used for further key derivation
 * key_len is fixed based on the ciphers used
 * rn is the rekey number, initially set to 0.
 * BSSID is the BSSID of the network
 * output - this field will contain the o/p key
 * len will contain the output len
 */
void wpa_btk_to_ptk(const u8 *key, size_t key_len, int *rn, unsigned char *bssid,
		    u8 *output, size_t len)
{
  u8 input[1024];
  u8 input_len = 0;

  os_memcpy(&input[input_len], rn, 4);
  input_len += 4;
  /* Do not put a zero between the RN and the BSSID */
  os_memcpy(&input[input_len], bssid, ETH_ALEN);
  input_len += ETH_ALEN;

  sha1_prf_ccx(key, key_len, NULL, input, input_len, output, len);
  wpa_hexdump_key(MSG_DEBUG, "PTK is: ", output, len);
}

void
wpa_cckm_hmac_calculatev1(u8 *key, u8 key_len, u8 *wpa_ie, u8 wpa_ie_len, u8 *addr1, 
			  u8 *addr2, int rekey_number, u8 *tsf, u8 *buf, u8 buf_len)
{
  /* Steps to be followed
   * Step-1: Get the KRK and RSNIE of the AP 
   * Step-2: Call HMAC-MD5 algorithm with appropriate arguments
   */
   u8 length = 0;
   u8 data[200];
   ...
   {
     test_len = SHA1_MAC_LEN;
     type = 1;
   }
   else
   {
     test_len = MD5_MAC_LEN;
     type = 0; 
   }
   /* Address1 has to be STA's MAC
    * Address2 has to be BSSID 
    */
   os_memcpy(data, addr1, ETH_ALEN);
   length += ETH_ALEN;
   os_memcpy(&data[ETH_ALEN], addr2, ETH_ALEN);
   ...
   os_memcpy(&data[length], wpa_ie, wpa_ie_len);
   length += wpa_ie_len;

   os_memcpy(&data[length], tsf, 8);
   length += 8;
   os_memcpy(&data[length], &rekey_number, 4);
   length += 4;
   ...
   wpa_printf(MSG_INFO, "Calculating new CCKM keys");
   wpa_hexdump(MSG_DEBUG, "Addr1 is", addr1, 6);
   wpa_hexdump(MSG_DEBUG, "Addr2 is", addr2, 6);
   wpa_hexdump(MSG_DEBUG, "WPA IE is", wpa_ie, wpa_ie_len);
   wpa_hexdump(MSG_DEBUG, "TSF is", tsf, 8);
   wpa_hexdump(MSG_DEBUG, "RN is", (u8 *)&rekey_number, 4);

   wpa_hexdump(MSG_DEBUG, "Buffer is", data, length);
   wpa_hexdump_key(MSG_DEBUG, "Key is", key, key_len); 	
   ...
   while (pos < buf_len) {
	   ...
	   if (plen >= test_len) {
		   if (type)
		   {	
		     hmac_sha1_vector(key, key_len, 1, addr, len,
		  	 	      &buf[pos]);
                   }
		   else
		   {
		     hmac_md5_vector(key, key_len, 1, addr, len,
		  		     &buf[pos]);
		   }  	 
		   pos += test_len;
	   } else {
...
void wpa_pmk_to_btk(const u8 *pmk, size_t pmk_len, const char *label,
		    const u8 *addr1, const u8 *addr2,
		    const u8 *nonce1, const u8 *nonce2,
		    u8 *ptk, size_t ptk_len)
{
	...
	/* Address1 has to be BSSID 
	 * Address2 has to be STA's MAC
	 */
	os_memcpy(data, addr1, ETH_ALEN);
	os_memcpy(data + ETH_ALEN, addr2, ETH_ALEN);

	/* Station nonce followed by key nonce */
	os_memcpy(data + 2 * ETH_ALEN, nonce1, WPA_NONCE_LEN);
	os_memcpy(data + 2 * ETH_ALEN + WPA_NONCE_LEN, nonce2, WPA_NONCE_LEN);

	sha1_prf_ccx(pmk, pmk_len, label, data, sizeof(data), ptk, ptk_len);

	wpa_printf(MSG_DEBUG, "WPA: PTK derivation - A1=" MACSTR " A2=" MACSTR,
		   MAC2STR(addr1), MAC2STR(addr2));
	wpa_hexdump_key(MSG_DEBUG, "WPA: PMK", pmk, pmk_len);
	wpa_hexdump_key(MSG_DEBUG, "WPA: KRK|BTK", ptk, ptk_len);
}
#endif
