...
#if defined(ROM_PATCH_FRAMEWORK)
#include "rom_patch_constants.h"
#include "wrappers_wlan.h"                        
#include "wrappers_bootloader_common.h"
#endif
#include "crypto_rom_image.h"
#include "umac_rom_image.h"
#include <wolfssl/wolfcrypt/sp_int.h>
#include "crypto_non_rom_image.h" 
int mp_read_radix_non_rom (mp_int * a, const char *str, int radix);
int wc_ecc_import_point_der_non_rom(byte* in, word32 inLen, const int curve_idx,
                            ecc_point* point);
#ifndef CONFIG_FIPS
...
#else
	return mp_addmod_non_rom((mp_int *) a, (mp_int *) b, (mp_int *) c,
			 (mp_int *) d) == MP_OKAY ?  0 : -1;
#endif
}
			 ...
			 struct crypto_bignum *d)
{
	...
	return mp_mulmod((mp_int *) a, (mp_int *) b, (mp_int *) m,
			 (mp_int *) d) == MP_OKAY ?  0 : -1;
#else
	return mp_mulmod_non_rom((mp_int *) a, (mp_int *) b, (mp_int *) m,
			 (mp_int *) d) == MP_OKAY ?  0 : -1;
#endif	
}
...
struct crypto_ec * crypto_ec_init(int group)
{
	...
	if (wc_ecc_init(&e->key) != 0 ||
#ifndef CHIP_9117	    
	    wc_ecc_set_curve_non_rom(&e->key, 0, curve_id) != 0 ||
#else
	    ...
	    mp_init(&e->b) != MP_OKAY ||
#ifndef CHIP_9117	    
	    mp_read_radix_non_rom(&e->a, e->key.dp->Af, 16) != MP_OKAY ||
	    mp_read_radix_non_rom(&e->b, e->key.dp->Bf, 16) != MP_OKAY ||
	    mp_read_radix_non_rom(&e->prime, e->key.dp->prime, 16) != MP_OKAY ||
	    mp_read_radix_non_rom(&e->order, e->key.dp->order, 16) != MP_OKAY ||
#else
...
#ifdef CHIP_9118_SUPP
#ifdef SSL_PORTING
	//uint8 proceed = rsi_ecc_projective_add_point((ecc_point*)a,(ecc_point*) b, (ecc_point*)c, modulus);
#ifdef CHIP_9117B0	 
	uint8 proceed = RSI_ECC_PROJECTIVE_ADD_POINT((ecc_point*)a,(ecc_point*) b, (ecc_point*)c, modulus);
#else	
	uint8 proceed = rsi_ecc_projective_add_point_non_rom((ecc_point*)a,(ecc_point*) b, (ecc_point*)c, modulus);
#endif	
	if(proceed == 0)
	{
		return 0;
				  ...
				  const struct crypto_bignum *x, int y_bit)
{
	...
	ret = wc_ecc_import_point_der_non_rom(buf, 1 + 2 * ret, e->key.idx,
				      (ecc_point *) p);
#else
			      ...
			      const struct crypto_bignum *x)
{
#if 0 //ndef CHIP_9117	
	mp_int *my2= NULL;
	sp_int y2[1], t[1], sx[1], sea[1], seb[1], sep[1];
	int calced = 0;
	uint8 buf1[64];
	uint32 unsigned_bin_length;
	int32 err;
	my2 = (mp_int *) crypto_bignum_init();
	if (!my2)
		goto done;
	if (sp_init(t) != MP_OKAY)
		goto done;
	if (sp_init(sx) != MP_OKAY)
		goto done;
	if (sp_init(sea) != MP_OKAY)
		goto done;
	if (sp_init(seb) != MP_OKAY)
		goto done;
	if (sp_init(sep) != MP_OKAY)
		goto done;
	if (sp_init(y2) != MP_OKAY)
		goto done;
	if (mp_to_unsigned_bin((mp_int *) x, buf1) != MP_OKAY) {
		goto done;
	}
	unsigned_bin_length = bignum_get_unsigned_bin_len((struct bignum *)x);
	if ((sp_read_unsigned_bin(sx, buf1, unsigned_bin_length)) != MP_OKAY) {
		goto done;
	}

	if (mp_to_unsigned_bin(&e->a, buf1) != MP_OKAY) {
		goto done;
	}
	unsigned_bin_length = bignum_get_unsigned_bin_len(&e->a);
	if ((sp_read_unsigned_bin(sea, buf1, unsigned_bin_length)) != MP_OKAY) {
		goto done;
	}
	if (mp_to_unsigned_bin(&e->b, buf1) != MP_OKAY) {
		goto done;
	}
	unsigned_bin_length = bignum_get_unsigned_bin_len(&e->b);
	if ((sp_read_unsigned_bin(seb, buf1, unsigned_bin_length)) != MP_OKAY) {
		goto done;
	}
	if (mp_to_unsigned_bin(&e->prime, buf1) != MP_OKAY) {
		goto done;
	}
	unsigned_bin_length = bignum_get_unsigned_bin_len(&e->prime);
	if ((sp_read_unsigned_bin(sep, buf1, unsigned_bin_length)) != MP_OKAY) {
		goto done;
	}
	if (sp_sqrmod(sx, sep, y2) != 0 ||
	    sp_mulmod(sx, y2, sep, y2) != 0 ||
	    sp_mulmod(sx, sea, sep, t) != 0 ||
	    sp_addmod(y2, t, sep, y2) != 0 ||
	    sp_addmod(y2, seb, sep, y2) != 0
	...
	if (sp_to_unsigned_bin((sp_int *) &y2, buf1) != MP_OKAY) {
		err = -1;
		goto done;
	}
	unsigned_bin_length = sp_unsigned_bin_size((sp_int *)y2);
	if ((mp_read_unsigned_bin(my2, buf1, unsigned_bin_length)) != MP_OKAY) {
		err = -1;
		goto done;
	}
done:
	...
	sp_clear(seb);
	sp_clear(sep);
	return (struct crypto_bignum *) my2;
#else
	mp_int *y2 = NULL;
	mp_int t;
	int calced = 0;
...
#if !defined(ROM_IMAGE_20)	    
	    mp_addmod_non_rom(y2, &t, &e->prime, y2) != 0 ||
	    mp_addmod_non_rom(y2, &e->b, &e->prime, y2) != 0
#else
