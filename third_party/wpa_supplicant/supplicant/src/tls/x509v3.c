...
{
	char *buf;
	buf = os_malloc(80); /* K60_PORTING To decrease stack */
	if (buf == NULL)
	{
		return -1;
	}
	...
	asn1_oid_to_str(&name->rid, buf, 80);
				      ...
				      const u8 **next)
{
	...
	unsigned long value;
	const u8 *subject_dn;
	int retval = -1; /* Error by default as there are majority of return -1 statements */

	sbuf = os_malloc(128);
	if (sbuf == NULL) /* K60_PORTING to decrease stack */
	{
		return retval;
	}
	...
	retval = 0; /* Make it zero for success case */
label_ret_val:
	os_free(sbuf);
	return retval;
}
				    ...
				    int *reason, int disable_time_checks)
{
	...
	buf = os_malloc(128); /* K60_PORTING To decrease stack */
	if (buf == NULL)
	{
		return -1;
	}
	...
	*reason = X509_VALIDATE_OK;
	...
	for (cert = chain, idx = 0; cert; cert = cert->next, idx++) {
		...
		x509_name_string(&cert->subject, buf, 128);
