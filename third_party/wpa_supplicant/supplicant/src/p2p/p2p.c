...
void p2p_remove_device(struct p2p_data *p2p, uint8 *dev_add)
{
	...
	dl_list_for_each_safe(dev, n, &p2p->devices, struct p2p_device, list) {
		...
		if(os_memcmp(dev->info.p2p_device_addr, dev_add, 6))
			continue;

		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Removing client "
...
#else /* This else part is taken from suppl-2.o original*/ 
...
int p2p_other_scan_completed(struct p2p_data *p2p)
{
	...
	if (p2p_find(p2p, p2p->last_p2p_find_timeout, p2p->find_type,
		     p2p->num_req_dev_types, p2p->req_dev_types,
		     p2p->find_dev_id, p2p->search_delay, 0, NULL, 0) < 0)
		return 0;
	return 1;
}
