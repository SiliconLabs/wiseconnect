 ...
 */
#ifdef ENABLE_11AX
typedef struct mbssid_params_s{
	uint8 tx_bssid_B5;
	uint8 mbssid_index;
	uint8 full_set_rx_per;
	uint8 max_bssid_indicator;
}__attribute__((packed)) mbssid_params_t;
#endif
struct wpa_scan_res {
	unsigned int flags;
	int freq;
...
/**
...
#ifdef RSI_ENABLE_CCX
	/**
	 * During CCKM, MIC for the reassociation response is given via
	 * this IOCTL
	 *                            
	 */
	int (*set_reassoc) (void *priv, u8 *mic, u8 *rn);
#endif
	/**
	 ...
	 * @ret_val: Return value(s)
	 * Returns: void
	 *
	 * This callback is used to let the driver call supplicant interface functions corresponding to the command type.
	 */
...
enum wpa_event_type {
	 ...
	 */
	EVENT_FT_RESPONSE,
	EVENT_CHANNEL_NOISY, /*ROAMING Changes*/
...
#ifdef RSI_ENABLE_CCX
	/**
	 *  struct cckm_reassoc - CCKM Reassociation request element
	 *  During CCKM Reassociation sequence, the driver is expected
	 *  to give the BSSID/TSF information to the supplicant. The
	 *  CCKM Reassociation request's MIC will be delivered via a call back.
	 */
	struct cckm_reassoc {
		u8  bssid[ETH_ALEN];
		u8  tsf[8];
	} cckm_request;
	struct cckm_response {
		unsigned int rekey_number;
		u8  keyid;
		u8  keyid_mcast;
		u8  rsc[8];
		u8  gkey_len[2];	
		u8  mic[8];
		u8  gtk[40]; /* Check if this is sufficient? */
	} cckm_response;
#endif
