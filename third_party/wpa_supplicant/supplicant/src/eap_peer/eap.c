...
#else
	void sm_EAP_INITIALIZE_Enter(STATE_MACHINE_DATA *sm, int global)  
#endif
...
#else
void sm_EAP_FAILURE_Enter(STATE_MACHINE_DATA *sm, int global)  
#endif
{
...
#if UNUSED_FEAT_IN_SUPP_29
STATIC void eap_peer_sm_step_local_wrapper(struct eap_sm *sm) __attribute__((section(".obe_low_power_ram")));
STATIC void eap_peer_sm_step_local_wrapper(struct eap_sm *sm)
{
...
static void eap_sm_parseEapReq(struct eap_sm *sm, const struct wpabuf *req)
{
	...
	case EAP_CODE_FAILURE:
		...
		/* If failure reason is not identified yet */
		if (!reason_code_eap_failure)
		{
			reason_code_eap_failure = EAP_FAILURE_RECEIVED;
		}
#endif
...
#ifndef EXCLUDE_EAP
    //Initialize tls only when operating mode is eap
    if(supp_mgmt_if_ctx.is_eap) {
