...
#ifdef TSF_SYNC
void rsi_sta_connected_notification_to_host(u8 *mac_addr, u8 con_event_type);
#endif
#ifdef SUPPLICANT_PORTING
#ifdef CONFIG_P2P
void p2p_remove_device(struct p2p_data *p2p, uint8 *dev_add);
#endif
#endif
...
#if 0
#ifdef TSF_SYNC
  //! station disconnect event in AP mode only
  if(sta->p2p_ie ==  NULL)
    rsi_sta_connected_notification_to_host(sta->addr, 0);
#endif
#endif
...
skip_poll:
	...
	if (sta->timeout_next == STA_NULLFUNC &&
		...
		wpa_dbg(hapd->msg_ctx, MSG_DEBUG,
			...
			MAC2STR(sta->addr));
#endif
		SL_PRINTF(WLAN_SUPP_TIMEOUT, WLAN_UMAC, LOG_INFO);
...
void ap_sta_replenish_timeout(struct hostapd_data *hapd, struct sta_info *sta,
			      u32 session_timeout)
{
	/* This function is invoking from gas_serv.c file only which is not icluded in compilation */
...
struct sta_info * ap_sta_add(struct hostapd_data *hapd, const u8 *addr)
{
	...
	if (sta)
		return sta;
		//ap_sta_remove(hapd, sta); /* Fixme: for WPA3 AP mode
...
#ifdef SUPPLICANT_PORTING
int ap_sta_remove(struct hostapd_data *hapd, struct sta_info *sta)
#else	
static int ap_sta_remove(struct hostapd_data *hapd, struct sta_info *sta)
#endif
{
	...
	if (authorized) {
		...
		if (hapd->msg_ctx_parent &&
			...
			wpa_msg_no_global(hapd->msg_ctx_parent, MSG_INFO,
					  ...
					  buf, ip_addr, keyid_buf);
#ifdef TSF_SYNC //Supplicant porting for RS9113
      rsi_sta_connected_notification_to_host(sta->addr, 1);
#endif 
	} else {
...
#ifdef CONFIG_P2P
      if(hapd->p2p)
        p2p_remove_device(hapd->p2p, dev_addr);
#endif  
#endif  
	}
...
#endif
	SL_PRINTF(WLAN_SUPP_IEEE_802_1X_SCHEDULED_DISCONNECTION_OF_STA_AFTER_EAP_FAILURE, WLAN_UMAC, LOG_INFO);
...
#endif
	SL_PRINTF(WLAN_SUPP_IEEE_802_1X_FORCE_DISCONNECTION_OF_STA_AFTER_EAP_FAILURE_IN_10_MS, WLAN_UMAC, LOG_INFO);
