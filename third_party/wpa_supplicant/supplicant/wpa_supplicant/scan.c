...
void wpas_wps_update_ap_info_bss(struct wpa_supplicant *wpa_s, struct wpa_scan_res *res);
void wpas_wps_dump_ap_info(struct wpa_supplicant *wpa_s);
...
#endif
	SL_PRINTF(WLAN_SUPP_ALREADY_ASSOCIATED_WITH_A_CONFIGURED_NETWORK_GENERATING_ASSOCIATED_EVENT, WLAN_UMAC, LOG_INFO);
...
int wpa_supplicant_enabled_networks(struct wpa_supplicant *wpa_s)
{
	...
	if (count == 0 && disabled > 0) {
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "No enabled networks (%d disabled "
			"networks)", disabled);
#endif
		SL_PRINTF(WLAN_SUPP_NO_ENABLED_NETWORKS, WLAN_UMAC, LOG_INFO, "No enabled networks (%d disabled networks)", disabled);
	}
	return count;
}
...
#endif
		SL_PRINTF(WLAN_SUPP_WPA_SUPPLICANT_ASSOC_TRY_REACHED_END_OF_SCAN_LIST_GO_BACK_TO_BEGINNING, WLAN_UMAC, LOG_INFO);
...
#endif
		SL_PRINTF(WLAN_SUPP_REJECT_SCAN_TRIGGER_SINCE_ONE_IS_ALREADY_PENDING, WLAN_UMAC, LOG_INFO);
		return -1;
	}
...
#endif
	SL_PRINTF(WLAN_SUPP_STARTING_DELAYED_SCHED_SCAN, WLAN_UMAC, LOG_INFO);
...
#endif
	SL_PRINTF(WLAN_SUPP_SCHED_SCAN_TIMEOUT_STOPPING_IT, WLAN_UMAC, LOG_INFO);
...
#endif
		SL_PRINTF(WLAN_SUPP_STOPPING_SCHED_SCAN_FAILED, WLAN_UMAC, LOG_INFO);
	...
	struct wpa_supplicant *wpa_s, struct wpa_driver_scan_params *params)
{
	...
	if (params->freqs == NULL && wpa_s->p2p_in_provisioning &&
		...
		if (wpa_s->p2p_in_provisioning < 5 &&
...
#endif
			SL_PRINTF(WLAN_SUPP_P2P_SCAN_ONLY_GO_PREFERRED_FREQUENCY_2, WLAN_UMAC, LOG_INFO, "preferred frequency %d MHz", wpa_s->go_params->freq);
			...
			wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Scan only common "
				"channels");
#endif
			SL_PRINTF(WLAN_SUPP_P2P_SCAN_ONLY_COMMON_CHANNELS, WLAN_UMAC, LOG_INFO);
...
#endif
			SL_PRINTF(WLAN_SUPP_P2P_SCAN_ONLY_GO_PREFERRED_FREQUENCY, WLAN_UMAC, LOG_INFO, "pref frequency %d MHz during invitation", wpa_s->p2p_invite_go_freq);
			params->freqs = os_calloc(2, sizeof(int));
			if (params->freqs)
...
#endif
			SL_PRINTF(WLAN_SUPP_P2P_CLEAR_P2P_IN_INVITATION, WLAN_UMAC, LOG_INFO);
...
#endif
		SL_PRINTF(WLAN_SUPP_WPS_SCAN_ONLY_FREQUENCY_PROVISIONING, WLAN_UMAC, LOG_INFO, "WPS: Scan only frequency %d MHz that was used during provisioning", wpa_s->wps_freq);
		params->freqs = os_calloc(2, sizeof(int));
		if (params->freqs)
	...
	if (params->freqs == NULL && wpa_s->known_wps_freq && wpa_s->wps_freq)
	{
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "WPS: Scan only frequency %u MHz",
			wpa_s->wps_freq);
#endif
		SL_PRINTF(WLAN_SUPP_WPS_SCAN_ONLY_FREQUENCY, WLAN_UMAC, LOG_INFO, "WPS: Scan only frequency %d MHz", wpa_s->wps_freq);
		params->freqs = os_calloc(2, sizeof(int));
		if (params->freqs)
			params->freqs[0] = wpa_s->wps_freq;
...
#endif
		SL_PRINTF(WLAN_SUPP_SKIP_SCAN_INTERFACE_DISABLED, WLAN_UMAC, LOG_INFO);
		return;
	}
...
#endif
		SL_PRINTF(WLAN_SUPP_DISCONNECTED_DO_NOT_SCAN, WLAN_UMAC, LOG_INFO);
...
#endif
		SL_PRINTF(WLAN_SUPP_ALREADY_SCANNING_RESCHEDULE_THE_INCOMING_SCAN_REQ, WLAN_UMAC, LOG_INFO);
		wpa_supplicant_req_scan(wpa_s, 1, 0);
		return;
	}
#endif
...
#endif
		SL_PRINTF(WLAN_SUPP_NO_ENABLED_NETWORKS_DO_NOT_SCAN, WLAN_UMAC, LOG_INFO);
...
#endif
		SL_PRINTF(WLAN_SUPP_USING_WIRED_AUTHENTICATION_OVERRIDING_AP_SCAN_CONFIGURATION, WLAN_UMAC, LOG_INFO);
...
#endif
		SL_PRINTF(WLAN_SUPP_DELAY_STATION_MODE_SCAN_WHILE_P2P_OPERATION_IS_IN_PROGRESS, WLAN_UMAC, LOG_INFO);
...
#endif
		SL_PRINTF(WLAN_SUPP_DEFER_SCAN_PNO_IS_IN_PROGRESS, WLAN_UMAC, LOG_INFO);
	...
	if (ssid && max_ssids == 1) {
		...
		if (!wpa_s->prev_scan_wildcard) {
...
#endif
			SL_PRINTF(WLAN_SUPP_STARTING_AP_SCAN_FOR_WILDCARD_SSID_INTERLEAVE_WITH_SPECIFIC_WILDCARD_SSID, WLAN_UMAC, LOG_INFO);
		} else {
			wpa_s->prev_scan_ssid = ssid;
			wpa_s->prev_scan_wildcard = 0;
#if UNUSED_FEAT_IN_SUPP_29
			wpa_dbg(wpa_s, MSG_DEBUG,
				...
				wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
#endif
			SL_PRINTF(WLAN_SUPP_STARTING_AP_SCAN_FOR_SPECIFIC_SSID, WLAN_UMAC, LOG_INFO, "SSID: %s", wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
		}
	} else if (ssid) {
		/* max_ssids > 1 */
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "Include wildcard SSID in "
			"the scan request");
#endif
		SL_PRINTF(WLAN_SUPP_INCLUDE_WILDCARD_SSID_IN_THE_SCAN_REQUEST, WLAN_UMAC, LOG_INFO);
		params.num_ssids++;
...
#endif
		SL_PRINTF(WLAN_SUPP_USE_PASSIVE_SCAN_BASED_ON_MANUAL_REQUEST, WLAN_UMAC, LOG_INFO);
	} else if (wpa_s->conf->passive_scan) {
#if UNUSED_FEAT_IN_SUPP_29
		wpa_dbg(wpa_s, MSG_DEBUG,
			"Use passive scan based on configuration");
#endif
		SL_PRINTF(WLAN_SUPP_USE_PASSIVE_SCAN_BASED_ON_CONFIGURATION, WLAN_UMAC, LOG_INFO);
	} else {
		wpa_s->prev_scan_ssid = WILDCARD_SSID_SCAN;
		params.num_ssids++;
#if UNUSED_FEAT_IN_SUPP_29
		wpa_dbg(wpa_s, MSG_DEBUG, "Starting AP scan for wildcard "
			"SSID");
#endif
		SL_PRINTF(WLAN_SUPP_STARTING_AP_SCAN_FOR_WILDCARD_SSID, WLAN_UMAC, LOG_INFO);
	}
...
#endif
		SL_PRINTF(WLAN_SUPP_LIMIT_MANUAL_SCAN_TO_SPECIFIED_CHANNELS, WLAN_UMAC, LOG_INFO);
...
#endif
		SL_PRINTF(WLAN_SUPP_LIMIT_SELECT_NETWORK_SCAN_TO_SPECIFIED_CHANNELS, WLAN_UMAC, LOG_INFO);
...
#endif
		SL_PRINTF(WLAN_SUPP_OPTIMIZE_SCAN_BASED_ON_PREVIOUSLY_GENERATED_FREQUENCY_LIST, WLAN_UMAC, LOG_INFO);
...
#endif
		SL_PRINTF(WLAN_SUPP_OPTIMIZE_SCAN_BASED_ON_CONF_FREQ_LIST_2, WLAN_UMAC, LOG_INFO);
...
#endif
				SL_PRINTF(WLAN_SUPP_SCAN_ONLY_THE_CURRENT_OPERATING_CHANNELS_SINCE_SCAN_CUR_FREQ_IS_ENABLED, WLAN_UMAC, LOG_INFO);
			} else {
				os_free(params.freqs);
				params.freqs = NULL;
			}
...
#endif
			SL_PRINTF(WLAN_SUPP_SCAN_A_PREVIOUSLY_SPECIFIED_BSSID_2, WLAN_UMAC, LOG_INFO);
		} else {
#if UNUSED_FEAT_IN_SUPP_29
			wpa_dbg(wpa_s, MSG_DEBUG,
				"Scan a previously specified BSSID " MACSTR,
				MAC2STR(params.bssid));
#endif
			SL_PRINTF(WLAN_SUPP_SCAN_A_PREVIOUSLY_SPECIFIED_BSSID, WLAN_UMAC, LOG_INFO);
		}
	}
...
#endif
				SL_PRINTF(WLAN_SUPP_SCAN_ONLY_THE_CURRENT_OPERATING_CHANNELS_SINCE_ALL_CHANNELS_ARE_ALREADY_USED, WLAN_UMAC, LOG_INFO);
			} else {
				os_free(params.freqs);
				params.freqs = NULL;
			}
...
void wpa_supplicant_req_scan(struct wpa_supplicant *wpa_s, int sec, int usec)
{
	...
	if (wpa_s->p2p_mgmt) {
		...
		wpa_dbg(wpa_s, MSG_DEBUG,
			...
			sec, usec);
#endif
		SL_PRINTF(WLAN_SUPP_IGNORE_SCAN_REQUEST, WLAN_UMAC, LOG_INFO, "Ignore scan request (%d.%d sec) on p2p_mgmt interface", sec, usec);
		return;
	}
	...
	if (res == 1) {
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "Rescheduling scan request: %d.%06d sec",
			sec, usec);
#endif
		SL_PRINTF(WLAN_SUPP_RESCHEDULING_SCAN_REQUEST, WLAN_UMAC, LOG_INFO, "Rescheduling scan request: %d.%d sec", sec, usec);
	} else if (res == 0) {
#if UNUSED_FEAT_IN_SUPP_29
		wpa_dbg(wpa_s, MSG_DEBUG, "Ignore new scan request for %d.%06d sec since an earlier request is scheduled to trigger sooner",
			sec, usec);
#endif
		SL_PRINTF(WLAN_SUPP_IGNORE_NEW_SCAN_REQUEST_FOR, WLAN_UMAC, LOG_INFO, "Ignore new scan request for %d.%d sec since an earlier request is scheduled to trigger sooner", sec, usec);
	} else {
#if UNUSED_FEAT_IN_SUPP_29
		wpa_dbg(wpa_s, MSG_DEBUG, "Setting scan request: %d.%06d sec",
			sec, usec);
#endif
		SL_PRINTF(WLAN_SUPP_SETTING_SCAN_REQUEST_2, WLAN_UMAC, LOG_INFO, "Setting scan request: %d.%d sec", sec, usec);
...
if (eloop_is_timeout_registered(wpa_supplicant_scan, wpa_s, NULL) &&
...
#endif
			SL_PRINTF(WLAN_SUPP_NOT_RESCHEDULING_SCAN_TO_ENSURE_THAT_SPECIFIC_SSID_SCANS_OCCUR, WLAN_UMAC, LOG_INFO);
			return;
		}
	}
	...
	wpa_dbg(wpa_s, MSG_DEBUG, "Setting scan request: %d sec %d usec",
		sec, usec);
#endif
	SL_PRINTF(WLAN_SUPP_SETTING_SCAN_REQUEST, WLAN_UMAC, LOG_INFO, "Setting scan request: %d sec %d usec", sec, usec);
...
int wpa_supplicant_req_sched_scan(struct wpa_supplicant *wpa_s)
{
...
#endif
		SL_PRINTF(WLAN_SUPP_ALREADY_SCHED_SCANNING, WLAN_UMAC, LOG_INFO);
		return 0;
	}
	...
	for (ssid = wpa_s->conf->ssid; ssid; ssid = ssid->next) {
		...
		if (!wpas_network_disabled(wpa_s, ssid) &&
			...
			wpa_dbg(wpa_s, MSG_DEBUG, "Use normal scan instead of "
				"sched_scan for WPS");
#endif
			SL_PRINTF(WLAN_SUPP_USE_NORMAL_SCAN_INSTEAD_OF_SCHED_SCAN_FOR_WPS, WLAN_UMAC, LOG_INFO);
			return -1;
		}
#endif /* CONFIG_WPS */
	}
	...
	if (wpa_s->normal_scans < 3 &&
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "Use normal scan instead of "
			...
			wpa_s->normal_scans);
#endif
		SL_PRINTF(WLAN_SUPP_USE_NORMAL_SCAN_INSTEAD_OF_SCHED_SCAN_FOR_INITIAL_SCANS, WLAN_UMAC, LOG_INFO, "(normal_scans=%d)", wpa_s->normal_scans);
		return -1;
	}
...
#endif
		SL_PRINTF(WLAN_SUPP_BEGINNING_OF_SSID_LIST, WLAN_UMAC, LOG_INFO);
...
#endif
		SL_PRINTF(WLAN_SUPP_ADD_WILDCARD_SSID_TO_SCHED_SCAN, WLAN_UMAC, LOG_INFO);
		params.num_ssids++;
	}
	...
	while (ssid) {
		...
		if (params.num_filter_ssids < wpa_s->max_match_sets &&
			...
			wpa_dbg(wpa_s, MSG_DEBUG, "add to filter ssid: %s",
				wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
#endif
			SL_PRINTF(WLAN_SUPP_ADD_TO_FILTER_SSID, WLAN_UMAC, LOG_INFO, "ssid: %s", wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
...
#endif
			SL_PRINTF(WLAN_SUPP_NOT_ENOUGH_ROOM_FOR_SSID_FILTER_FOR_SCHED_SCAN_DROP_FILTER, WLAN_UMAC, LOG_INFO);
			os_free(params.filter_ssids);
		...
		if (ssid->scan_ssid && ssid->ssid && ssid->ssid_len) {
			...
			wpa_dbg(wpa_s, MSG_DEBUG,
				...
				wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
#endif
			SL_PRINTF(WLAN_SUPP_ADD_TO_ACTIVE_SCAN_SSID, WLAN_UMAC, LOG_INFO, "ssid: %s", wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
			params.ssids[params.num_ssids].ssid =
				ssid->ssid;
			params.ssids[params.num_ssids].ssid_len =
				ssid->ssid_len;
	...
	if (wpa_s->conf->freq_list && !params.freqs) {
		...
		wpa_dbg(wpa_s, MSG_DEBUG,
			"Optimize scan based on conf->freq_list");
#endif
		SL_PRINTF(WLAN_SUPP_OPTIMIZE_SCAN_BASED_ON_CONF_FREQ_LIST, WLAN_UMAC, LOG_INFO);
		int_array_concat(&params.freqs, wpa_s->conf->freq_list);
	}
...
#endif
		SL_PRINTF(WLAN_SUPP_TOO_MANY_SSIDS_DEFAULT_TO_USING_SINGLE_SCHEDULED_SCAN_PLAN, WLAN_UMAC, LOG_INFO);
...
#endif
		SL_PRINTF(WLAN_SUPP_STARTING_SCHED_SCAN_AFTER, WLAN_UMAC, LOG_INFO, "%d seconds: interval %d timeout %d", params.sched_scan_start_delay, params.sched_scan_plans[0].interval, wpa_s->sched_scan_timeout);
	} else {
#if UNUSED_FEAT_IN_SUPP_29
		wpa_dbg(wpa_s, MSG_DEBUG,
			...
			params.sched_scan_start_delay);
#endif
		SL_PRINTF(WLAN_SUPP_STARTING_SCHED_SCAN_AFTER_NO_TIMEOUT, WLAN_UMAC, LOG_INFO, "%d seconds", params.sched_scan_start_delay);
	}
...
#endif
	SL_PRINTF(WLAN_SUPP_CANCELLING_SCAN_REQUEST, WLAN_UMAC, LOG_INFO);
	eloop_cancel_timeout(wpa_supplicant_scan, wpa_s, NULL);
}
...
#endif
	SL_PRINTF(WLAN_SUPP_CANCELLING_DELAYED_SCHED_SCAN, WLAN_UMAC, LOG_INFO);
...
#endif
	SL_PRINTF(WLAN_SUPP_CANCELLING_SCHED_SCAN, WLAN_UMAC, LOG_INFO);
...
#ifdef ENABLE_11AX
		ie = get_ie((uint8 *)(res + 1), res->ie_len, WLAN_EID_HT_CAP);
#else
...
#ifdef ENABLE_11AX
		ie = get_ie((uint8 *)(res + 1), res->ie_len, WLAN_EID_HT_OPERATION);
#else
...
#ifdef ENABLE_11AX
		ie = get_ie((uint8 *)(res + 1), res->ie_len, WLAN_EID_VHT_CAP);
#else
...
#ifdef ENABLE_11AX
			ie = get_ie((uint8 *)(res + 1), res->ie_len, WLAN_EID_VHT_OPERATION);
#else
				...
				struct scan_info *info, int new_scan)
{
...
#endif
		SL_PRINTF(WLAN_SUPP_FAILED_TO_GET_SCAN_RESULTS, WLAN_UMAC, LOG_INFO);
		return NULL;
	}
	...
	if (wpas_wps_searching(wpa_s)) {
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "WPS: Order scan results with WPS "
			"provisioning rules");
#endif
		SL_PRINTF(WLAN_SUPP_WPS_ORDER_SCAN_RESULTS_WITH_WPS_PROVISIONING_RULES, WLAN_UMAC, LOG_INFO);
...
#endif
		SL_PRINTF(WLAN_SUPP_DO_NOT_UPDATE_BSS_TABLE_BASED_ON_PENDING_POST_FLUSH_SCAN_RESULTS, WLAN_UMAC, LOG_INFO);
		wpa_s->ignore_post_flush_scan_res = 0;
		return scan_res;
	}
...
#endif
	SL_PRINTF(WLAN_SUPP_SCAN_ONLY_RESULTS_RECEIVED, WLAN_UMAC, LOG_INFO);
	if (wpa_s->last_scan_req == MANUAL_SCAN_REQ &&
...
#endif
		SL_PRINTF(WLAN_SUPP_PNO_TOO_MANY_SSIDS_TO_MATCH, WLAN_UMAC, LOG_INFO);
	}
...
#endif
		SL_PRINTF(WLAN_SUPP_LIMIT_SCHED_SCAN_TO_SPECIFIED_CHANNELS, WLAN_UMAC, LOG_INFO);
...
#endif
		SL_PRINTF(WLAN_SUPP_ABORT_AN_ONGOING_SCAN, WLAN_UMAC, LOG_INFO);
...
#endif
	SL_PRINTF(WLAN_SUPP_NO_ONGOING_SCAN_P2P_SCAN_FOUND_TO_ABORT, WLAN_UMAC, LOG_INFO);
	return -1;
}
#endif
