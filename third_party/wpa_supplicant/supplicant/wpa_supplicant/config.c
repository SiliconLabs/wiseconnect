				...
				const char *value)
{
	...
	if (*value == '"') {
		...
		/*Check if PMK is present no need to genearate*/
		if(!is_pmk_present()) {
			if(!is_wpa3_mode()) {
				wpa_config_update_psk(ssid);
				wise_store_pmk((char *)ssid->psk);
			} else {
				os_memcpy(ssid->psk, wise_get_pmk(), PMK_LEN);
				ssid->psk_set = 1;
			}
		} else {
			ssid->psk_set = 1;
				     ...
				     const char *value)
{
	...
	while (*start != '\0') {
		...
		*end = '\0';
		...
		else if (os_strcmp(start, "WPA-CCKM") == 0)
					...
					struct wpa_ssid *ssid)
{
	...
	if (ssid->key_mgmt & WPA_KEY_MGMT_CCKM) {
		ret = os_snprintf(pos, end - pos, "%sWPA-CCKM",
				pos == buf ? "" : " ");
...
static int * wpa_config_parse_int_array(const char *value)
{
	...
	while (pos) {
		...
		freqs[used] = sl_atoi((uint8_t *)pos);
		if (freqs[used] == 0)
			break;
		used++;
		pos = os_strchr(pos + 1, ' ');
...
#else
  nlist = os_malloc(sizeof(struct wpa_ssid *));
#endif
	if (nlist == NULL)
		return -1;
...
/**
 * This function is used to generate PMK from PSK and SSID
 * input parameters 
 * a)passphrase PSK
 * b)SSID
 * c)Length of SSID
...
{
	pbkdf2_sha1((const char *)passphrase, ssid, ssid_len, 4096,
		    pmk, PMK_LEN);
}
					   ...
					   const char *driver_param)
{
    ...
    config->roam_hysteresis = 2;
	config->roam_threshold = 5;
#ifdef CONFIG_MBO_STA
...
#endif
return 0; // ADDED TO HANDLE MACRO
