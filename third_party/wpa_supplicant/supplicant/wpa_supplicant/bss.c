...
#endif
	SL_PRINTF(WLAN_SUPP_BSS_REMOVE_ID, WLAN_UMAC, LOG_INFO, "BSS: Remove id %d BSSID", bss->id);
...
STATIC int wpa_bss_in_use(struct wpa_supplicant *wpa_s, struct wpa_bss *bss) _OBE_LOW_POWER_RAM_
				    ...
				    struct os_reltime *fetch_time)
{
...
#ifdef ENABLE_11AX
      sl_memcpy((uint8 *)&bss->mbssid_params,(uint8 *)&res->mbssid_params,sizeof(mbssid_params_t)); 
#endif
	...
	wpa_dbg(wpa_s, MSG_DEBUG, "BSS: Add new id %u BSSID " MACSTR
		...
		bss->freq, extra);
#endif
	SL_PRINTF(WLAN_SUPP_BSS_ADD_NEW_ID, WLAN_UMAC, LOG_INFO, "BSS: Add new id %d", bss->id);
...
#ifdef ENABLE_11AX
		new_ie = get_ie((uint8 *)(new_res + 1), new_res->ie_len, ie);
#else
	       ...
	       struct wpa_scan_res *res, struct os_reltime *fetch_time)
{
	...
	if (wpa_bss_get_vendor_ie(bss, P2P_IE_VENDOR_TYPE) &&
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "BSS: Do not update scan IEs for "
			...
			MAC2STR(bss->bssid));
#endif
		SL_PRINTF(WLAN_SUPP_BSS_DO_NOT_UPDATE_SCAN_IES_SINCE_THAT_WOULD_REMOVE_P2P_IE_INFORMATION, WLAN_UMAC, LOG_INFO);
	} else
#endif /* CONFIG_P2P */
...
void wpa_bss_update_start(struct wpa_supplicant *wpa_s)
{
	...
	wpa_dbg(wpa_s, MSG_DEBUG, "BSS: Start scan result update %u",
		wpa_s->bss_update_idx);
#endif
	SL_PRINTF(WLAN_SUPP_BSS_START_SCAN_RESULT_UPDATE, WLAN_UMAC, LOG_INFO, "BSS: Start scan result update %d", wpa_s->bss_update_idx);
			     ...
			     struct os_reltime *fetch_time)
{
...
#endif
			SL_PRINTF(WLAN_SUPP_BSS_IGNORE_OLD_DRIVER_BSS_TABLE_ENTRY, WLAN_UMAC, LOG_INFO, "BSS: Ignore driver BSS table entry that is %d.%d seconds older than our scan trigger", (int) age.sec, (int) age.usec);
			return;
		}
	}

#ifdef ENABLE_11AX
	ssid = get_ie((u8 *)(res + 1), res->ie_len,  WLAN_EID_SSID);
#else
...
#endif
		SL_PRINTF(WLAN_SUPP_BSS_NO_SSID_IE_INCLUDED, WLAN_UMAC, LOG_INFO);
		return;
	}
	if (ssid[1] > SSID_MAX_LEN) {
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "BSS: Too long SSID IE included for "
			MACSTR, MAC2STR(res->bssid));
#endif
		SL_PRINTF(WLAN_SUPP_BSS_TOO_LONG_SSID_IE_INCLUDED, WLAN_UMAC, LOG_INFO);
		return;
	}
...
#ifdef ENABLE_11AX
	mesh = get_ie((uint8 *)(res + 1), res->ie_len, WLAN_EID_MESH_ID);
#else
	...
	if (wpa_s->last_scan_res_used >= wpa_s->last_scan_res_size) {
		...
		n = os_realloc_array(wpa_s->last_scan_res, siz,
				     sizeof(struct wpa_bss *));
#else
    n = os_malloc(sizeof(struct wpa_bss *));
#endif
		if (n == NULL)
    {
      SLI_WLAN_MGMT_ASSERT(UMAC_ASSERT_OS_MALLOC_FAILED_IN_UPDATE_SCAN_RES);
			return;
    }
...
int dl_list_empty_wrapper(struct dl_list *list) _OBE_LOW_POWER_RAM_
int dl_list_empty_wrapper(struct dl_list *list)
{
 ...
 */
void wpa_bss_flush_by_age(struct wpa_supplicant *wpa_s, int age) _OBE_LOW_POWER_RAM_ 
void wpa_bss_flush_by_age(struct wpa_supplicant *wpa_s, int age)
{
	struct wpa_bss *bss, *n;
	struct os_reltime t;

	if (dl_list_empty_wrapper(&wpa_s->bss))
		return;
	...
	dl_list_for_each_safe(bss, n, &wpa_s->bss, struct wpa_bss, list) {
		...
		if (os_time_before_ram(&bss->last_update, &t)) {
			wpa_bss_remove(wpa_s, bss, __func__);
		} else
			break;
	}
