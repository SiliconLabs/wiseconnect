...
void wpas_wps_timeout(void *eloop_ctx, void *timeout_ctx);
...
#endif
		SL_PRINTF(WLAN_SUPP_WPS_EVENT_FAIL, WLAN_UMAC, LOG_INFO);
				   ...
				   const struct wps_credential *cred)
{
...
#endif
			SL_PRINTF(WLAN_SUPP_EVENT_CRED_RECEIVED_ATTR, WLAN_UMAC, LOG_INFO, "%s%s", WPS_EVENT_CRED_RECEIVED, buf);
			os_free(buf);
		}
...
#endif
		SL_PRINTF(WLAN_SUPP_WPS_EVENT_CRED_RECEIVED, WLAN_UMAC, LOG_INFO);

	wpa_hexdump_key(MSG_DEBUG, "WPS: Received Credential attribute",
			cred->cred_attr, cred->cred_attr_len);
	
	supp_mgmt_if_ctx.update_psk_input.key = (unsigned char*)cred->key; 
	supp_mgmt_if_ctx.update_psk_input.key_len = cred->key_len; 
	sl_wpa_driver_wrapper(wpa_s, WPA_DRV_UPDATE_PSK, NULL);
	...
	ssid->ssid = os_zalloc(cred->ssid_len + 1);
#else
	...
	case WPS_AUTH_OPEN:
		...
		if (registrar) {
			...
			wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_OPEN_NETWORK
				...
				ssid->id, ssid->id);
#endif
			SL_PRINTF(WLAN_SUPP_WPS_EVENT_OPEN_NETWORK_ID, WLAN_UMAC, LOG_INFO, "id=%d", ssid->id);
			ssid->disabled = 1;
		}
#endif /* CONFIG_WPS_REG_DISABLE_OPEN */
		break;
					 ...
					 struct wps_event_m2d *m2d)
{
...
#endif
	SL_PRINTF(WLAN_SUPP_WPS_EVENT_M2D_DEV_PASSWORD_ID_2, WLAN_UMAC, LOG_INFO, "dev_password_id=%d config_error=%d", m2d->dev_password_id, m2d->config_error);
	...
	if (wpa_s->p2pdev && wpa_s->p2pdev != wpa_s) {
		...
		wpa_msg(wpa_s->p2pdev, MSG_INFO, WPS_EVENT_M2D
			...
			m2d->dev_password_id, m2d->config_error);
#endif
		SL_PRINTF(WLAN_SUPP_WPS_EVENT_M2D_DEV_PASSWORD_ID, WLAN_UMAC, LOG_INFO, "dev_password_id=%d config_error=%d", m2d->dev_password_id, m2d->config_error);
	}
...
#endif
		SL_PRINTF(WLAN_SUPP_WPS_EVENT_FAIL_MSG_REASON_2, WLAN_UMAC, LOG_INFO, "msg=%d config_error=%d reason=%d", fail->msg, fail->config_error, fail->error_indication);
		if (wpa_s->p2pdev && wpa_s->p2pdev != wpa_s)
#if UNUSED_FEAT_IN_SUPP_29
			wpa_msg(wpa_s->p2pdev, MSG_INFO, WPS_EVENT_FAIL
				...
				wps_ei_str(fail->error_indication));
#endif
			SL_PRINTF(WLAN_SUPP_WPS_EVENT_FAIL_MSG_REASON, WLAN_UMAC, LOG_INFO, "msg=%d config_error=%d reason=%d", fail->msg, fail->config_error, fail->error_indication);
	} else {
#if UNUSED_FEAT_IN_SUPP_29
		wpa_msg(wpa_s, MSG_INFO,
			...
			fail->msg, fail->config_error);
#endif
		SL_PRINTF(WLAN_SUPP_WPS_EVENT_FAIL_MSG_2, WLAN_UMAC, LOG_INFO, "msg=%d config_error=%d", fail->msg, fail->config_error);
		if (wpa_s->p2pdev && wpa_s->p2pdev != wpa_s)
#if UNUSED_FEAT_IN_SUPP_29
			wpa_msg(wpa_s->p2pdev, MSG_INFO, WPS_EVENT_FAIL
				...
				fail->msg, fail->config_error);
#endif
			SL_PRINTF(WLAN_SUPP_WPS_EVENT_FAIL_MSG, WLAN_UMAC, LOG_INFO, "msg=%d config_error=%d", fail->msg, fail->config_error);
	}
...
#endif
	SL_PRINTF(WLAN_SUPP_WPS_EVENT_SUCCESS, WLAN_UMAC, LOG_INFO);
	wpa_s->wps_success = 1;
...
#endif
	SL_PRINTF(WLAN_SUPP_WPS_EVENT_ER_AP_REMOVE, WLAN_UMAC, LOG_INFO, "%s", uuid_str);
}
	...
	struct wpa_supplicant *wpa_s, struct wps_event_er_enrollee *enrollee)
{
	...
	SL_PRINTF(WLAN_SUPP_WPS_EVENT_ER_ENROLLEE_REMOVE, WLAN_UMAC, LOG_INFO, "%s", uuid_str);
}
...
#endif
		SL_PRINTF(WLAN_SUPP_WPS_EVENT_ER_SET_SEL_REG_START, WLAN_UMAC, LOG_INFO, "uuid=%s sel_reg=%d dev_passwd_id=%d", uuid_str, ev->sel_reg, ev->dev_passwd_id);
		break;
...
#endif
		SL_PRINTF(WLAN_SUPP_WPS_EVENT_ER_SET_SEL_REG_DONE, WLAN_UMAC, LOG_INFO, "uuid=%s", uuid_str);
		break;
...
#endif
		SL_PRINTF(WLAN_SUPP_WPS_EVENT_ER_SET_SEL_REG_FAILED, WLAN_UMAC, LOG_INFO, "uuid=%s", uuid_str);
		break;
	}
}
#endif
...
#endif
		SL_PRINTF(WLAN_SUPP_WPS_EVENT_ACTIVE, WLAN_UMAC, LOG_INFO);
		break;
	case WPS_EV_PBC_DISABLE:
#if UNUSED_FEAT_IN_SUPP_29
		wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_DISABLE);
#endif
		SL_PRINTF(WLAN_SUPP_WPS_EVENT_DISABLE, WLAN_UMAC, LOG_INFO);
		break;
#ifdef CONFIG_WPS_ER		
...
void wpas_wps_timeout(void *eloop_ctx, void *timeout_ctx)
{
	struct wpa_supplicant *wpa_s = eloop_ctx;
	union wps_event_data data;
	uint32 state = wpa_s->wpa_state ;
#if UNUSED_FEAT_IN_SUPP_29
	wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_TIMEOUT "Requested operation timed "
		"out");
#endif
	SL_PRINTF(WLAN_SUPP_WPS_EVENT_TIMEOUT_REQUESTED_OPERATION_TIMED_OUT, WLAN_UMAC, LOG_INFO);
	...
	wpas_clear_wps(wpa_s);
	/*FIXME change state checking to <WPA_COMPLETED after changing sme rejoin start*/
  /*Send this error from here in case of dual band max scan retries will not happen
   * before that this timeout(2 minutes) will happen because no of channels we have to scan increases in case of dual band*/
  if(state == WPA_SCANNING)
    sl_wpa_driver_wrapper(wpa_s, WPA_DRV_TIMEOUT_SEND_ERROR_TO_HOST, NULL);
}
				   ...
				   const struct wps_device_data *dev)
{
	...
	len = os_snprintf(txt, 400, "WPS-EVENT-PIN-NEEDED %s " MACSTR
			  " [%s|%s|%s|%s|%s|%s]",
...
#endif
	SL_PRINTF(WLAN_SUPP_WPS_UUID, WLAN, LOG_INFO, "WPS: UUID %s: %s", src, buf);
}
...
int wpas_wps_init(struct wpa_supplicant *wpa_s)
{
	...
	if (wps->dev.rf_bands == 0) {
		 ...
		 */
        sl_wpa_driver_wrapper(wpa_s, WPA_DRV_CHK_BAND_2_4GHZ, (int *)&is_band_24ghz);
		  if(is_band_24ghz) {
            wps->dev.rf_bands = WPS_RF_24GHZ;
        }
    else 
	...
	for (i = 0; i < wpa_s->num_wps_ap; i++) {
		...
		if (sel_uuid == NULL ||
			...
			wpa_msg(wpa_s, MSG_INFO, "WPS: PBC overlap detected: "
				...
				MAC2STR(ap->bssid));
#endif
			SL_PRINTF(WLAN_SUPP_WPS_PBC_OVERLAP_DETECTED, WLAN_UMAC, LOG_INFO);
			break;
		}
...
STATIC void wpas_wps_dump_ap_info(struct wpa_supplicant *wpa_s)
{
	...
	os_get_reltime(&now);
#ifdef ENABLE_SUPPLICANT_PRINTS
#ifdef DBG_PRINT_OBE
  size_t i;
	for (i = 0; i < wpa_s->num_wps_ap; i++) {
		struct wps_ap_info *ap = &wpa_s->wps_ap[i];
...
void wpas_wps_update_ap_info_bss(struct wpa_supplicant *wpa_s,
