			       ...
			       int after_new_scan)
{
	...
	supp_mgmt_if_ctx.roam_bssid_ind_update_input.roam_bssid = bss->bssid;
...
#ifdef ENABLE_11AX
			ssid_ie = get_ie((uint8 *)(res + 1), res->ie_len, WLAN_EID_SSID);
#else
...
#if defined(CONFIG_MBO_STA) && defined(CONFIG_TESTING_OPTIONS)
...
#endif /* CONFIG_MBO_STA && CONFIG_TESTING_OPTIONS */
...
#ifdef SUPPLICANT_PORTING
		supp_mgmt_if_ctx.trigger_bg_scan = 1;
		sl_wpa_driver_wrapper(wpa_s, WPA_DRV_TRIGGER_BG_SCAN, NULL);
#endif
