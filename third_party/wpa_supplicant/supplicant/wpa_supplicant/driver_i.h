...
{
	if(params->status == WLAN_STATUS_FINITE_CYCLIC_GROUP_NOT_SUPPORTED || params->status == WLAN_STATUS_UNKNOWN_AUTH_TRANSACTION || params->status == WLAN_STATUS_UNSPECIFIED_FAILURE){
        wpa_supplicant_deauthenticate(wpa_s, params->status);
		return 0;
	}
...
static inline void sl_wpa_driver_wrapper(struct wpa_supplicant *wpa_s, unsigned int cmd, int *ret_val)
{
	if(wpa_s == NULL)
		return;
	if (wpa_s->driver->sl_wpa_driver)
	  return wpa_s->driver->sl_wpa_driver(wpa_s, cmd, ret_val);
}
#endif /* DRIVER_I_H */
