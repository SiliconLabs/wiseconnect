...
#define MAX_SCAN_ID 16
	...
	int auth_alg;
	u16 last_owe_group;
	/*ROAMING Change---start*/
		  struct channel_info
		{
			u8	 avoidtime;
			char status; 
			//channel_status[MAX_CHANNELS_5GHZ + MAX_CHANNELS_2_4GHZ];
		}channel_status[25 + 14];
			u8 channel_timeout;
	/*****ROAMIN-change end*****************/
...
#define MAC_ADDR_RAND_ALL        (MAC_ADDR_RAND_SCAN | \
	...
	/*
	 * This should be under CONFIG_MBO_STA, but it is left out to allow using
...
void sme_external_auth_mgmt_rx(struct wpa_supplicant *wpa_s, const u8 *auth_frame, size_t len);
