...
fail:
	...
	bands = wpas_get_bands(wpa_s, params.freqs);  // addeed during 2.8 porting
...
STATIC void wpas_go_neg_completed(void *ctx, struct p2p_go_neg_results *res)
{
	...
	eloop_register_timeout(150 + res->peer_config_timeout / 100,
			   ...
			   int new_device)
{
	...
	if (new_device) { /* K60_PORTING */
		supp_mgmt_if_ctx.notify_p2p_dev_input.new_device = 1;
		supp_mgmt_if_ctx.notify_p2p_dev_input.device_name = (u8 *)info->device_name;
		supp_mgmt_if_ctx.notify_p2p_dev_input.p2p_device_addr  = (u8 *)info->p2p_device_addr;
		supp_mgmt_if_ctx.notify_p2p_dev_input.pri_dev_type = (u8 *)info->pri_dev_type;
		sl_wpa_driver_wrapper(wpa_s, WPA_DRV_NOTIFY_P2P_DEV_FOUND, NULL);
	}
...
static void wpas_dev_lost(void *ctx, const u8 *dev_addr)
{
	...
	/* K60_PORTING */
	supp_mgmt_if_ctx.notify_p2p_dev_input.new_device = 0;
	supp_mgmt_if_ctx.notify_p2p_dev_input.device_name = NULL;
	supp_mgmt_if_ctx.notify_p2p_dev_input.p2p_device_addr  = (u8*)dev_addr;
	supp_mgmt_if_ctx.notify_p2p_dev_input.pri_dev_type = NULL;
	sl_wpa_driver_wrapper(wpa_s, WPA_DRV_NOTIFY_P2P_DEV_FOUND, NULL);
				     ...
				     struct p2p_channels *cli_chan)
{
	...
	if (chan->reg_class[cla].channels)
		cla++;
	sl_wpa_driver_wrapper(wpa_s, WPA_DRV_CHK_BAND_2_4GHZ, &is_band_24ghz);
 	if(!is_band_24ghz){
				    ...
				    const u8 *ssid, size_t ssid_len)
{
	...
	while ((s = wpas_p2p_get_persistent(wpa_s, peer, NULL, 0))) {
		...
		for (size_t i = 0; i < s->num_p2p_clients; i++) {
...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
	...
	p2p.send_probe_resp = NULL;
#ifdef SERVICE_DISCOVERY_SUPPORT
...
{
	struct p2p_device *p2p_dev = (struct p2p_device *)dev;
	u8 peer_addr[ETH_ALEN];
	char *pin = NULL;
	int wps_method = WPS_PBC;
	...
	supp_mgmt_if_ctx.join_p2p_dev = NULL;
	supp_mgmt_if_ctx.waiting_for_pd_resp = 0;
	wpa_s->driver->probe_req_report(NULL, 0);

	if(p2p_dev->info.group_capab & P2P_GROUP_CAPAB_GROUP_OWNER)
	{
		join = 1;
	}
	else
	...
	os_memcpy(peer_addr, p2p_dev->info.p2p_device_addr, ETH_ALEN);
	/* Give connect command to this device */

	return wpas_p2p_connect(wpa_s, peer_addr, pin, wps_method,
			persistent_group, auto_join, join, auth, go_intent, freq, -1, !(p2p_dev->flags & P2P_DEV_PD_REQUEST_RECEIVED), 0,0,0,0, 0 ,NULL, 0);
}

int wise_p2p_connect(void *ctx, uint8 *p2p_match_device)
{
	struct wpa_supplicant *wpa_s = ctx;
	struct p2p_data *p2p = wpa_s->global->p2p;
	struct p2p_device *dev, *p2p_dev = NULL;
	//uint8 *p2p_match_device = (uint8*)((wpa_s->conf->p2p_go_intent == 15) ? "Ralink_STA" :"Ralink_GO" );

	dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
		if ((os_memcmp(dev->info.device_name, p2p_match_device, 8) == 0) /*&& (dev->connect_reqs == 0) */)
		{
			//printf("P2P Device found \n"); /* WFD_PRINT */
			p2p_dev = dev;
			break;
		}
	}
	...
	if(p2p_dev)
	{
		supp_mgmt_if_ctx.join_p2p_dev = &p2p_dev;
		return	wise_p2p_join(wpa_s, p2p_dev);
	}
	else
	{
		return P2P_DEV_NOT_FOUND;
	}
}
				   ...
				   const struct p2p_channels *channels)
{
	...
	if (wpa_s->conf->p2p_oper_reg_class == 81 &&
	    ...
	    wpa_s->conf->p2p_oper_channel <= 13 &&
