...
uint16_t IEEE754_FloatToHalf(float f)
{
  ...
  const int32_t nSingleUnbiasedExponent =
    (int32_t)((uSingle & SINGLE_EXPONENT_MASK) >> SINGLE_EXPONENT_SHIFT) - SINGLE_EXPONENT_BIAS;
  ...
  const uint32_t uHalfPrecision = uHalfSignificand | (uHalfBiasedExponent << HALF_EXPONENT_SHIFT)
...
uint16_t IEEE754_DoubleToHalf(double d)
{
  ...
  const int64_t nDoubleUnbiasedExponent =
  ...
  if (nDoubleUnbiasedExponent == DOUBLE_EXPONENT_INF_OR_NAN) {
    ...
    uHalfBiasedExponent = 0;
    uHalfSignificand =
      0; // Any subnormal single will be too small to express as a half precision; TODO, is this really true?
  ...
  // Put the 3 values in the right place for a half precision
  const uint64_t uHalfPrecision = uHalfSignificand | (uHalfBiasedExponent << HALF_EXPONENT_SHIFT)
                                  | (uHalfSign << HALF_SIGN_SHIFT);
  // Cast is safe because all the masks and shifts above work to
  // make a half precision value which is only 16 bits.
  return (uint16_t)uHalfPrecision;
...
double IEEE754_HalfToDouble(uint16_t uHalfPrecision)
{
  ...
  const int64_t nHalfUnBiasedExponent =
    (int64_t)((uHalfPrecision & HALF_EXPONENT_MASK) >> HALF_EXPONENT_SHIFT) - HALF_EXPONENT_BIAS;
  ...
  const uint64_t uDouble = uDoubleSignificand | (uDoubleBiasedExponent << DOUBLE_EXPONENT_SHIFT)
...
IEEE754_union IEEE754_FloatToSmallest(float f)
{
  ...
  if (uSingle == 0) {
    ...
    result.uSize  = IEEE754_UNION_IS_HALF;
    result.uValue = IEEE754_FloatToHalf(f);
  } else if ((nSingleExponent >= HALF_EXPONENT_MIN) && nSingleExponent <= HALF_EXPONENT_MAX
             && (!(uSingleSignificand & uDroppedSingleBits))) {
...
IEEE754_union IEEE754_DoubleToSmallestInternal(double d, int bAllowHalfPrecision)
{
  ...
  if (d == 0.0) { // Take care of positive and negative zero
    ...
    result.uSize  = IEEE754_UNION_IS_HALF;
    result.uValue = IEEE754_DoubleToHalf(d);
  } else if (bAllowHalfPrecision && (nDoubleExponent >= HALF_EXPONENT_MIN) && nDoubleExponent <= HALF_EXPONENT_MAX
             && (!(uDoubleSignificand & uDroppedHalfBits))) {
    // Can convert to half without precision loss
    result.uSize  = IEEE754_UNION_IS_HALF;
    result.uValue = IEEE754_DoubleToHalf(d);
  } else if ((nDoubleExponent >= SINGLE_EXPONENT_MIN) && nDoubleExponent <= SINGLE_EXPONENT_MAX
             && (!(uDoubleSignificand & uDroppedSingleBits))) {
