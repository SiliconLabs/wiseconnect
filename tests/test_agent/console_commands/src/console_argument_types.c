...
const char *ap_client_deauth_type[]    = { "unspecified", NULL };
...
const char *cert_type_type[]           = { "ca_cert",     "client_cert", "eap_client",  "http_client",
                                           "mqtt_client", "private_key", "puiblic_key", NULL };
const char *continuous_flag_type[]     = { "continuous", "one_time", NULL };
const char *credential_id_type[]       = { "http_client",     "http_server", "mqtt_client", "mqtt_server", "tls_client",
                                           "tls_server",      "user",        "wifi_ap",     "wifi_client", "wifi_eap_client",
                                           "wifi_eap_server", NULL };
...
const char *http_client_tls_version_type[] = { "tls_v_1_0", "tls_v_1_1", "tls_v_1_2", "tls_v_1_3", NULL };
const char *http_client_version_type[]     = { "http_v_1_0", "http_v_1_1", NULL };
const char *ip_protocol_type[]             = { "tcp", "udp", NULL };
const char *ipv4_or_ipv6_type[]            = { "ipv4", "ipv6", NULL };
const char *mqtt_qos_levels_type[]         = { "qos0", "qos1", "qos2", NULL };
...
const char *option_level_type[]            = { "SOL_SOCKET", "SOL_TCP", NULL };
const char *option_name_type[]             = { "SL_SI91X_SO_MAXRETRY",
                                               "SL_SI91X_SO_MSS",
                                               "SO_CERT_INDEX",
                                               "SO_KEEPALIVE",
...
const char *region_code_type[]   = { "default", "europe", "japan", "korea", "singapore", "us", "worldwide", NULL };
...
const char *sl_wifi_timeout_type_t_type[]          = { "authentication_association_timer",
...
const arg_list_t console_argument_types[] = {
  ...
  [CONSOLE_TYPE(cert_type)]                       = cert_type_type,
  [CONSOLE_TYPE(continuous_flag)]                 = continuous_flag_type,
  [CONSOLE_TYPE(credential_id)]                   = credential_id_type,
  ...
  [CONSOLE_TYPE(http_client_tls_version)]         = http_client_tls_version_type,
  [CONSOLE_TYPE(http_client_version)]             = http_client_version_type,
  [CONSOLE_TYPE(ip_protocol)]                     = ip_protocol_type,
  [CONSOLE_TYPE(ipv4_or_ipv6)]                    = ipv4_or_ipv6_type,
  [CONSOLE_TYPE(mqtt_qos_levels)]                 = mqtt_qos_levels_type,
  [CONSOLE_TYPE(net_interface)]                   = net_interface_type,
  [CONSOLE_TYPE(operating_band)]                  = operating_band_type,
  [CONSOLE_TYPE(option_level)]                    = option_level_type,
  ...
  [CONSOLE_TYPE(region_code)]                     = region_code_type,
  ...
  [CONSOLE_TYPE(sl_wifi_timeout_type_t)]          = sl_wifi_timeout_type_t_type,
...
const value_list_t console_argument_values[] = {
  [CONSOLE_TYPE(ap_client_deauth)]    = (const uint32_t[]){ SL_WIFI_DEAUTH_UNSPECIFIED },
  ...
  [CONSOLE_TYPE(cert_type)]           = (const uint32_t[]){ SL_NET_SIGNING_CERTIFICATE,
                                                            SL_NET_CERTIFICATE,
  ...
  [CONSOLE_TYPE(credential_id)]       = (const uint32_t[]){ SL_NET_HTTP_CLIENT_CREDENTIAL_START,
  ...
  [CONSOLE_TYPE(http_client_tls_version)] =
    (const uint32_t[]){ SL_TLS_V_1_0, SL_TLS_V_1_1, SL_TLS_V_1_2, SL_TLS_V_1_3 },
  [CONSOLE_TYPE(http_client_version)] = (const uint32_t[]){ SL_HTTP_V_1_0, SL_HTTP_V_1_1 },
  ...
  [CONSOLE_TYPE(mqtt_qos_levels)] = (const uint32_t[]){ SL_MQTT_QOS_LEVEL_0, SL_MQTT_QOS_LEVEL_1, SL_MQTT_QOS_LEVEL_2 },
  ...
  [CONSOLE_TYPE(option_level)]    = (const uint32_t[]){ 65535, 6 },
  [CONSOLE_TYPE(option_name)]     = (const uint32_t[]){ 24, 40, 4134, 8, 12306, 4102, 31 },
  ...
  [CONSOLE_TYPE(region_code)]      = (const uint32_t[]){ DEFAULT_REGION, EU, JP, KR, SG, US, WORLD_DOMAIN },
  ...
  [CONSOLE_TYPE(sl_wifi_timeout_type_t)]          = (const uint32_t[]){ SL_SI91X_AUTHENTICATION_ASSOCIATION_TIMEOUT,
