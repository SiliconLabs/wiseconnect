...
extern osSemaphoreId_t ble_main_task_sem, ble_slave_conn_sem;
#if ENABLE_POWER_SAVE
extern uint8_t powersave_cmd_given;
#endif
/*========================================================================*/
//!  CALLBACK FUNCTIONS
/*=======================================================================*/
...
{
  LOG_EVENT("rsi_check_dev_master_list \n");
  uint8_t i                   = 0;
...
uint8_t rsi_check_dev_list(uint8_t *remote_dev_name, uint8_t *adv_dev_addr)
{
  ...
  if (slave_device_found == DEV_FOUND) {
    for (i = 0; i < TOTAL_CONNECTIONS; i++) {
      if (!(strcmp((const char *)rsi_ble_conn_info[i].rsi_remote_name, (const char *)remote_dev_name))) {
        ...
        LOG_PRINT("Device %s already connected!!!\n", adv_dev_addr);
        break;
      }
    }
  }
  ...
  if (!strcmp(RSI_BLE_DEV_1_ADDR, (char *)adv_dev_addr)) {
    ...
    slave_device_found = DEV_FOUND;
  } else if (!strcmp((char *)target_ble_remote_dev_addr, (char *)adv_dev_addr)) {
    //TODO : This is new else clause
    // Review - target_ble_remote_dev_addr is assigned when ble_start_scanning is called can be found in ble_commands.c
    slave_device_found = DEV_FOUND;
  } else
    return slave_device_found;
...
static void rsi_ble_add_char_serv_att(void *serv_handler,
                                      ...
                                      uuid_t att_val_uuid)
{
  LOG_PRINT("rsi_ble_add_char_serv_att\n");
...
void rsi_gatt_add_attribute_to_list(rsi_ble_t *p_val,
                                    ...
                                    uint8_t char_prop)
{
  LOG_PRINT("rsi_gatt_add_attribute_to_list\n");
...
{
  LOG_PRINT("rsi_gatt_get_attribute_from_list\n");
  uint16_t i;
...
static void rsi_ble_add_char_val_att(void *serv_handler,
                                     ...
                                     uint8_t auth_read)
{
  LOG_PRINT("rsi_ble_add_char_val_att\n");
  rsi_ble_req_add_att_t new_att = { 0 };
...
{
  LOG_PRINT("rsi_ble_add_simple_chat_serv\n");
  uuid_t new_uuid                       = { 0 };
...
{
  LOG_PRINT("rsi_ble_add_simple_chat_serv3\n");
...
{
  LOG_PRINT("rsi_ble_add_custom_service_serv\n");
  uuid_t new_uuid                       = { 0 };
  rsi_ble_resp_add_serv_t new_serv_resp = { 0 };
  uint8_t data[1]                       = { 90 };
...
/*==============================================*/
/**
 * @fn         rsi_ble_simple_peripheral_on_remote_features_event_gatt
 ...
 */
void rsi_ble_simple_peripheral_on_remote_features_event_gatt(
  rsi_ble_event_remote_features_t *rsi_ble_event_remote_features)
{
  LOG_PRINT("rsi_ble_simple_peripheral_on_remote_features_event_gatt\n");
...
/*==============================================*/
/**
 * @fn         rsi_ble_simple_central_on_adv_report_event_gatt
 ...
 */
void rsi_ble_simple_central_on_adv_report_event_gatt(rsi_ble_event_adv_report_t *adv_report)
{
  PRINT_EVENT_START(ADVERTISE, BLE)
  uint8_t device_already_connected = 0;
  uint8_t count                    = 0;
  rsi_6byte_dev_address_to_ascii(remote_dev_addr, adv_report->dev_addr);
  LOG_EVENT("Found : %s\n", remote_dev_addr);
  if (strcmp((const char *)remote_dev_addr, (const char *)target_ble_remote_dev_addr) == 0) {
    LOG_EVENT(
      " ====rsi_ble_simple_central_on_adv_report_event_gatt- dev_addr: %s, dev_addr_type: %d, adv_data_len: %d, "
  ...
  //LOG_PRINT("advertised details remote_name = %s, dev_addr = %s \n",remote_name,(int8_t *) remote_dev_addr);
  ...
  if (device_already_connected == DEV_NOT_CONNECTED) {
    ...
    if (ble_conn_id < TOTAL_CONNECTIONS) {
      ...
      //! wrong state
      LOG_EVENT("connection identifier is wrong \n");
      // while (1)
      //   ;
    }
  }
...
/*==============================================*/
/**
 * @fn         rsi_ble_on_connect_event_gatt
 ...
 * This callback function indicates the status of the connection
 */
static void rsi_ble_on_connect_event_gatt(rsi_ble_event_conn_status_t *resp_conn)
{
  LOG_EVENT("rsi_ble_on_connect_event_gatt\n");
  ...
  if (resp_conn->status != 0) {
    //LOG_PRINT("On connect event status report : %d", resp_conn->status);
  ...
  if (remote_device_role == SLAVE_ROLE) //check for the connection is from slave or master
  {
    ...
    if (master_dev_found == DEV_NOT_CONNECTED) {
      ...
      if (ble_conn_id < TOTAL_CONNECTIONS) {
        ...
        memcpy(&rsi_ble_conn_info[ble_conn_id].conn_event_to_app, resp_conn, sizeof(rsi_ble_event_conn_status_t));
        rsi_ble_conn_info[ble_conn_id].is_enhanced_conn = 0;
...
/*==============================================*/
/**
 * @fn         rsi_ble_on_enhance_conn_status_event_gatt
 ...
 * This callback function indicates the status of the connection
 */
void rsi_ble_on_enhance_conn_status_event_gatt(rsi_ble_event_enhance_conn_status_t *resp_enh_conn)
{
  LOG_PRINT("rsi_ble_on_enhance_conn_status_event_gatt\n");
  uint8_t master_dev_found = NO_DEV_FOUND;
    ...
    if (master_dev_found == DEV_NOT_CONNECTED) {
      ...
      if (ble_conn_id < TOTAL_CONNECTIONS) {
        ...
        memcpy(&rsi_ble_conn_info[ble_conn_id].rsi_enhc_conn_status,
               resp_enh_conn,
               sizeof(rsi_ble_event_enhance_conn_status_t));
        rsi_ble_conn_info[ble_conn_id].is_enhanced_conn = 1;
        master_conn_id                                  = ble_conn_id;
        //! set common event
...
/*==============================================*/
/**
 * @fn         ble_on_conn_update_complete_event_gatt
 ...
 * This callback function indicates the status of the connection
 */
static void ble_on_conn_update_complete_event_gatt(rsi_ble_event_conn_update_t *resp_conn_update, uint16_t resp_status)
{
  LOG_PRINT("ble_on_conn_update_complete_event_gatt\n");
  LOG_PRINT("In conn update cb\n");
  if (resp_status != 0) {
    LOG_PRINT("RSI_BLE_CONN_UPDATE_EVENT FAILED : %d\n", resp_status);
    return;
  } else {
    //! convert to ascii
...
/*==============================================*/
/**
 * @fn         rsi_ble_on_remote_conn_params_request_event_gatt
 ...
 * This callback function indicates the status of the connection
 */
static void rsi_ble_on_remote_conn_params_request_event_gatt(rsi_ble_event_remote_conn_param_req_t *remote_conn_param,
                                                             uint16_t status)
{
  LOG_PRINT("rsi_ble_on_remote_conn_params_request_event_gatt\n");
  //! convert to ascii
...
/*==============================================*/
/**
 * @fn         rsi_ble_on_disconnect_event_gatt
 ...
 */
static void rsi_ble_on_disconnect_event_gatt(rsi_ble_event_disconnect_t *resp_disconnect, uint16_t reason)
{

  LOG_PRINT("rsi_ble_on_disconnect_event_gatt\n");
  //! convert to ascii
 ...
 */
static void rsi_ble_on_event_write_resp_gatt(uint16_t event_status, rsi_ble_set_att_resp_t *rsi_ble_event_set_att_rs)
{
  LOG_PRINT("rsi_ble_on_event_write_resp_gatt\n");
  //! convert to ascii
  LOG_PRINT("in write resp event\n");
  ...
  if (event_status != RSI_SUCCESS) {
    LOG_PRINT("write event response failed , error : 0x%x\n", event_status);
  } else {
    //! set conn specific event
...
/*==============================================*/
/**
 * @fn         rsi_ble_on_event_indication_confirmation_gatt
 ...
 * @section description
 */
static void rsi_ble_on_event_indication_confirmation_gatt(uint16_t resp_status,
                                                          rsi_ble_set_att_resp_t *rsi_ble_event_set_att_rsp)
{

  LOG_PRINT("rsi_ble_on_event_indication_confirmation_gatt\n");
...
/*==============================================*/
/**
 * @fn         rsi_ble_on_gatt_write_event_gatt
 ...
 */
static void rsi_ble_on_gatt_write_event_gatt(uint16_t event_id, rsi_ble_event_write_t *rsi_ble_write)
{
  LOG_PRINT("rsi_ble_on_gatt_write_event_gatt\n");
  LOG_PRINT("in write event \n");
  //! convert to ascii
  ...
  LOG_EVENT("\nrsi_ble_on_gatt_write_event_gatt - att_value - %d\n",
            *(uint8_t *)rsi_ble_conn_info[ble_conn_id].app_ble_write_event.att_value);
  LOG_EVENT("\nrsi_ble_on_gatt_write_event_gatt - resolved\n");
  //! set conn specific event
...
/*==============================================*/
/**
 * @fn         rsi_ble_on_gatt_prepare_write_event_gatt
 ...
 * This callback function is invoked when write/notify/indication events are received
 */
static void rsi_ble_on_gatt_prepare_write_event_gatt(uint16_t event_id,
                                                     rsi_ble_event_prepare_write_t *rsi_app_ble_prepared_write_event)
{
  LOG_PRINT("rsi_ble_on_gatt_prepare_write_event_gatt\n");
  LOG_PRINT("in rsi_ble_on_gatt_prepare_write_event_gatt \n");
  //! convert to ascii
...
static void rsi_ble_on_execute_write_event(uint16_t event_id, rsi_ble_execute_write_t *rsi_app_ble_execute_write_event)
{
  ...
  LOG_PRINT("rsi_ble_on_execute_write_event\n");
  LOG_PRINT("in rsi_ble_on_execute_write_event \n");
  //! convert to ascii
...
/*==============================================*/
/**
 * @fn         rsi_ble_on_read_req_event_gatt
 ...
 * This callback function is invoked when read events are received
 */
static void rsi_ble_on_read_req_event_gatt(uint16_t event_id, rsi_ble_read_req_t *rsi_ble_read_req)
{
  LOG_PRINT("rsi_ble_on_read_req_event_gatt\n");
  //! convert to ascii
...
static void rsi_ble_on_event_mtu_exchange_info_gatt(
  rsi_ble_event_mtu_exchange_information_t *rsi_ble_event_mtu_exchange_info)
{
  LOG_PRINT("rsi_ble_on_event_mtu_exchange_info_gatt\n");
  //! convert to ascii
 ...
 */
static void rsi_ble_on_mtu_event_gatt(rsi_ble_event_mtu_t *rsi_ble_mtu)
{
  LOG_PRINT("rsi_ble_on_mtu_event_gatt\n");
  //! convert to ascii
 ...
 */
static void rsi_ble_profile_event_gatt(uint16_t resp_status, rsi_ble_event_profile_by_uuid_t *rsi_ble_event_profile)
{
  LOG_PRINT("rsi_ble_profile_event_gatt\n");
...
/*==============================================*/
/**
 * @fn         rsi_ble_char_services_event_gatt
 ...
 * @section description
 */
static void rsi_ble_char_services_event_gatt(uint16_t resp_status,
                                             rsi_ble_event_read_by_type1_t *rsi_ble_event_char_services)
{
  LOG_PRINT("rsi_ble_char_services_event_gatt\n");
  //! convert to ascii
...
/*==============================================*/
/**
 * @fn         rsi_ble_on_read_resp_event_gatt
 ...
 * @section description
 */
static void rsi_ble_on_read_resp_event_gatt(uint16_t event_status, rsi_ble_event_att_value_t *rsi_ble_event_att_val)
{
  PRINT_EVENT_START(GET_ATT_DESC, BLE);
  LOG_EVENT("rsi_ble_on_read_resp_event_gatt\n");
  ...
  LOG_EVENT("\nrsi_ble_on_read_resp_event_gatt - att_value - %d\n",
            *(uint8_t *)rsi_ble_conn_info[ble_conn_id].rsi_char_descriptors.att_value);
  LOG_EVENT("\nrsi_ble_on_read_resp_event_gatt - resolved\n");
...
/*==============================================*/
/**
 * @fn         rsi_ble_on_event_read_att_value
 * @brief      invoked when response is received for get value att
 * @param[out] rsi_ble_on_event_read_att_value_t, contains att values
 * @return     none
 * @section description
 */
static void rsi_ble_on_event_read_att_value(uint16_t event_status,
                                            rsi_ble_event_read_by_type3_t *rsi_ble_event_read_type3)
{
  PRINT_EVENT_START(GET_ATT_VALUE, BLE);
  LOG_EVENT("rsi_ble_on_event_read_att_value length:%d\n", rsi_ble_event_read_type3->length);
  gatt_start_handle = rsi_bytes2R_to_uint16(rsi_ble_event_read_type3->handle);
  LOG_EVENT("att_value:%d,%d, handle:0x%02x\n",
            rsi_ble_event_read_type3->att_value[0],
            rsi_ble_event_read_type3->att_value[1],
            gatt_start_handle);
  LOG_EVENT("RSI_BLE_READ_ATT_VALUE_EVT - resolved\n");
  PRINT_EVENT_END();
}
...
{
  LOG_PRINT("rsi_ble_phy_update_complete_event\n");
  //! convert to ascii
...
/*==============================================*/
/**
 * @fn         rsi_ble_on_smp_request_gatt
 ...
 */
static void rsi_ble_on_smp_request_gatt(rsi_bt_event_smp_req_t *remote_smp)
{

  LOG_PRINT("rsi_ble_on_smp_request_gatt\n");
  //! convert to ascii
...
/*==============================================*/
/**
 * @fn         rsi_ble_on_smp_response_gatt
 ...
 */
void rsi_ble_on_smp_response_gatt(rsi_bt_event_smp_resp_t *remote_smp)
{
  LOG_PRINT("rsi_ble_on_smp_response_gatt\n");
...
/*==============================================*/
/**
 * @fn         rsi_ble_on_smp_passkey_gatt
 ...
 */
static void rsi_ble_on_smp_passkey_gatt(rsi_bt_event_smp_passkey_t *smp_pass_key)
{
  LOG_PRINT("rsi_ble_on_smp_passkey_gatt\n");
...
/*==============================================*/
/**
 * @fn         rsi_ble_on_smp_passkey_display_gatt
 * @brief      its invoked when smp passkey event is received.
 * @param[in]  remote_dev_address, it indicates remote bd address.
 * @return     none.
 * @section description
 ...
 * Note: We have to send SMP passkey command
 */
static void rsi_ble_on_smp_passkey_display_gatt(rsi_bt_event_smp_passkey_display_t *smp_passkey_display)
{
  LOG_PRINT("rsi_ble_on_smp_passkey_display_gatt\n");
 ...
 * Note: We have to send SMP passkey command
 */
static void rsi_ble_on_sc_passkey_gatt(rsi_bt_event_sc_passkey_t *sc_passkey)
{
  LOG_PRINT("rsi_ble_on_sc_passkey_gatt\n");
  //! convert to ascii
 ...
 */
static void rsi_ble_on_smp_failed_gatt(uint16_t status, rsi_bt_event_smp_failed_t *remote_dev_address)
{
  LOG_PRINT("rsi_ble_on_smp_failed_gatt\n");
  ...
  LOG_PRINT("SMP failed for remote address : %s with status : %x", remote_dev_addr_conn, status);
...
/*==============================================*/
/**
 * @fn         rsi_ble_on_encrypt_started_gatt
 ...
 */
static void rsi_ble_on_encrypt_started_gatt(uint16_t status, rsi_bt_event_encryption_enabled_t *enc_enabled)
{
  LOG_PRINT("rsi_ble_on_encrypt_started_gatt\n");
  //! convert to ascii
...
/*==============================================*/
/**
 * @fn         rsi_ble_on_le_ltk_req_event_gatt
 * @brief      invoked when disconnection event is received
 * @param[in]  resp_disconnect, disconnected remote device information
 * @param[in]  reason, reason for disconnection.
 * @return     none.
 ...
 * This callback function indicates disconnected device information and status
 */
static void rsi_ble_on_le_ltk_req_event_gatt(rsi_bt_event_le_ltk_request_t *le_ltk_req)
{
  LOG_PRINT("rsi_ble_on_le_ltk_req_event_gatt\n");
  //! convert to ascii
...
/*==============================================*/
/**
 * @fn         rsi_ble_on_le_security_keys_event_gatt
 ...
 */
static void rsi_ble_on_le_security_keys_event_gatt(rsi_bt_event_le_security_keys_t *le_sec_keys)
{
  LOG_PRINT("rsi_ble_on_le_security_keys_event_gatt\n");
  //! convert to ascii
...
/*==============================================*/
/**
 * @fn         rsi_ble_gatt_error_event_gatt
 ...
 * @section description
 */
static void rsi_ble_gatt_error_event_gatt(uint16_t resp_status, rsi_ble_event_error_resp_t *rsi_ble_gatt_error)
{
  LOG_PRINT("rsi_ble_gatt_error_event_gatt\n");
  //! convert to ascii
...
static void rsi_ble_more_data_req_event_gatt(rsi_ble_event_le_dev_buf_ind_t *rsi_ble_more_data_evt)
{
  LOG_PRINT("rsi_ble_more_data_req_event_gatt\n");
 ...
 */
void rsi_ble_on_sc_method_gatt(rsi_bt_event_sc_method_t *scmethod)
{
  LOG_PRINT("rsi_ble_on_sc_method_gatt\n");
...
static int32_t rsi_ble_dual_role(void)
{
  ...
  //TODO: Not requried for current test case as we are only advertising
  ...
  rsi_ble_gap_register_callbacks(rsi_ble_simple_central_on_adv_report_event_gatt,
                                 rsi_ble_on_connect_event_gatt,
                                 rsi_ble_on_disconnect_event_gatt,
                                 NULL,
                                 NULL, /*rsi_ble_phy_update_complete_event*/
                                 NULL, /*rsi_ble_data_length_change_event*/
                                 rsi_ble_on_enhance_conn_status_event_gatt,
                                 NULL,
                                 ble_on_conn_update_complete_event_gatt,
                                 rsi_ble_on_remote_conn_params_request_event_gatt);
  LOG_PRINT("rsi_ble_gap_register_callbacks\n");
  ...
  rsi_ble_gatt_register_callbacks(NULL,
                                  ...
                                  NULL,
                                  NULL,
                                  rsi_ble_on_gatt_write_event_gatt,
                                  rsi_ble_on_gatt_prepare_write_event_gatt,
                                  rsi_ble_on_execute_write_event,
                                  rsi_ble_on_read_req_event_gatt,
                                  rsi_ble_on_mtu_event_gatt,
                                  rsi_ble_gatt_error_event_gatt,
                                  NULL,
                                  rsi_ble_profiles_list_event_gatt,
                                  rsi_ble_profile_event_gatt,
                                  rsi_ble_char_services_event_gatt,
                                  NULL,
                                  rsi_ble_on_event_read_att_value,
                                  rsi_ble_on_read_resp_event_gatt,
                                  rsi_ble_on_event_write_resp_gatt,
                                  rsi_ble_on_event_indication_confirmation_gatt,
                                  NULL);
  LOG_PRINT("rsi_ble_gatt_register_callbacks\n");

  rsi_ble_gatt_extended_register_callbacks(rsi_ble_on_event_mtu_exchange_info_gatt);
  rsi_ble_gap_extended_register_callbacks(rsi_ble_simple_peripheral_on_remote_features_event_gatt,
                                          rsi_ble_more_data_req_event_gatt);
  LOG_PRINT("rsi_ble_gatt_extended_register_callbacks\n");

#if SMP_ENABLE
  //! registering the SMP callback functions
  rsi_ble_smp_register_callbacks(rsi_ble_on_smp_request_gatt,
                                 rsi_ble_on_smp_response_gatt,
                                 rsi_ble_on_smp_passkey_gatt,
                                 rsi_ble_on_smp_failed_gatt,
                                 rsi_ble_on_encrypt_started_gatt,
                                 rsi_ble_on_smp_passkey_display_gatt,
                                 rsi_ble_on_sc_passkey_gatt,
                                 rsi_ble_on_le_ltk_req_event_gatt,
                                 rsi_ble_on_le_security_keys_event_gatt,
                                 NULL,
#if SIG_VUL
                                 rsi_ble_on_sc_method_gatt
#else
                                 NULL
#endif
  );
  LOG_PRINT("rsi_ble_smp_register_callbacks\n");
#endif
  ...
  // LOG_PRINT("rsi_ble_set_local_irk_value\n");
  // print_status(status);
  // if (status != RSI_SUCCESS) {
  //   LOG_PRINT("Setting the Local IRK Value Failed = %lx\n", status);
  //   return status;
  // }
  ...
  // //TODO start advertisement to be removed from here and called from UTF
  ...
  //   LOG_PRINT("rsi_ble_start_advertising\n");
  //   print_status(status);
  //   if (status != RSI_SUCCESS) {
  //     LOG_PRINT("advertising failed \n");
  //   }
  //   LOG_PRINT("advertising started \n");
  // }

  // //TODO start scanning to be removed from here and called from UTF
...
#if ENABLE_POWER_SAVE
  LOG_PRINT("Initiate module in to power save \n");
  if (!powersave_cmd_given) {
    ...
    LOG_PRINT("Module is in power save \n");
...
void rsi_ble_main_app_task()
{
  ...
  while (1) {
    ...
    if (p_event_id1 != event_id) {
      LOG_EVENT("main task : event_id received for %ld \n", event_id);
    }
    switch (event_id) {
        ...
        if (slave_task_instances < RSI_BLE_MAX_NBR_SLAVES) {
          ...
          if ((slave_conn_id < TOTAL_CONNECTIONS) && (slave_con_req_pending == 1)) {
            ...
            const osThreadAttr_t attr = {
              .name       = "ble_slave_task",
              .priority   = RSI_BLE_APP_TASK_PRIORITY,
              .stack_mem  = 0,
              .stack_size = RSI_BLE_APP_TASK_SIZE,
              .cb_mem     = 0,
              .cb_size    = 0,
              .attr_bits  = 0u,
              .tz_module  = 0u,
            ...
            //                                     &ble_app_task_handle[slave_conn_id]);
            LOG_EVENT("rsi_task_create slave_conn_id %d \n", slave_conn_id);
            if (ble_app_task_handle[slave_conn_id] == 0) {
              LOG_PRINT("task%d failed to create, reason = %ld\n", slave_conn_id, status);
              slave_con_req_pending = 0;
            ...
            LOG_EVENT("in wrong state \n");
            // while (1)
            //   ;
          }
        } else {
          LOG_EVENT("Maximum slave connections reached\n");
        }
      } break;
      case RSI_BLE_CONN_EVENT: {
        //! event invokes when connection was completed
        LOG_EVENT("In on comm-conn evt\n");
        ...
        if (remote_device_role == MASTER_ROLE) {
          if (master_task_instances < RSI_BLE_MAX_NBR_MASTERS) {
            ...
            if (master_conn_id < TOTAL_CONNECTIONS) {
              ...
              ble_app_task_handle[master_conn_id] = osThreadNew(
                rsi_ble_task_on_conn,
                &rsi_parsed_conf.rsi_ble_config.rsi_ble_conn_config[RSI_BLE_MAX_NBR_SLAVES + master_task_instances],
                &attr);
              ...
              LOG_PRINT("rsi_task_create master_conn_id %d \n", master_conn_id);
              //              print_status(status);

              if (ble_app_task_handle[master_conn_id] == 0) {
                LOG_PRINT("task%d failed to create\n", master_conn_id);
      ...
      case RSI_BLE_GATT_PROFILES: {
        ...
        LOG_PRINT("in gatt test:rsi_ble_gatt_profiles \n");
      } break;
      ...
      case RSI_BLE_SCAN_RESTART_EVENT: {
        ...
        //        print_status(status);
        if (status != RSI_SUCCESS) {
          LOG_PRINT("scanning stop cmd status = %lx\n", status);
          //return status;	//! TODO
        } else {
        ...
        //        print_status(status);
        if (status != RSI_SUCCESS) {
          LOG_PRINT("scanning start cmd status = %lx\n", status);
          //return status;	//! TODO
        } else {
        ...
        if (status != RSI_SUCCESS) {
          LOG_PRINT("Restart_scanning\n");
          rsi_ble_app_set_event(RSI_BLE_SCAN_RESTART_EVENT);
        }
      } break;
      case RSI_APP_EVENT_REMOTE_CONN_PARAM_REQ: {
        LOG_PRINT("In conn params req evt\n");
